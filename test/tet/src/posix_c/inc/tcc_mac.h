...
#define BAIL_OUT(_msg) 		{ 					\
					int errnotmp; 			\
					errnotmp = errno;		\
					(void) fprintf(stderr,"\nTcc fatal error reported from source file %s line %d :\n",__FILE__,__LINE__); 		\
					errno = errnotmp;		\
					perror(_msg);			\
					(void) fprintf(stderr,"\n");	\
					tet_shutdown(); 		\
...
#define ERROR(_msg) tet_error(_msg, NONFATAL, __FILE__, __LINE__)
...
#define FATAL_ERROR(_msg)	{					     \
					tet_error(_msg, FATAL, __FILE__, __LINE__); \
					tet_shutdown(); 		     \
				}
