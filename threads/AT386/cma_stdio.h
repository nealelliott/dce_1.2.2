 ...
 * HISTORY
 * $Log: cma_stdio.h,v $
 * Revision 1.1.4.2  1996/02/18  23:08:11  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  22:30:58  marty]
 *
 * Revision 1.1.4.1  1995/12/07  21:43:53  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/07  20:58:29  root]
 * 
 * Revision 1.1.2.3  1994/06/09  13:33:39  devsrc
 * 	 CR10892 - fix copyright in file
 * 	[1994/06/09  13:24:53  devsrc]
 * 
 * Revision 1.1.2.2  1994/05/06  16:43:47  jd
 * 	Moved define of system and popen to within NOWRAPPER conditional.
 * 	[1994/05/06  15:34:53  jd]
 * 
 * Revision 1.1.2.1  1994/04/01  20:13:10  jd
 * 	Moved from src/threads to here.
 * 	[1994/04/01  16:24:42  jd]
 * 
...
/*
 * On OSF/1 (or other platforms that don't disable the process
 * virtual interval timer across an exec()), wrap functions
 * that call exec() internally.  This should be viewed as a
 * temporary workaround only -- these systems really should
 * disable the timer properly.
 *
 * syslog(3) should be included here, as well -- but it's
 * a variadic function (i.e., uses stdarg), and there's no
 * way to wrap it portably.
 */
...
extern char     *cma_ttyname (int);
extern int      cma_vfprintf (FILE *, const char *, va_list);
extern int      cma_vprintf (const char *, va_list);
extern int      cma_vsprintf (char *, const char *, va_list);
extern int      cma_scanf (const char *, ...);
extern int      cma_fscanf (FILE *, const char *, ...);
extern int      cma_sscanf (const char *, const char *, ...);
int             cma_pclose (FILE *);
void            cma_setbuffer (FILE *, char*, int);
extern char      *cma_tempnam (const char*, const char*);
void            cma_setlinebuf (FILE *);
extern int      cma_getc (FILE *);
extern int      cma_getchar (void);
...
extern char     *cma_mktemp(char *);
extern char     cma_ungetc(int,FILE *);
extern int      cma_ttyslot(void);
#endif
