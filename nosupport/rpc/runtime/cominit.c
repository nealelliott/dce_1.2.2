 ...
 * HISTORY
 * $Log: cominit.c,v $
 * Revision 1.1.2.1  1996/10/29  17:07:03  arvind
 * 	OSF DCE 1.2.2 No support
 * 	[1996/10/29  17:06:29  arvind]
 *
 * Revision /main/DCE_1.2.2/1  1996/05/24  23:12 UTC  sommerfeld
 ...
 * 
 * Revision 1.1.97.2  1996/02/18  00:02:35  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  22:55:15  marty]
 * 
 * Revision 1.1.97.1  1995/12/08  00:18:16  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/HPDCE02/gaz_dce_instr/jrr_1.2_mothra/1  1995/11/16  21:30 UTC  jrr
 * 	Remove Mothra specific code
 * 
 * 	HP revision /main/HPDCE02/gaz_dce_instr/5  1995/08/24  20:28 UTC  gaz
 ...
 * 
 * 	HP revision /main/HPDCE02/gaz_dce_instr/3  1995/07/17  20:41 UTC  gaz
 * 	Merge out RPC atfork() fix
 * 
 * 	HP revision /main/HPDCE02/11  1995/07/14  19:06 UTC  tatsu_s
 * 	Submitted the fix for CHFts15608.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.local_rpc.b4/1  1995/07/06  20:58 UTC  tatsu_s
 * 	Do nothing in the exit handler, if RPC runtime is not initialized.
 * 
 * 	HP revision /main/HPDCE02/gaz_dce_instr/2  1995/06/16  15:04 UTC  gaz
 * 	Merge out memory allocation failure detection logic
 * 
 * 	HP revision /main/HPDCE02/10  1995/05/25  21:40 UTC  lmm
 * 	Merge allocation failure detection cleanup work
 * 
 * 	HP revision /main/HPDCE02/lmm_alloc_fail_clnup/1  1995/05/25  21:00 UTC  lmm
 * 	allocation failure detection cleanup
 * 
 * 	HP revision /main/HPDCE02/9  1995/04/02  01:14 UTC  lmm
 * 	Merge allocation failure detection functionality into Mothra
 * 
 * 	HP revision /main/HPDCE02/lmm_rpc_alloc_fail_detect/1  1995/04/02  00:18 UTC  lmm
 * 	add memory allocation failure detection
 * 
 * 	HP revision /main/HPDCE02/gaz_dce_instr/1  1995/06/16  14:39 UTC  gaz
 * 	Hook DMS observer initialization into rpc__init() [actually init_once()]
 * 	Hook DMS fork handling into rpc__fork_handler()
 * 
 * 	HP revision /main/HPDCE02/8  1995/03/06  17:19 UTC  tatsu_s
 * 	Submitted the local rpc security bypass.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.local_rpc.b1/2  1995/03/01  19:33 UTC  tatsu_s
 * 	Fixed KRPC.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.local_rpc.b1/1  1995/02/16  19:51 UTC  tatsu_s
 * 	Added the backdoor to disable the single-thread client.
 * 
 * 	HP revision /main/HPDCE02/7  1995/02/07  20:56 UTC  tatsu_s
 * 	Submitted CHFtsthe fix for CHFts14267 and misc. itimer fixes.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.vtalarm.b1/1  1995/02/02  19:40 UTC  tatsu_s
 * 	Disabled itimer in atfork & atexit handlers.
 * 
 * 	HP revision /main/HPDCE02/6  1995/01/31  21:16 UTC  tatsu_s
 * 	Submitted the local rpc cleanup.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.vtalarm.b0/1  1995/01/26  20:36 UTC  tatsu_s
 * 	Added rpc__local_fork_handler() and rpc__exit_handler().
 * 
 * 	HP revision /main/HPDCE02/5  1995/01/16  19:14 UTC  markar
 * 	Merging Local RPC mods
 * 
 * 	HP revision /main/HPDCE02/markar_local_rpc/1  1995/01/05  18:31 UTC  markar
 * 	Implementing Local RPC bypass mechanism
 * 
 * 	HP revision /main/HPDCE02/4  1994/12/12  21:42 UTC  tatsu_s
 * 	Fixed RPC_PREFERRED_PROTSEQ=<invalid protseq>.
 * 
 * 	HP revision /main/HPDCE02/3  1994/12/09  19:17 UTC  tatsu_s
 * 	Submitted rfc31.0: Single-threaded DG client and RPC_PREFERRED_PROTSEQ.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.st_dg.b0/4  1994/12/07  20:59 UTC  tatsu_s
 * 	rfc31.0: Cleanup.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.st_dg.b0/3  1994/12/07  14:33 UTC  tatsu_s
 * 	Fixed the initialization of rpc_g_preferred_protseq_id.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.st_dg.b0/2  1994/12/05  18:53 UTC  tatsu_s
 * 	RPC_PREFERRED_PROTSEQ: Initial version.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.st_dg.b0/1  1994/11/30  22:25 UTC  tatsu_s
 * 	rfc31.0: Initial version.
 * 
 * 	HP revision /main/HPDCE02/2  1994/08/17  20:41 UTC  tatsu_s
 * 	Merged from Hpdce02_01
 * 	[1995/12/07  23:58:09  root]
 * 
 ...
 * 	Merged mothra up to dce 1.1.
 * 
 * Revision 1.1.10.2  1993/11/11  21:45:09  markar
 * 	         Fixed pointer bug in 1.1.9.2.
 * 	[1993/11/10  18:52:27  markar]
 * 
 * 	    Added support for RPC_SUPPORTED_NETADDRS environment variable
 * 	[1993/11/09  18:56:57  markar]
 * 
 * Revision 1.1.8.5  1993/10/05  17:59:13  tatsu_s
 * 	    Loading drop DCE1_0_3b931005
 * 
...
GLOBAL rpc_addr_vector_p_t rpc_supported_netaddrs = NULL;
#define MAX_SUPPORTED_NETADDRS 32
...
INTERNAL void init_getenv_netaddrs _DCE_PROTOTYPE_ ((void));
...
# endif
    ...
    /*
     * See if this server should limit itself to using a subset of its 
     * available network addresses.
     */
 
    init_getenv_netaddrs();
#endif                                  /* ! NO_GETENV */
...
**++
**
**  ROUTINE NAME:       init_getenv_netaddrs
**
**  SCOPE:              INTERNAL
**
**  DESCRIPTION:
**
**  Read the RPC_SUPPORTED_NETADDRS environment variable to determine whether
**  we're restricting the server's use of the available networks.
**
**  INPUTS:             none
**
**  INPUTS/OUTPUTS:     none
...
INTERNAL void init_getenv_netaddrs (void)

{
    unsigned32          i, count, status;
    unsigned_char_p_t   s, sp, env_name;
    unsigned_char_t     buff[32]; 
    rpc_protseq_id_t    pseq_id;
    rpc_addr_p_t        netaddr_list[MAX_SUPPORTED_NETADDRS];

    /*
     * See if environment variable exists.  If it doesn't, we're done now.
     */

    env_name = (unsigned_char_p_t) getenv ("RPC_SUPPORTED_NETADDRS");
    if (env_name == NULL)
    {
        return;
    }
    ...
    /*
     * Parse the value of the environment variable into a list of network
     * addresses.
     */
    ...
    {
        /*
         * First look for the protocol sequence portion of the network
         * address...
         */
        pseq_id = RPC_C_INVALID_PROTSEQ_ID;
        sp = buff;
        while (*s != '\0' && *s != ':')
            *sp++ = *s++;
        ...
        if (*s == ':')
        {
            *sp = '\0';
            s++;
            pseq_id = rpc__network_pseq_id_from_pseq (buff, &status);
        }   

        /*
         * We should have a valid protocol sequence now. If not, bail out.
         */
        if (status != rpc_s_ok || pseq_id == RPC_C_INVALID_PROTSEQ_ID)
        {
            RPC_DBG_GPRINTF((
                "(init_getenv_netaddrs) Invalid protocol sequence.\n"));
            return;
        }

        /*
         * Next, look for the network part of the address...
         */
        sp = buff;
        while (*s != '\0' && *s != ',')
            *sp++ = *s++;
        ...
        *sp = '\0';
        rpc__naf_addr_alloc (pseq_id, RPC_PROTSEQ_INQ_NAF_ID(pseq_id),
                             NULL, buff, NULL, &netaddr_list[count], 
                             &status);

        /*
         * We should have a network address now. If not, bail out.
         */
        if (status != rpc_s_ok || buff[0] == '\0')
        {
            RPC_DBG_GPRINTF((
                "(init_getenv_netaddrs) Invalid network address.\n"));
            return;
        }

        count++;

    } while (*s++ != '\0' && count < MAX_SUPPORTED_NETADDRS);

    /*
     * Alloc up and initialize the global vector of supported network
     * addresses.
     */

    RPC_MEM_ALLOC(rpc_supported_netaddrs, rpc_addr_vector_p_t,
        sizeof (*rpc_supported_netaddrs) + (count - 1) * sizeof (rpc_addr_p_t),
        RPC_C_MEM_RPC_ADDR_VEC, RPC_C_MEM_WAITOK);
#ifndef HPDCE_ALLOC_FAIL_DETECT
    if (rpc_supported_netaddrs == NULL){
	/* free naf addresses alloc'ed above */
	for (i = 0; i < count; i++)
	    rpc__naf_addr_free(&netaddr_list[i], &status);
	return;
    }
#endif /* HPDCE_ALLOC_FAIL_DETECT */

    rpc_supported_netaddrs->len = count;

    for (i = 0; i < count; i++)
    {
        if (RPC_DBG(rpc_e_dbg_general, 1))
        {
            unsigned_char_t *tbuff;

            rpc__naf_addr_inq_netaddr(netaddr_list[i], &tbuff, &status);

            if (status == rpc_s_ok)
            {
                RPC_DBG_GPRINTF(("(init_getenv_netaddr) supporting [%s]\n", tbuff));
                rpc_string_free (&tbuff, &status);
            }
        }

        rpc_supported_netaddrs->addrs[i] = netaddr_list[i];
    }
