 ...
 * HISTORY
 * $Log: astp_dmp.c,v $
 * Revision 1.1.11.2  1996/02/18  18:50:52  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  18:04:35  marty]
 *
 * Revision 1.1.11.1  1995/12/07  22:13:06  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/dat_xidl2/1  1995/11/17  17:01 UTC  dat
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/12/07  21:09:45  root]
 * 
 * Revision 1.1.2.1  1995/10/23  01:47:19  bfc
 * 	oct 95 idl drop
 * 	[1995/10/23  00:42:49  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/21  22:55:15  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE 1.1
 * 	[1995/10/21  17:23:27  bfc]
 * 
 * Revision 1.1.7.1  1994/05/02  22:43:27  tom
 * 	From DEC: Add binding callout function.
 * 	[1994/05/02  21:06:25  tom]
 * 
 * Revision 1.1.4.2  1993/07/07  19:55:10  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:29:28  ganni]
 * 
 * $EndLog$
 */
/*
...
#endif
{
    ...
    if (AST_LOCAL_SET(if_n_p) || (if_n_p->op_count == 0)) {
    ...
    if (AST_IN_LINE_SET(if_n_p)     ||
        ...
        if_n_p->lookup_callout_name != NAMETABLE_NIL_ID ||
        if_n_p->malloc_callout_name != NAMETABLE_NIL_ID ||
        if_n_p->free_callout_name != NAMETABLE_NIL_ID ||
        if_n_p->cxx_delegate_name != NAMETABLE_NIL_ID ||
        ...
        AST_EXPLICIT_HANDLE_SET(if_n_p)) {
            ...
            if (if_n_p->lookup_callout_name != NAMETABLE_NIL_ID) {
                indent(1);
                printf("[lookup_callout(");
                dump_nametable_id ("%s)] \n", if_n_p->lookup_callout_name);
            }
            if (if_n_p->malloc_callout_name != NAMETABLE_NIL_ID) {
                indent(1);
                printf("[client_memory(");
                dump_nametable_id ("%s, ", if_n_p->malloc_callout_name);
                dump_nametable_id ("%s)] \n", if_n_p->free_callout_name);
            }
            if (if_n_p->cxx_delegate_name != NAMETABLE_NIL_ID) {
                indent(1);
                printf("[delegate_name(");
                dump_nametable_id ("%s)] \n", if_n_p->cxx_delegate_name);
            }
...
#endif
{
    ...
    if (AST_STATIC_SET(operation_node_ptr)) {
        indent (indentation);
        printf ("[cxx_static");
        if (operation_node_ptr->cxx_static_name != NAMETABLE_NIL_ID)
            dump_nametable_id("(%s)", operation_node_ptr->cxx_static_name);
        printf("] \n");
    }
    ...
    if (operation_node_ptr->cxx_static_name != NAMETABLE_NIL_ID) {
        indent (indentation);
        printf ("[cxx_new");
            dump_nametable_id("(%s)", operation_node_ptr->cxx_new_name);
        printf("] \n");
    }
