 ...
 * HISTORY
 * $Log: utilichr.c,v $
 * Revision 1.1.4.2  1996/02/17  23:14:45  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:35:01  marty]
 *
 * Revision 1.1.4.1  1995/12/11  20:08:19  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  19:32:34  root]
 * 
 * Revision 1.1.2.2  1993/08/11  16:27:38  ganni
 * 	tests for new IDL functionality
 * 	[1993/08/11  16:15:28  ganni]
 * 
 * $EndLog$
 */
/*
        ...
        case RTN_STAG:
            *p_convert_type = idl_cs_new_buffer_convert;
            if (p_w_storage_len != NULL)
                *p_w_storage_len = l_storage_len / 2;
            return;
        default:
...
#endif
{
    ...
    *p_st = error_status_ok;
    ...
    {
        case RTN_STAG:
            for (i=0; i<l_data_len/2; i++)
            {
                wdata[i] = (ldata[2 * i] << 4) | ldata [2 * i + 1];
            }
            break;
        default:
            *p_st = rpc_s_ss_incompatible_codesets;
...
#endif
{
    ...
    switch (tag)
    {
        case RTN_STAG:
            ...
            {
                ldata[2 * i] = (wdata[i] & 0xF0) >> 4;
                ldata[2 * i + 1] = (wdata[i] & 0xF);
            }
            break;
        default:
            *p_st = rpc_s_ss_incompatible_codesets;
