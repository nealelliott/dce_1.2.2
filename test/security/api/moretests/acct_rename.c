 ...
 * HISTORY
 * $Log: acct_rename.c,v $
 * Revision 1.1.14.1  1996/06/04  22:12:43  arvind
 ...
 * 
 * Revision 1.1.11.2  1995/12/11  20:20:18  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  19:51:55  root]
 * 
 * Revision 1.1.7.1  1994/10/25  17:17:56  sekhar
 * 	[ OT 12398 ] call sec_rgy_cursor_reset() before account lookup.
 * 	             and other fixes.
 * 	[1994/10/24  18:13:28  sekhar]
 * 
 * 	[ OT 12398 ]  Checking in Sean Mullan's changes for testing
 * 		      security replication.
 * 	[1994/10/24  16:59:54  sekhar]
 * 
 * Revision 1.1.4.1  1993/10/07  18:58:57  mccann
 * 	CR9057 remove unnecessary verison requirement
 * 	[1993/10/07  18:57:15  mccann]
 * 
 * Revision 1.1.2.3  1993/02/25  22:04:19  cuti
 * 	Change sec_rgy_site_open() to sec_rgy_site_open_update() for using site update operation.
 * 	[1993/02/25  16:29:00  cuti]
 * 
 * Revision 1.1.2.2  1992/12/31  17:27:44  htf
 * 	Embedding copyright notice
 * 	[1992/12/31  16:53:08  htf]
 * 
 * Revision 1.1  1992/01/19  04:07:31  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
 ...
 *	from delta on	11/13/91 14:17:32
 *
 *  $LOG$
 *
 *
...
#ifndef LINT
static char sccs_id[] = "src/test/security/api/moretests/acct_rename.c, , dce_osf, dce.75d";
#endif
...
/* Local variables */
   ...
   sec_rgy_login_name_t *old_login_name;/* sec_rgy_acct_rename */
   sec_rgy_login_name_t *new_login_name;/* sec_rgy_acct_rename */
...
/* Add an additional test org item to the registry for the test of
   ...
   if (subcase == ACCT_RENAME_UNIQUE_KEY_PARTS || subcase == ACCT_RENAME) 
     {
       bzero ((char *)&pgo_item, sizeof (pgo_item));

       pgo_item.unix_num = 5006;

       memcpy ((char *)pgo_item.fullname,
	       (char *)"newkey:  marco's new test org item\0",
	       sizeof (pgo_item.fullname));
       ...
       TSHMessage (ctx, MOTRACE,
                   "sec_rgy_pgo_add (key org) status: %d\n", tstatus);

       if (TSHStatus (ctx, SETUP,
                      "sec_rgy_pgo_add (key org) failed\n", rpc_s_ok, tstatus))
       ...
       TSHMessage (ctx, MOTRACE,
                   "sec_rgy_pgo_add_member (key org) status: %d\n", tstatus);

       if (TSHStatus (ctx, SETUP,
             "sec_rgy_pgo_add_member (key org) failed\n", rpc_s_ok, tstatus))
         goto EXIT_DEL_KEY_ORG_ITEM;
...
/* Setup the parameters to the sec_rgy_acct_add function. */
   ...
   if (subcase == ACCT_RENAME_UNIQUE_KEY_PARTS)
     key_parts = (sec_rgy_acct_key_t)sec_rgy_acct_key_org;
   else
...
/* Add a second account if this is the test of non-unique key_parts. */
...
/* Setup the old and new login name for the rename test. */

   switch (subcase)
     {
       case ACCT_RENAME_NOT_FOUND:

        /* On the object_not_found case, set the old login name to a name not
	   in the registry.  The new login name is irrelavent, but it has to
	   be set to something. */

           old_login_name = &ACCT_NON_LOGIN_NAME;
	   new_login_name = &ACCT_ALT_LOGIN_NAME;
	   break;

       case ACCT_RENAME_ALREADY_EXISTS:

        /* On the name_exists case, use the same name for the old and new
	   login names. */

           old_login_name = &ACCT_LOGIN_NAME;
	   new_login_name = &ACCT_LOGIN_NAME;
	   break;

       case ACCT_RENAME_UNIQUE_KEY_PARTS:

        /* On the test of non-unique key parts, the account with 
	   acct_login_name has key parts of org.  A second account was added
	   with key parts of group having the same person and group data as
	   the first account.  An attempt will be made to rename the first
	   account using a key parts argument of group.  This should cause
	   a conflict with the second account, and the expected key parts
	   returned from the rename is org. */

           old_login_name = &ACCT_LOGIN_NAME;
	   new_login_name = &ACCT_KEY_LOGIN_NAME;
	   break;

       default:

        /* This is the standard rename configuration.  Note that the alt
	   login name was only added as an account on the unique key parts
	   test. */
...
/* Call the rpc under test. */
   ...
   sec_rgy_acct_rename (write_context,
                        old_login_name,
                        new_login_name,
                        &key_parts,
                        &tstatus);
   ...
   TSHMessage (ctx, MOTRACE,
               "sec_rgy_acct_rename (test) status: %d\n", tstatus);

   if (estatus == tstatus)
     {
       tstatus = sec_rgy_status_ok;

       if ((subcase == ACCT_RENAME) ||
	   (subcase == ACCT_RENAME_UNIQUE_KEY_PARTS))
         {
        ...
        /* Even if tstatus came back with the expected return on the 
           specified rename above, check to see if the account was really
           changed by attempting to look it up in the registry. */
           ...
           TSHMessage (ctx, MOTRACE,
                      "sec_rgy_acct_rename (after rename) status: %d\n", tstatus);

        /* If the lookup failed or if the login name returned from the lookup
           did not match the login name supplied as input to acct_rename,
    ...
    /* If this was the test of the non-unique key_parts replacement and
       the key_parts were not reset to acct_key_org, then signal a
       test failure. */

       if ((subcase == ACCT_RENAME_UNIQUE_KEY_PARTS) &&
           (lkp_key_parts != sec_rgy_acct_key_org))
         {
           TSHMessage (ctx, MOFAILURE, "sec_rgy_acct_rename failed\n");
	   tstatus = TSH_TEST_FAILED;
         }
   ...
   if (subcase == ACCT_RENAME_UNIQUE_KEY_PARTS)
     {
       ...
       TSHMessage (ctx, MOTRACE,
                  "sec_rgy_acct_delete (cleanup key parts) status: %d\n", cstatus);
...
EXIT_DEL_ACCT:
   ...
   if (subcase == ACCT_RENAME || subcase == ACCT_RENAME_UNIQUE_KEY_PARTS)
   {
       sec_rgy_acct_delete (write_context,
                           &ACCT_KEY_LOGIN_NAME,
                           &cstatus);
...
EXIT_DEL_KEY_ORG_MEMBER:

   if (subcase == ACCT_RENAME || subcase == ACCT_RENAME_UNIQUE_KEY_PARTS)
     {
       sec_rgy_pgo_delete_member (write_context, (sec_rgy_domain_t)sec_rgy_domain_org,
                                  ACCT_KEY_ORG_NAME, ACCT_PERSON_NAME, &cstatus);

       TSHMessage (ctx, MOTRACE,
                  "sec_rgy_pgo_delete_member (cleanup key org) status: %d\n", cstatus);

       if (TSHStatus (ctx, CLNUP,
             "sec_rgy_pgo_delete_member (key org) failed\n", rpc_s_ok, cstatus))
         tstatus = cstatus;
     }

EXIT_DEL_KEY_ORG_ITEM:

   if (subcase == ACCT_RENAME || subcase == ACCT_RENAME_UNIQUE_KEY_PARTS)
     {
       sec_rgy_pgo_delete (write_context, (sec_rgy_domain_t)sec_rgy_domain_org,
                           ACCT_KEY_ORG_NAME, &cstatus);

       TSHMessage (ctx, MOTRACE,
                  "sec_rgy_pgo_delete (cleanup key org) status: %d\n", cstatus);

       if (TSHStatus (ctx, CLNUP,
                      "sec_rgy_pgo_delete (key org) failed\n", rpc_s_ok, cstatus))
         tstatus = cstatus;
     }
