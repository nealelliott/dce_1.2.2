 ...
 * HISTORY
 * $Log: spppmzz.c,v $
 * Revision 1.1.4.2  1996/03/11  13:19:40  marty
 * 	Update OSF copyright years
 * 	[1996/03/11  13:08:51  marty]
 *
 * Revision 1.1.4.1  1995/12/08  17:31:06  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/08  16:17:42  root]
 * 
 * Revision 1.1.2.3  1994/06/10  21:06:45  devsrc
 * 	cr10871 - fix copyright
 * 	[1994/06/10  14:49:28  devsrc]
 * 
 * Revision 1.1.2.2  1994/05/10  15:59:23  marrek
 * 	April 1994 code submission.
 * 	[1994/04/27  12:52:59  marrek]
 * 
 * Revision 1.1.2.1  1994/03/23  15:33:46  keutel
 * 	creation
 * 	[1994/03/21  13:22:08  keutel]
 * 
 * $EndLog$
 */
#if !defined(lint) && defined(GDS_RCS_ID)
static char rcsid[] = { "@(#) $RCSfile: spppmzz.c,v $ $Revision: 1.1.4.2 $ $Date: 1996/03/11 13:19:40 $" } ;
#endif
...
static char *sccsid =    "@(#) spppmzz.c 1.1 93/12/20   RTS-V2.00"; 
	...
	if(((*matchz)|32)==160 )
	  ...
	  if(((*matchz)|32)==161 )
	    ...
	    sterr=Components_of_set_dec(hdl,poszm,
                  (unsigned long int FAR *)&(*x).hX410RtpmNode,
	    RC104v_RTOACapdu,(void NEAR *)pContext);
	    }
	  else
	    if(((*matchz)|32)==162 )
	      {
	...
	asn1_pvoid pContext)
{
  ...
  if ((*x).hX410RtpmNode != 0)
    ...
    Components_of_set_cod(hdl,2,1,
         (unsigned long int FAR *)&(*x).hX410RtpmNode,RC104v_RTOACapdu,
         (void NEAR *)pContext)
    ;
    }
	...
	asn1_pvoid pContext)
{
  ...
  unsigned char  matchz[6]; /* 21 */
	...
	sterr=Components_of_set_dec(hdl,poszm,
            (unsigned long int FAR *)&(*x).hX410RtpmNode,
	RC104w_RTORJapdu,(void NEAR *)pContext);
	...
	asn1_pvoid pContext)
{
  switch (CAST_FOR_SWITCH((*x).lX410orNormalChoice))
    ...
    case RC041a_X410Mode:
      Components_of_set_cod(hdl,-1,0,
         (unsigned long int FAR *)&(*x).hX410RtpmNode,RC104w_RTORJapdu,
           (void NEAR *)pContext);
      break;
	...
	asn1_pvoid pContext)
{
  ...
  unsigned char  matchz[6]; /* 21 */
	...
	sterr=Components_of_set_dec(hdl,poszm,
               (unsigned long int FAR *)&(*x).hX410RtpmNode,
	RC104z_RTABapdu,(void NEAR *)pContext);
	(*x).lX410orNormalChoice = RC041a_X410Mode;
	}
      else
	if(((*matchz)|32)==160 )
	...
	asn1_pvoid pContext)
{
  switch (CAST_FOR_SWITCH((*x).lX410orNormalChoice))
    ...
    case RC041a_X410Mode:
      Components_of_set_cod(hdl,-1,0,
        (unsigned long int FAR *)&(*x).hX410RtpmNode,RC104z_RTABapdu,
      (void NEAR *)pContext);
      break;
    case RC041b_NormalMode:
	...
	asn1_pvoid pContext)
{
  ...
  unsigned char  matchz[6]; /* 21 */
    ...
    if ( sterr != OK )
      ...
      if(((*matchz)|32)==96 )
	{
	sterr=Simply_encoded_data_dec(hdl,poszm,
               (unsigned long int FAR *)&(*UserData).unUserData.hSimply,
	       (void NEAR *)pContext);
	...
	asn1_pvoid pContext)
{
  switch (CAST_FOR_SWITCH((*UserData).lSimplyFullyChoice))
    ...
    case RC0417_SimplyEncoded:
      Simply_encoded_data_cod(hdl,1,0,
             (unsigned long int FAR *)&(*UserData).unUserData.hSimply,
              (void NEAR *)pContext);
      break;
