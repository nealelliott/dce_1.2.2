 ...
 * Revision 1.1.7.2  1996/02/17  23:32:41  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:49:02  marty]
 *
 * Revision 1.1.7.1  1995/12/07  21:44:28  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/07  20:58:43  root]
 * 
 * Revision 1.1.2.2  1994/06/09  13:33:47  devsrc
 * 	 CR10892 - fix copyright in file
 * 	[1994/06/09  13:25:00  devsrc]
 * 
 * Revision 1.1.2.1  1994/04/01  20:13:20  jd
 * 	Moved from src/threads to here.
 * 	[1994/04/01  16:24:03  jd]
 * 
 ...
 * 
 * Revision 1.1.5.7  1993/03/29  20:21:47  jd
 ...
 * 
 * Revision 1.1.5.6  1993/02/01  22:31:43  hinman
 ...
 * 
 * Revision 1.1.5.5  1992/12/15  22:31:58  alan
 * 	Insert copyright notices
 * 	[1992/12/07  19:43:05  alan]
 * 
 * Revision 1.1.5.4  1992/10/23  21:04:18  bolinger
 ...
 * 
 * Revision 1.1.5.3  1992/09/29  20:16:02  devsrc
 * 	[OT 5373]    SNI/SVR4 merge.
 * 	[1992/09/17  20:55:20  sekhar]
 * 
 * Revision 1.1.5.2  1992/09/03  14:41:33  bolinger
 * 	Replace current source file with equivalent from DEC BL10+
 * 	source base, as originally integrated in the
 * 	nosupport/threads_bl10 tree.  (See OT defect 5300.)
 * 	[1992/09/02  14:07:14  bolinger]
 * 
 * Revision 1.1.5.2  1992/08/28  20:46:15  bolinger
 * 	Move latest 1.0.1-based revision into 1.0.2.
 * 	[1992/08/28  20:42:57  bolinger]
 * 
 * Revision 1.1.3.3  1992/08/28  14:14:54  bolinger
 * 	Bring forward IBM integration changes to BL10 sources.
 * 
 * Revision 1.1.3.2  1992/08/10  16:35:18  bolinger
...
cma__open_general(int	fd)
	...
	*/
	...
	switch (errno)	{
		...
		/*
		 * We need to set FIFO's (pipes) to be non-blocking.
		 * In OSF/1 1.1.1 pipes (that is, what an application
		 * gets if it calls the pipe() entry point in libc)
		 * are implemented as STREAMS devices.  A STREAM is a
		 * character special device.  So we need to try to set
		 * character special devices to be non-blocking.  However,
		 * since devices other than STREAMS may reject the
		 * fcntl() call used to do so, we have to allow this
		 * failure without complaint, by remembering the fact
		 * that this fd corresponds to a char special device.
		 *
		 * This code (and the addition of is_char_dev in
		 * cma_thdio_defs.h) should probably be used on any
		 * platform on which STREAMS appear as character
		 * devices.
		 */
		cma__g_file_obj[fo].is_char_dev =
			(format == S_IFCHR) ? cma_c_true : cma_c_false;
	switch (format) {
	    /*
		...
		/*
		 * Avoid select()'ing on a block-special file, which never,
		 * ever succeeds on OSF/1 1.1.  There's no need to try
		 * setting the file descriptor to non-blocking (with fcntl()),
		 * though, so pretend that we already have.
		 *
		 * Pretending that this fd is non-blocking will cause CMA
		 * always to do I/O with the fd before calling select().
		 * Since the I/O will always succeed (or fail with a "non-
		 * blocking" error), we avoid select() on this fd.
		 */
	    case S_IFBLK: {
		cma__g_file_obj[fo].non_blocking = cma_c_true;
...
cma__set_nonblocking(
    ...
    if (fcntl (fd, F_SETFL, 
		...
		cma__g_file[fd]->is_char_dev == cma_c_false)
