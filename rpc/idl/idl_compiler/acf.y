 ...
 * HISTORY
 * $Log: acf.y,v $
 * Revision 1.1.16.2  1996/03/09  23:25:46  marty
 * 	Update OSF copyright year
 * 	[1996/03/09  22:41:27  marty]
 *
 * Revision 1.1.16.1  1995/12/07  22:12:18  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/dat_xidl2/1  1995/11/17  17:01 UTC  dat
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/12/07  21:09:29  root]
 * 
 * Revision 1.1.2.1  1995/10/23  01:47:08  bfc
 * 	oct 95 idl drop
 * 	[1995/10/23  01:46:22  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/21  21:21:59  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE 1.1
 * 	[1995/10/21  17:24:18  bfc]
 * 
 * Revision 1.1.11.4  1994/05/27  18:06:30  tom
 * 	Bug 10794 - extern_exceptions attribute can omit the list.
 * 	[1994/05/27  18:06:11  tom]
 * 
 * Revision 1.1.11.3  1994/05/04  20:29:34  tom
 * 	Bug 10057 - Fix non PROTO declaration.
 * 	[1994/05/04  20:26:22  tom]
 * 
 * Revision 1.1.11.2  1994/05/03  14:44:51  tom
 * 	Fix typo in declaration of binding_callout_name (OT 10508).
 * 	[1994/05/03  14:44:39  tom]
 * 
 * Revision 1.1.11.1  1994/05/02  22:43:21  tom
 * 	From DEC: Add binding callout function.
 * 	[1994/05/02  21:06:21  tom]
 * 
 * Revision 1.1.7.2  1993/10/14  12:27:36  hinxman
 * 	CR 8899 Improve error reporting of missing required type
 * 	[1993/10/14  12:27:16  hinxman]
 * 
 * Revision 1.1.7.1  1993/10/01  13:26:02  hinxman
 * 	OT 8874 - change design for spelling of #include generated
 * 	from ACF include statement
 * 	[1993/10/01  13:25:37  hinxman]
 * 
 * Revision 1.1.5.3  1993/07/19  16:48:56  hinxman
 * 	OT8272 Don't stop compilation if the file name in an ACF
 * 	include statement has an extension
 * 	[1993/07/15  17:57:36  hinxman]
 * 
 * Revision 1.1.5.2  1993/07/07  19:53:59  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:28:58  ganni]
 * 
...
%token TYPEDEF_KW
%token CXX_LOOKUP_KW
%token CLIENT_MEMORY_KW
%token CXX_DELEGATE_KW
%token CXX_NEW_KW
...
acf_include:
    ...
    |   LBRACKET SSTUB_KW RBRACKET INCLUDE_KW acf_include_list
        {
        if (ASTP_parsing_main_idl)
    ...
    |   LBRACKET CSTUB_KW RBRACKET INCLUDE_KW acf_include_list
        {
        if (ASTP_parsing_main_idl)
    ...
    |   LBRACKET CSTUB_KW COMMA SSTUB_KW RBRACKET INCLUDE_KW acf_include_list
        {
        if (ASTP_parsing_main_idl) {
            the_interface->cincludes = (AST_include_n_t *)
                AST_concat_element((ASTP_node_t *)the_interface->cincludes,
    ...
    |   LBRACKET SSTUB_KW COMMA CSTUB_KW RBRACKET INCLUDE_KW acf_include_list
        {
        if (ASTP_parsing_main_idl) {
            the_interface->sincludes = (AST_include_n_t *)
                AST_concat_element((ASTP_node_t *)the_interface->sincludes,
...
acf_cxx_new_attr:
        CXX_NEW_KW LPAREN acf_cxx_new_name RPAREN
    ;

acf_cxx_new_name:
        IDENTIFIER
    {
        NAMETABLE_id_to_string($<y_id>1, &cxx_new_name);
    }
    ;
...
acf_lookup_callout_attr:
	CXX_LOOKUP_KW LPAREN acf_lookup_callout_name RPAREN
    ;

acf_lookup_callout_name:
        IDENTIFIER
    {
        NAMETABLE_id_to_string($<y_id>1, &lookup_callout_name);
    }
    ;

acf_client_memory_callout_attr:
	CLIENT_MEMORY_KW LPAREN acf_client_memory_malloc_callout_name COMMA acf_client_memory_free_callout_name RPAREN
    ;

acf_client_memory_malloc_callout_name:
        IDENTIFIER
    {
        NAMETABLE_id_to_string($<y_id>1, &malloc_callout_name);
    }
    ;

acf_client_memory_free_callout_name:
        IDENTIFIER
    {
        NAMETABLE_id_to_string($<y_id>1, &free_callout_name);
    }
    ;


acf_delegate_attr:
	CXX_DELEGATE_KW LPAREN acf_delegate_name RPAREN
    ;

acf_delegate_name:
        IDENTIFIER
    {
        NAMETABLE_id_to_string($<y_id>1, &cxx_delegate_name);
    }
    ;
...
acf_cxx_static_attr:
    CXX_STATIC_KW LPAREN IDENTIFIER RPAREN
    {
        NAMETABLE_id_to_string($<y_id>3, &cxx_static_name);
    }
    |   CXX_STATIC_KW
    {
        cxx_static_name = NULL;
