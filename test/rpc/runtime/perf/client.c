 ...
 * HISTORY
 * $Log: client.c,v $
 * Revision 1.1.16.1  1996/06/04  22:12:17  arvind
 * 	Enable single_threaded test
 * 	[1996/04/29  14:47 UTC  bissen  /main/DCE_1.2/2]
 *
 * Revision 1.1.14.2  1996/02/17  23:23:46  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:42:13  marty]
 * 
 * Revision 1.1.14.1  1995/12/11  20:14:55  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/HPDCE02/jrr_1.2_mothra/1  1995/11/30  17:45 UTC  psn
 * 	Put ifdef to comment out single threaded stuff out of AIX port.
 * 
 * 	HP revision /main/HPDCE02/3  1995/08/29  19:19 UTC  tatsu_s
 * 	Submitted the fix for CHFts14943.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.pipe_timeout.b0/1  1995/08/07  19:41 UTC  tatsu_s
 * 	Increased pipe_client_sleep.
 * 
 * 	HP revision /main/HPDCE02/2  1995/05/25  21:38 UTC  lmm
 * 	Submit max thread mod for testing alloc failures
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/11  1995/04/06  15:19 UTC  tatsu_s
 * 	Increased the max number of threads.
 * 
 * 	HP revision /main/HPDCE02/1  1995/03/06  15:47 UTC  tatsu_s
 * 	Submitted the pipe test.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/10  1995/03/01  14:17 UTC  tatsu_s
 * 	Fixed the static cancel test.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/9  1995/02/16  18:26 UTC  tatsu_s
 * 	Added the client sleep in the pipe test.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/8  1995/02/16  17:47 UTC  tatsu_s
 * 	Added exec() test.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/7  1994/12/05  18:55 UTC  tatsu_s
 * 	Added more messages.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/6  1994/11/30  22:27 UTC  tatsu_s
 * 	rfc31.0: Added the exit status.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/5  1994/10/24  17:19 UTC  tatsu_s
 * 	Added more pipe tests.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/4  1994/10/19  18:15 UTC  tatsu_s
 * 	Removed the erroneous message.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/3  1994/10/14  21:00 UTC  tatsu_s
 * 	Added #include <limits.h> for MIN().
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/2  1994/10/11  13:59 UTC  tatsu_s
 * 	Fixed the verbose level.
 * 
 * 	HP revision /main/tatsu_s_func_test_b1/1  1994/10/10  13:49 UTC  tatsu_s
 * 	Added the pipe test.
 * 	[1995/12/11  19:47:45  root]
 * 
 * Revision 1.1.9.5  1994/08/25  22:05:38  bmw
 * 	OT 10091 - AIX compiler problem; moved code to correct problem
 * 	[1994/08/25  21:32:48  bmw]
 * 
 * Revision 1.1.9.4  1994/08/12  16:22:31  tom
 * 	Add -B option to test setting TCP buffer sizes (OT 11440).
 * 	[1994/08/11  20:20:31  tom]
 * 
 * Revision 1.1.9.3  1994/07/15  16:56:24  tatsu_s
 * 	Added more fork() tests.
 * 	[1994/07/09  02:45:46  tatsu_s]
 * 
 * Revision 1.1.9.2  1994/05/27  15:36:56  tatsu_s
 * 	Prints ms/pass if ms/call == 0.
 * 	[1994/05/20  21:04:41  tatsu_s]
 * 
 * 	Added the back-to-back maybe call test.
 * 	Print exception info in CATCH_ALL.
 * 	Measure user/system time if NO_TIMES not defined.
 * 	[1994/04/22  19:23:06  tatsu_s]
 * 
 * Revision 1.1.9.1  1994/04/04  20:50:44  tom
 * 	Bug 9819 - Never let rand return zero.
 * 	[1994/04/04  20:50:19  tom]
 * 
 * Revision 1.1.7.1  1993/09/28  13:12:33  tatsu_s
 * 	Bug 7489 - Use the original server binding for
 * 	perf_unregister_b() in forwarding_test().
 * 	[1993/09/27  16:05:46  tatsu_s]
 * 
 * Revision 1.1.5.3  1993/01/31  17:31:14  weisman
 * 	Sanity-check calls/pass input argument.  It now must be > 0.  (OT
 * 	6948)
 * 	[1993/01/31  17:28:34  weisman]
 * 
 * Revision 1.1.5.2  1993/01/13  19:56:15  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/06  18:23:59  bbelch]
 * 
 * Revision 1.1.2.4  1992/06/11  12:28:42  mishkin
 * 	Enhance generic test to test for unsupported typed object.
 * 	[1992/06/10  19:16:26  mishkin]
 * 
 * Revision 1.1.2.3  1992/05/12  17:48:54  mishkin
 * 	Guard new use of "multithread" with _POSIX_THREAD #ifdef.
 * 	[1992/05/12  15:34:12  mishkin]
 * 
 * 	Add 'r' and 'R' switches to reset/recreate bindings agressively in looping test.
 * 	[1992/05/08  21:22:56  mishkin]
 * 
 * Revision 1.1.2.2  1992/05/01  15:51:57  rsalz
 * 	 28-apr-92 mishkin   print exc info in multi-thread test
 * 	 07-feb-92 mishkin   fix NO_GETTIMEOFDAY path in previous change
 * 	 31-jan-92 mishkin   - improve timing granularity
 * 	                     - add one shot test
 * 	 03-dec-91 sommerfeld - flush rpc_x_call_failed (it's obsolete).
 * 	 17-oct-91 labossiere - generic_test: add server runtime bug test
 * 	                     - exception_test: fix printfs
 * 	 15-oct-91 mishkin   - remove a couple of hard tabs
 * 	                     - add compat mode to avoid doing things that an NCS 1.5
 * 	                       server is unlikely to be able to do.
 * 	                     - make exception test print an error if no exception occurs.
 * 	 08-oct-91 labossiere fix pthread_cancel_e and cancel timeout detection
 * 	 01-oct-91 hu        fix prototype warning.
 * 	 13-sep-91 hu        add a call to ep_resolve_binding
 * 	 04-sep-91 mishkin   use vfork instead of fork on VMS
 * 	 19-jul-91 mishkin   - fix printf in float test
 * 	                     - Use rpc_mgmt_inq_server_princ_name, as needed
 * 	                     - change inq_if_ids test to use binding_from_string_binding
 * 	                     - add -o switch to make bindings contain an object
 * 	                       (make fwd'ing for mgmt i/f work w/o having to give
 * 	                       endpoint on cmd line).
 * 	[1992/05/01  00:58:25  rsalz]
 * 
 * Revision 1.1  1992/01/19  03:41:52  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
...
struct tinfo_t
{
...
} tinfo[] =
    {
        ...
        /* 19 */     {pipe_test, "Variable length input pipe",
                "host passes calls/pass verify? nbytes bsize ssleep csleep"},
        /* 20 */     {pipe_test, "Variable length output pipe",
                "host passes calls/pass verify? nbytes bsize ssleep csleep"},
        /* 21 */     {pipe_test, "Variable length input/output pipe",
                "host passes calls/pass verify? nbytes bsize ssleep csleep"},
    };
...
idl_boolean single_threaded = true;
...
unsigned32 pipe_client_sleep = 0;
unsigned32 pipe_num_push = 0;
unsigned32 pipe_num_pull = 0;
...

{
    ...
    else
    {
        ...
        fprintf (stderr, "              [-f <opt>] [-B <bufsize>] [-E <secs>]\n");
        fprintf (stderr, "              test# ...\n");
        ...
        fprintf (stderr, "  -E: Execute \"sleep secs\" at the end\n");
#ifndef NO_TIMES
...

{
    ...
    rpc_binding_to_string_binding (h, &name2, &st);
    if (st != rpc_s_ok)
    {
    ...
    rpc_string_binding_parse(name2, NULL, &protseq, NULL, NULL, NULL, &st);
    if (st != rpc_s_ok)
    {
        fprintf (stderr, "*** Can't parse name \"%s\" - %s\n",
            name, error_text (st));
        exit(1);
    }
    if (strcmp(protseq, "ncacn_ip_tcp") == 0)
...

{
    ...
    idl_boolean         global;
#define FSIZE 4000
    unsigned long       d[FSIZE];
    unsigned32          fst;
    int                 i;
    unsigned long       sum, rsum;
    idl_char            result[256];
    ...
    TRY
    {
        ...
        TRY
        {
            perfb_in(rh, d, (unsigned long) FSIZE, true, &sum);
            if (sum != rsum)
            {
...
#define MSIZE (1024 * 1024)
    ...
    struct msec_time    avg_time;
    unsigned short      cpp;
    unsigned long       *d = (unsigned long *) malloc(MSIZE);
    unsigned long       len;
    unsigned long       sum, rsum;
    unsigned long       n_calls, n_brd, n_maybe, n_brd_maybe;
    idl_boolean         idem;
    idl_boolean         verify;
                ...
                case 2:
                {
                    ...
                    if (idem)
                    {
                        perf_out_idem (rh, d, &tlen, len,
                            (unsigned long) pass * calln, verify);
                    }
                    else
                    {
...
void pipe_client_pull(state, buf, esize, ecount)
rpc_ss_pipe_state_t state;
idl_char *buf;
idl_ulong_int esize;
idl_ulong_int *ecount;
{
    client_pipe_state_t *cl_state = state;
    unsigned char *d = state;

    pipe_num_pull++;

    VRprintf(3, ("(pipe_client_pull) esize = %lu\n", esize));
    esize = MIN(esize, MSIZE);
    if (cl_state->forever != true)
	esize = MIN(esize, cl_state->count);
    ...
    if (pipe_client_sleep > 0)
    {
	SLEEP((pipe_client_sleep*pipe_num_pull));
    }

    memmove(buf, cl_state->d, esize);
    *ecount = esize;
    if (cl_state->forever != true)
	cl_state->count -= esize;

    return;
}

void pipe_client_push(state, buf, ecount)
rpc_ss_pipe_state_t state;
idl_char *buf;
idl_ulong_int  ecount;
{
    ...
    VRprintf(3, ("(pipe_client_push) ecount = %lu\n", ecount));
    ...
    if (pipe_client_sleep > 0)
    {
	SLEEP((pipe_client_sleep*pipe_num_push));
    }

    if (cl_state->forever != true)
	cl_state->count += ecount;
    memmove(cl_state->d, buf, MIN(ecount,MSIZE));

    return;
}
                      
void pipe_client_alloc(state, bsize, buf, bcount)
rpc_ss_pipe_state_t state;
idl_ulong_int bsize;
idl_char **buf;
idl_ulong_int *bcount;
{
    ...
    VRprintf(3, ("(pipe_client_alloc) bsize = %lu\n", bsize));

    bsize = MIN(bsize, cl_state->bsize);

    *buf = cl_state->b;
    *bcount = bsize;
}

static unsigned long pipe_test1_client(rh, len, bsize, secs)
handle_t rh;
unsigned long len;
unsigned long bsize;
unsigned long secs;
{
    unsigned char *d = malloc(MSIZE);
    pipe_of_chars client_in_pipe;
    unsigned long outval = 0;
    client_pipe_state_t client_in_pipe_state;

    client_in_pipe_state.bsize = bsize;
    client_in_pipe_state.b = malloc(bsize);
    client_in_pipe_state.count = len;
    client_in_pipe_state.d = d;
    if (len == 0)
	client_in_pipe_state.forever = true;
    else
	client_in_pipe_state.forever = false;

    client_in_pipe.pull = pipe_client_pull;
    client_in_pipe.alloc = pipe_client_alloc;
    client_in_pipe.state = &client_in_pipe_state;

    TRY
    {
        perfp_op1(rh, secs, client_in_pipe, &outval);
    }
    CATCH_ALL
    {
        exc_report(THIS_CATCH);
    ...
    if (d != NULL)
        free(d);
    if (client_in_pipe_state.b != NULL)
        free(client_in_pipe_state.b);

    return outval;
}

static unsigned long pipe_test2_client(rh, len, bsize, secs)
handle_t rh;
unsigned long len;
unsigned long bsize;
unsigned long secs;
{
    ...
    unsigned long outval = 0;
    client_pipe_state_t client_out_pipe_state;

    client_out_pipe_state.bsize = bsize;
    client_out_pipe_state.b = malloc(bsize);
    client_out_pipe_state.count = 0;
    client_out_pipe_state.d = d;
    if (len == 0)
	client_out_pipe_state.forever = true;
    else
	client_out_pipe_state.forever = false;

    client_out_pipe.push = pipe_client_push;
    client_out_pipe.alloc = pipe_client_alloc;
    client_out_pipe.state = &client_out_pipe_state;

    TRY
    {
        perfp_op2(rh, secs, len, client_out_pipe);
    }
    CATCH_ALL
    {
        exc_report(THIS_CATCH);
    ...
    if (d != NULL)
        free(d);
    if (client_out_pipe_state.b != NULL)
        free(client_out_pipe_state.b);

    return client_out_pipe_state.count;
}

static unsigned long pipe_test3_client(rh, len, bsize, secs)
handle_t rh;
unsigned long len;
unsigned long bsize;
unsigned long secs;
{
    ...
    client_out_pipe_state.bsize = bsize;
    client_out_pipe_state.b = client_in_pipe_state.b;
    client_out_pipe_state.count = 0;
    client_out_pipe_state.d = d;
    client_out_pipe_state.forever = false;
    ...
    TRY
    {
        perfp_op3(rh, secs, client_in_pipe, client_out_pipe, &outval);
    }
    CATCH_ALL
    {
        exc_report(THIS_CATCH);
...
static void pipe_test (test, argc, argv)
...

{
    ...
    bsize = atoi(argv[7]);
    secs = atoi(argv[8]);
    pipe_client_sleep = atoi(argv[9]);

    printf("    passes: %d; calls/pass: %d; verification: %s;"
	   " bytes/call: %ld; bsize: %ld; ssleep: %d; csleep %d\n",
           passes, cpp, verify ? "on" : "off",
	   len, bsize, secs, pipe_client_sleep);
    ...
    for (pass = 1; pass <= passes; pass++)
    {
	    ...
	    {
	    case 19:
		retlen = pipe_test1_client(rh, len, bsize, secs);
		if (len != retlen)
		    {
			fprintf(stderr, "*** Length mismatch (%lu, %lu)\n",
				len, retlen);
			failures++;
		    }
		break;
	    case 20:
		retlen = pipe_test2_client(rh, len, bsize, secs);
		if (len != retlen)
		    {
			fprintf(stderr, "*** Length mismatch (%lu, %lu)\n",
				len, retlen);
		...
		break;
	    case 21:
		retlen = pipe_test3_client(rh, len, bsize, secs);
		if (len != retlen)
		    {
			fprintf(stderr, "*** Length mismatch (%lu, %lu)\n",
				len, retlen);
...
static int start_test(test, argc, argv)
...
static int multi_task (info, len)
...

{
    ...
    TRY
    {
        rc = start_test(info->test, info->argc, info->argv);
    }
    CATCH_ALL
    {
        exc_report(THIS_CATCH);
...
static int multi_test (test, argc, argv)
...
char                *argv[];
{
    ...
    printf("  Multithreading%s: # tasks = %d\n",
	   (use_shared_handle?" (shared binding)":""), n_tasks);
    ...
    for (i = 0; i < n_tasks; i++)
    {
        ...
        if (pthread_create(&tasks[i], pthread_attr_default,
#ifndef OSF
                (pthread_startroutine_t) multi_task, (pthread_addr_t) info
#else
                (pthread_func_t) multi_task, (any_t) info
#endif
                ) == -1)
        {
    ...
    while (!done)
    {
        TRY {
            ...
            {
                int junk;
                if (pthread_join(tasks[i], &junk) == -1)
                {
                    perror("pthread_join");
                    exit(1);
                }
...
static void parse_auth_option()
{
    ...
    if ((tmp = malloc(strlen(optarg)+1)) == NULL)
    {
...
fork_test_replay:
    ...
    while ((c = getopt(argc, argv, "oslDi1E:B:d:p:m:M:t:c:f:w:v:r:R:")) != EOF)
    {
        switch (c)
        {
        case 'd':
    ...
    if (reset_binding_freq > 0)
    {
        printf(" (reset binding %d)", reset_binding_freq);
    }

    if (recreate_binding_freq > 0)
    {
        printf(" (recreate binding %d)", recreate_binding_freq);
    }
    ...
    if (fork_count != 0 || do_fork != 6)
    {
        ...
        if (multithread)
        {
            rc = multi_test(test, argc, argv);
            exit(rc);
        }
#endif

        rc = start_test(test, argc, argv);
    ...
    if (single_threaded && pthread_is_multithreaded_np())
    {
	fprintf(stderr, "*** Became multi-threaded\n");
	exit(2);
    }
	...
	execlp("sleep", "sleep", do_exec, NULL);
	fprintf(stderr, "*** sleep %d failed, errno = %d\n", do_exec, errno);
	exit(2);
    }
