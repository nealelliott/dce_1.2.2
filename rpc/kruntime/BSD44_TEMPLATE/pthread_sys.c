 ...
 * HISTORY
 * $Log: pthread_sys.c,v $
 * Revision 1.1.8.2  1996/02/17  23:59:51  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  22:53:21  marty]
 *
 * Revision 1.1.8.1  1995/12/08  00:13:48  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/07  23:55:34  root]
 * 
 * Revision 1.1.6.1  1994/01/21  22:31:10  cbrooks
 * 	RPC Code Cleanup
 * 	[1994/01/21  20:38:24  cbrooks]
 * 
 * Revision 1.1.4.3  1993/01/03  22:35:00  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/03  19:50:23  bbelch]
 * 
 * Revision 1.1.4.2  1992/12/23  19:36:33  zeliff
 * 	Embedding copyright notice
 * 	[1992/12/23  14:47:24  zeliff]
 * 
 * Revision 1.1.2.2  1992/05/01  17:54:43  rsalz
 * 	     Remove Mach stuff.
 * 	[1992/05/01  17:49:53  rsalz]
 * 
 * Revision 1.1  1992/01/19  03:15:42  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
...
pthread_addr_t              arg;
{
    ...
    RPC_MUTEX_LOCK(create_m); /* single thread (for new_thrd) */
    ...
    if (newproc(0)) {
        ...
        new_thrd = (pthread_t) u.u_procp->p_cptr;
        exit(u.u_procp, 0);
    }
    ...
    *new_thread = new_thrd;
    ...
    RPC_MUTEX_UNLOCK(create_m);

    /*
     * Wait for our intermediate (child) thread (it's now a zombie).
     * Hack upon hack...
     * This is all probably quite (wait4) system specific...
     */
    {
        int retval[2];
        struct args {
        ...
        args.pid = u.u_procp->p_cptr->p_pid;    /* pid to wait for */
        args.status = NULL;                     /* no status return */
        args.options = 0;                       /* no options - block */
        args.rusage = NULL;                     /* no rustatus return */
        args.compat = 0;                        /* no compat */

        wait4(u.u_procp, &args, retval);
        if (args.pid != retval[0])
            RPC_DBG_GPRINTF(("pthread_create: wait4 wanted %d got %d\n",
                args.pid, retval[0]));
    }
...
pthread_exc_ctxt_t ***_exc_ctxt_head;
