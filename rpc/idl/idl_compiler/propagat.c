 ...
 * HISTORY
 * $Log: propagat.c,v $
 * Revision 1.1.13.2  1996/02/18  18:52:32  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  18:05:47  marty]
 *
 * Revision 1.1.13.1  1995/12/07  22:21:46  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/dat_xidl2/1  1995/11/17  17:05 UTC  dat
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/12/07  21:12:59  root]
 * 
 * Revision 1.1.2.2  1995/10/31  21:15:34  bfc
 * 	fix filtering
 * 	[1995/10/31  21:14:56  bfc]
 * 
 * Revision 1.1.2.1  1995/10/23  01:48:37  bfc
 * 	oct 95 idl drop
 * 	[1995/10/23  00:44:25  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/21  22:58:40  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE 1.1
 * 	[1995/10/21  17:25:09  bfc]
 * 
 * Revision 1.1.9.2  1994/06/23  18:25:21  tom
 * 	From DEC: add opaque context handle support.
 * 	[1994/06/23  18:24:07  tom]
 * 
 * Revision 1.1.9.1  1994/02/18  15:28:31  rico
 * 	Fix for OT 9415 - IDL compiler crashes on pointer to type synonym.
 * 	[1994/02/18  15:27:22  rico]
 * 
 * Revision 1.1.6.2  1993/07/07  20:03:36  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:34:36  ganni]
 * 
 * $EndLog$
 */
/*
...
**
**  27-Apr-92   J. Harrow       Set HAS_FULL_PTRS to optimize node table initialization.
**  27-Dec-91   J. Harrow       Add logic to move large stack surrogate into 
**                              heap storage due to the small stack size normally
**                              provided by DCE threads.
*/
/*
**
**  Copyright (c) 1989, 1991, 1992, 1993 by
...
typedef struct
{
      ...
      boolean               has_interface;    /* T => type has interface    */
...
(
    ...
    long                string_set,     /* [in] TRUE => [string] on instance */
    long                ref_set         /* [in] TURE => [ref] on instance */
)
#else
(param_p, type_p, fattr_p, string_set, ref_set)
    AST_parameter_n_t   *param_p;       /* [in] Ptr to AST parameter node */
    AST_type_n_t        *type_p;        /* [in] Ptr to AST type node */
    AST_field_attr_n_t  *fattr_p;       /* [in] Field attributes on instance */
    long                string_set;     /* [in] TRUE => [string] on instance */
    long                ref_set;        /* [in] TRUE => [ref] on instance */
#endif
...
(
    AST_type_n_t        *type_p,        /* [in] Ptr to AST type node */
    long                set_in,         /* [in] TRUE => set [in] */
    long                set_out,        /* [in] TRUE => set [out] */
    long                set_out_pa_ref, /* [in] TRUE => set [out_pa_ref] */
    long                varying         /* [in] TRUE => set varying flags */
)
#else
(type_p, set_in, set_out, set_out_pa_ref, varying)
    ...
    long                set_out;        /* [in] TRUE => set [out] */
    long                set_out_pa_ref; /* [in] TRUE => set [out_pa_ref] */
    long                varying;        /* [in] TRUE => set varying */
#endif
    ...
    case AST_pointer_k:
        ...
        if (type_p->type_structure.pointer->pointee_type->array_rep_type != NULL)
        {
            type_process_in_out_attrs(
                ...
                set_in, set_out, set_out_pa_ref,
                varying || AST_STRING_SET(type_p));
        }
        break;
    ...
    case AST_structure_k:
    {
            ...
            ;   field_p = field_p->next)
        {
            ...
            type_process_in_out_attrs(field_p->type,
                set_in, set_out, set_out_pa_ref, AST_VARYING_SET(field_p));
        }
        break;
    }
        ...
        for (arm_p = union_p->arms
            ...
            if (arm_p->type != NULL)
            {
                ...
                type_process_in_out_attrs(arm_p->type,
                      set_in, set_out, set_out_pa_ref, AST_VARYING_SET(arm_p));
            }
        break;
    }
...

{
    ...
    boolean             ref_set;        /* TRUE if [ref] set */
    ...
    if (param_p->type == type_p)
        ref_set = AST_REF_SET(param_p)!=0;
    else
        ref_set = AST_REF_SET(type_p)!=0;

    type_prop_param(param_p, type_p, param_p->field_attrs,
                    AST_STRING_SET(param_p), ref_set);
...
(
    AST_type_n_t        *type_p,        /* [in] Ptr to AST type node */
    long                in,             /* [in] true if in */
    long                out,            /* [in] true if out */
    long                varying         /* [in] true if varying */
)
#else
...

{
     ...
     */
    if (parent_int_p == NULL && !AST_LOCAL_SET(int_p) && (int_p->op_count > 0))
    {
        visited_list = NULL;            /* Init the visited list */
...

{
    ...
    /* 
     * if generating c++ code and the operation has no
     * binding handle defined, and it is not an object
     * creator operation, nor a static member function,
     * make it's binding handle explicit
     */
    if ((int) CMD_vals[opt_lang] == opt_lang_cxx)
    {
        if (!IS_CONSTRUCTOR(op_p) && !AST_STATIC_SET(op_p))
	{
	    if (op_p->parameters == NULL ||
	        !type_is_handle(op_p->parameters->type))
	    {
		    AST_SET_EXPLICIT_HANDLE(op_p);
	    }
...
#ifdef PROTO
(AST_type_p_n_t **list_root, boolean filter_xmit_as)
#else
