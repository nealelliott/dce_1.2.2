 ...
 * HISTORY
 * $Log: object_read.c,v $
 * Revision 1.1.9.3  1996/02/18  00:30:36  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  23:16:45  marty]
 *
 * Revision 1.1.9.2  1995/12/11  15:18:58  root
 * 	Submit
 * 	[1995/12/11  14:33:19  root]
 * 
 * Revision 1.1.5.1  1994/10/25  19:46:43  baum
 * 	Used proper UUID to delete server in _2 (OT 12723)
 * 	[1994/10/25  19:46:28  baum]
 * 
 * Revision 1.1.2.2  1994/10/03  21:23:41  baum
 * 	read_object_all fixed (OT 12477)
 * 	[1994/10/03  21:22:43  baum]
 * 
 * Revision 1.1.2.1  1994/09/28  21:48:02  melman
 * 	moved admin tests to test/functional
 * 	[1994/09/28  21:15:42  melman]
 * 
 * Revision 1.1.2.3  1994/07/20  19:58:00  ganni
 * 	introduce the tests for srvrconf and srvrexec services
 * 	[1994/07/20  19:40:10  ganni]
 * 
 * Revision 1.1.2.2  1994/06/17  18:44:42  devsrc
 * 	     cr10872 - fix copyright
 * 	     [1994/06/17  18:32:10  devsrc]
 * 	Revision 1.1.2.1  1994/05/18  21:21:45  ganni
 * 	Initial Version
 * 	[1994/05/18  21:12:05  ganni]
 * 
 * $EndLog$
 */

/*
** Module: 	object_read.c
**
...
void dced_object_read_1(void);          /* Test purposes 1 */
void dced_object_read_2(void);          /* Test purposes 2 */
void dced_object_read_3(void);          /* Test purposes 3 */
void dced_object_read_4(void);          /* Test purposes 4 */
...
{
    { dced_object_read_1,  1 },
    { dced_object_read_2,  2 },
    { dced_object_read_3,  3 },
    { dced_object_read_4,  4 },
    { NULL, 0 }
};
...
dced_object_read_1()
{
	...
	tet_infoline("dced_object_read_1, Assertion 01(A):");
	tet_infoline(
	"When srvrconf dced binding handle is supplied to ");
	tet_infoline(
	"dced_object_read along with the object uuid");
	tet_infoline(
	"the associated object (server_t) is returned.");
...
{
        server_t		*out_data;
	uuid_t			srvr_id, sav_uuid;
        unsigned32              fail_stat;
        error_status_t          st;

	tet_infoline("dced_object_read_2, Assertion 02(A):");
	tet_infoline(
	...
	{
	        sav_uuid = server_entry.id;
	        server_entry.id = srvr_id;
	        if (!verify_server_t( &server_entry, (server_t *)out_data))
		{
#ifdef DEBUG
		    tet_infoline("verify_server_t failed");
...
dced_object_read_3()
{
	...
	tet_infoline("dced_object_read_3, Assertion 03(A):");
	tet_infoline(
	"When a hostdata dced binding handle is supplied to ");
	tet_infoline(
	"dced_object_read along with the hostdata entry id,");
	tet_infoline(
	"the associated object (sec_attr_t) is returned.");
...
dced_object_read_4()
{
	...
	tet_infoline("dced_object_read_4, Assertion 04(A):");
	tet_infoline(
	"When a keytab dced binding handle is supplied to ");
	tet_infoline(
	"dced_object_read along with the keytab entry id");
	tet_infoline(
	"the associated object (dced_key_list_t) is returned.");
