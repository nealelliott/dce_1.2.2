 ...
 * HISTORY
 * $Log: cma_ux.c,v $
 * Revision 1.1.6.1  1996/05/10  13:29:59  arvind
 * 	Drop 1 of DCE 1.2.2 to OSF
 *
 * 	HP revision /main/DCE_1.2/1  1996/03/08  21:52 UTC  bissen
 * 	Merge from branch jss_mothra_fix.
 * 	[1995/01/20  23:38 UTC  jss  /main/HPDCE02/5]
 *
 * 	More single threaded client changes.  cma_fork() should call at_fork() handlers
 * 	even if we are not completely initialized. Also add change to call libc_r
 * 	fork handler.
 * 	[1995/01/20  23:18 UTC  jss  /main/HPDCE02/jss_mothra_fix/1]
 *
 * 	Merge from branch jss_single_thread.
 * 	[1994/12/22  20:02 UTC  jss  /main/HPDCE02/4]
 *
 * 	Changes for single threaded client.  Don't use wrappers if single threaded.
 * 	[1994/12/22  19:25 UTC  jss  /main/HPDCE02/jss_single_thread/1]
 *
 * 	Merge from branch jss_fork_mothra.
 * 	[1994/12/21  20:50 UTC  jss  /main/HPDCE02/3]
 *
 * 	Restore fix for virtual timer after fork. (CHFts10135).
 * 	[1994/11/15  16:31 UTC  jss  /main/HPDCE02/jss_fork_mothra/1]
 *
 * 	Comment out fix for virtual timer after fork. (CHFts10135).
 ...
 *
 * 	Fix for CHFts10135. Turn on the virtual timer in the child after a fork.
 * 	[1994/06/16  15:04 UTC  jss  /main/HPDCE01/jss_rodan_r/1]
 *
 * 	merge kk and hpdce01
 * 	[1994/01/25  15:12  truitt  /main/HPDCE01/1]
 *
 * 	merge kk and dce1_0_3
 * 	[1993/09/21  15:58:08  truitt]
 * 	 *
 *
 * 	Loading drop DCE1_0_3b03
 * 	[1993/09/15  17:44:52  root  1.1.9.3]
 *
 * 	Initial King Kong branch
 * 	[1993/07/08  19:12:59  root]
 *
 * 	[hinman@sni] - Check in SNI merged version (these files have no tainted or private code
 * 	[1993/06/17  14:48:22  hinman]
 *
 * 	remove include of cma_uxcalls.h
 * 	[1993/03/30  22:50:25  hopkins]
 *
 * 	Gamera merge
 * 	[1993/03/26  23:57:16  hopkins]
 *
 * 	Loading drop DCE1_0_2B18
 * 	[1993/03/11  22:29:24  osfrcs  1.1.1.8]
 *
 * 	DCE 1.0.1 Release
 * 	[1992/09/23  20:39:45  hopkins]
 *
 ...
 *
 * Revision 1.1.2.3  1994/06/13  17:57:18  jd
 * 	Added wrapper for cma_execv and cma_execve
 * 	[1994/06/13  14:23:05  jd]
 * 
 * Revision 1.1.2.2  1994/06/09  13:34:55  devsrc
 * 	 CR10892 - fix copyright in file
 * 	[1994/06/09  13:25:46  devsrc]
 * 
 * Revision 1.1.2.1  1994/04/01  20:14:03  jd
 * 	Moved from src/threads to here
 * 	[1994/04/01  17:04:35  jd]
 * 
...
cma_fork(void)
    ...
    /* In single threaded client mode, RPC still needs to execute its
     * atfork() handlers even if we do not need to execute the entire
     * CMA fork wrapper code. So we will do that here.
     */
    ...
    /* If we got here we are in multi-threaded mode, so execute all of the CMA
     * fork wrapper code.
     */
    cma__trace ((
	    cma__c_trc_wrp,
...
#ifdef _HP_LIBC_R
    /*
     * If we're on HPUX system and using libc_r instead of wrappers, then
     * we need to prepare the libc_r mutexes for forking.
     */
    _cma__reinit_libc (cma__c_reinit_prefork_lock);
#endif
    cma__int_lock (cma__g_global_lock);
    cma__set_kernel ();		/* No interference! */

    getitimer(ITIMER_VIRTUAL, &unix_oldtime);       
	 ...
	 * we need to prepare the libc_r mutexes for forking.
	 */
	_cma__reinit_libc (cma__c_reinit_postfork_unlock);
#endif
	...
	cma__reinit_memory (cma__c_reinit_postfork_unlock);
#ifdef _HP_LIBC_R
	cma__reinit_hp (cma__c_reinit_postfork_clear);
	cma__reinit_hp (cma__c_reinit_postfork_unlock);
#endif
	...
	cma__trace ((
		...
		"(fork) child completed postfork actions"));
	/*
	 * CHFts10135. Some of the DCE daemons depend upon being able to 
	 * continue execution after a fork in a multi-threaded environment.
	 * So we restore the virtual timer here to allow pre-emption.
	 */
