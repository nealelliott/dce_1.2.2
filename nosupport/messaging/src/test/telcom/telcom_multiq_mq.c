 ...
 * ______________________________________________________________________
 *
 * MODULE:       telcom_multiq_mq.c
 * 
 * DESCRIPTION:  Illustrates using MOS-API calls -
 *               This program will:
 *                   -Create or attach three queues by name
 ...
 *                   -Enqueue five "charge" messages to the "charge" queue
 *                   -Enqueue five "mwi" messages to the "mwi" queue
 *                   -Dequeue all "charge" messages from the "charge" queue
 *                   -Dequeue all "mwi" messages from the "mwi" queue
 *                   -Dequeue all ACKs from the reply queue on "charge" msgs.
 *                   -Exit MOS
 *
 * ADDED NOTE:
 *               In this program, the same datatype messages are within a queue.
 *               For example: 
 *                   The "charge" queue holds only "charge" messages.
 *                   The "mwi" queue holds only "mwi" messages.
 *                   The "reply" queue holds only "ack" messages.
 * ______________________________________________________________________
 *
 * $Log: telcom_multiq_mq.c,v $
 * Revision 1.1  1996/11/20  17:13:39  salamone
 * Initial version
 *
 * $EndLog$
 * ______________________________________________________________________
 ...
 * INTERNAL VARIABLES
 */
static unsigned_char_t  *charge_que_name = (unsigned_char_t *)"charge";
static unsigned_char_t  *mwi_que_name = (unsigned_char_t *)"mwi";
static unsigned_char_t  *reply_que_name = (unsigned_char_t *)"reply";
...
/*
 * Resource handles to the Q-mgr, charge, mwi, and reply queues
 */
mos_handle_t     dflt_qmgrh;
mos_handle_t     charge_qh;
mos_handle_t     mwi_qh;       
...
telcom__find_resources(error_status_t *      status)
{
   ...
   if (GOOD_STATUS(status)) {
       ...
       if (*status == mos_queue_already_exists) {
           mos_rsrc_attach(charge_que_name,
   ...
   if (GOOD_STATUS(status)) {
       mos_que_mgmt_create(
                      (void *)NULL,     /* Default queue manager used */
                      reply_que_name,   /* Junction name under default Q-mgr */
                      (void *)NULL,     /* Use default queue attributes */
                      &reply_qh,
                      status);
   }
