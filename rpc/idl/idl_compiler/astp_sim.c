 ...
 * HISTORY
 * $Log: astp_sim.c,v $
 * Revision 1.1.12.2  1996/02/18  18:50:58  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  18:04:38  marty]
 *
 * Revision 1.1.12.1  1995/12/07  22:13:28  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/dat_xidl2/1  1995/11/17  17:01 UTC  dat
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/12/07  21:09:54  root]
 * 
 * Revision 1.1.2.1  1995/10/23  01:47:22  bfc
 * 	oct 95 idl drop
 * 	[1995/10/23  00:42:52  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/21  22:55:21  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE 1.1
 * 	[1995/10/21  17:23:29  bfc]
 * 
 * Revision 1.1.8.1  1993/10/01  13:29:46  hinxman
 * 	OT 8874 - change design for spelling of #include generated from ACF include
 * 	statement
 * 	[1993/10/01  13:29:27  hinxman]
 * 
 * Revision 1.1.6.2  1993/07/07  19:55:36  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:29:40  ganni]
 * 
 * $EndLog$
 */
/*
...
**
**  12-Dec-91   A.I.Hinxman A handle_t passed by reference does not make
**                          AST_HAS_INS true"
*/
...
#else
(declarator, op_kind, op_info, pointer_count, const_pointer)
    ASTP_declarator_n_t     *declarator;
    AST_type_k_t            op_kind;
    ASTP_node_t             *op_info;
    int                     pointer_count;
...
#endif
{
     ...
     */
    if (overflow)
        log_warning(yylineno, NIDL_TOOMANYELEM, "enum") ;
...
#endif
{
     ...
     *  type if neither REF or UNIQUE or & attributes were not specified.  Also
     *  don't set it if it is a "void *" because either it has [context_handle] or
     *  checker will issue an error anyway.
     *  Don't set it if it is a interface reference (interface *).
     */
	...
	!AST_AMP_SET(operation_node_p->result))
...
#endif
{
     ...
     */
    if (AST_AMP_SET(result_type)) AST_SET_AMP(operation_node_p->result);
...
/*
 *  A S T _ o p t _ i m p o r t _ n o d e
 *  =====================================
 *
 *  Searches given interface's import list for given filename.
 *  If an entry already exists, returns NULL.  Otherwise, creates and
 *  initializes an import node and returns its address.
 */

AST_import_n_t *AST_opt_import_node
#ifdef PROTO
(
    STRTAB_str_t imported_file,
...
#endif
{
    ...
    for (import_node_ptr = interface_p->imports;
         import_node_ptr != NULL;
         import_node_ptr = import_node_ptr->next)
    {
        if (imported_file == import_node_ptr->file_name)
            return NULL;    /* already exists, no new import created */
    }

    return AST_import_node(imported_file);
}
