 ...
 * HISTORY
 * $Log: exp_command.c,v $
 * Revision 1.1.2.2  1996/10/03  17:55:05  arvind
 * 	Add signal handling.
 * 	[1996/06/18  13:25 UTC  psn  /main/psn_krb5/1]
 *
 * 	Port to tcl_dce
 * 	[1996/06/18  13:25 UTC  psn  /main/psn_krb5/1]
 *
 * Revision 1.1.2.1  1996/08/09  19:18:27  arvind
 * 	Port to tcl_dce
 * 	[1996/06/18  13:25 UTC  psn  /main/psn_krb5/1]
 * 
 * $EndLog$
...
#define NO_CHILD -2
	...
	if (result == -1) {
		sprintf(interp->result,"%d %d -1 %d POSIX %s %s",
			f->pid,m,errno,Tcl_ErrnoId(&dummy_res),strerror(errno));
		result = TCL_OK;
		...
		if (WIFSIGNALED(f->wait)) {
			...
			Tcl_AppendElement(interp,Tcl_SignalId((int)(WTERMSIG(f->wait)), &dummy_res));
			Tcl_AppendElement(interp,Tcl_SignalMsg((int) (WTERMSIG(f->wait))));
		} else if (WIFSTOPPED(f->wait)) {
			Tcl_AppendElement(interp,"CHILDSUSP");
			Tcl_AppendElement(interp,Tcl_SignalId((int) (WSTOPSIG(f->wait)), &dummy_res));
