...
MODIFICATIONS:
		...
		Changed debug information in do_rescode_file() and in
		rescode_num_to_action();
		David G. Sawyer, UniSoft Ltd, 5 Apr 1992.

		Changed process_line() to deal with whitespace at the end 
		of directives and multiple test cases per line.
		David G. Sawyer, UniSoft Ltd, 6 Apr 1992.

		Changed do_rescodes_file() so that names for reserved
		result codes can be changed - changing of the action
		didn't worked either - thats now fixed aswell.
		David G. Sawyer, UniSoft Ltd, 6 Apr 1992.
...
static char zeetime[18];       /* Big enough to contain "HH:MM:YY YYYYMMDD" */
...
#ifdef DBG2
		(void) fprintf(stderr,"Return from build is %d\n", rc);
#endif
            }      
            ...
            if ((exec_mode == TRUE) && (rc != FAILURE))
            {
                ...
                rc = exec_tc(runline);
#ifdef DBG2
		(void) fprintf(stderr,"Return from exec is %d\n", rc);
#endif
            }
...
#ifdef DBG2
		(void) fprintf(stderr,"Return from clean is %d\n", rc);
#endif
            }
...
int get_rescode_file()
{
    ...
    /*
     *  Build up the file name... tet_root + suite_name + file_name
     */
    (void) strcpy(rcf_name,tet_root);
    if ((rcf_name)[strlen(rcf_name)-1] != DIR_SEP_CHAR)
        (void) strcat(rcf_name,DIR_SEP_STR);
...
#ifdef DBG
    (void) fprintf(stderr,"do_rescode_file(rc_fp\n");
#endif
