 ...
 * HISTORY
 * $Log: ddbe.h,v $
 * Revision 1.1.8.2  1996/02/18  23:45:09  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  22:44:04  marty]
 *
 * Revision 1.1.8.1  1995/12/07  22:16:23  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/HPDCE02/jrr_1.2_mothra/1  1995/11/20  19:57 UTC  jrr
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/11/17  17:02 UTC  dat  /main/dat_xidl2/1]
 * 
 * 	may 5 version from nightly
 * 	[1995/10/31  18:45:37  bfc]
 * 	 *
 * 	DCE for DEC OSF/1: populate from OSF DCE R1.1
 * 	[1995/10/31  18:43:43  bfc]
 * 	 *
 * 
 * 	HP revision /main/HPDCE02/2  1995/03/05  14:01 UTC  gaz
 * 	Merge DMS changes to mainline
 * 
 * 	HP revision /main/HPDCE02/lmm_mothra_instr/1  1995/02/01  23:56 UTC  lmm
 * 	Change internal #ifdef macro to _HP_DCE_INSTR_
 * 
 * 	HP revision /main/HPDCE02/1  1994/11/01  23:52 UTC  lmm
 * 	adding DMS support
 * 	[1995/12/07  21:11:03  root]
 * 
 * Revision 1.1.4.1  1994/08/26  17:02:42  tom
 * 	Bug 11901 - Increase encoding minor version number to 2.
 * 	[1994/08/26  15:11:19  tom]
 * 
 * Revision 1.1.2.2  1993/07/07  20:45:34  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  20:43:39  ganni]
 * 
 * $EndLog$
 */
/*
...
#define DDBE_result_is_param(oper_p, lang_option) \
    (lang_option == opt_lang_fortran \
     && oper_p->result->type->kind != AST_void_k \
     && oper_p->result->type->kind != AST_pointer_k \
     && (!type_is_scalar(oper_p->result->type) \
         || oper_p->result->type->kind == AST_hyper_integer_k \
         || oper_p->result->type->kind == AST_hyper_unsigned_k))
...
#ifdef _HP_DCE_INSTR_
/*
 * macro used by stub generation code to spell out timestamps 
 */
#define DMS_server_side_k 0
#define DMS_client_side_k 1
#define GEN_DMS_TIMESTAMP(metric, side) \
  fprintf(fid, "#ifdef _DCE_INSTR_\n");\
  fprintf(fid, "DMS_TIMESTAMP(&time_ts);\n");\
  fprintf(fid, "rpc_ss_dms_timestamp(%sIDL_call_h, time_ts, %s);\n", \
	  (side ? "IDL_ms." : ""), metric);\
  fprintf(fid, "#endif\n"); 
#endif /* _HP_DCE_INSTR_ */
...
typedef struct DDBE_vec_rep_t {
    union {                         /* Arm valid for kind ==            */
        ...
        NAMETABLE_id_t  name;       /* name*_k, tag_k                   */
