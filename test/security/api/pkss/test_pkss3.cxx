...
//
// HISTORY
// $Log: test_pkss3.cxx,v $
// Revision 1.1.2.1  1996/11/14  17:14:39  arvind
// 	OSF DCE 1.2.2 Drop 5
// 	[1996/11/14  16:03:30  arvind]
//
// Revision /main/DCE_1.2.2/1  1996/11/05  22:03 UTC  arvind
// 	Change sec_pk_domain_dce_pk_login to sec_pk_domain_dce_general
// 	[1996/11/04  15:31 UTC  arvind  /main/arvind_pkss/3]
// 
// 	Correction to PKSS tests
// 	[1996/09/11  14:09 UTC  arvind  /main/arvind_pkss/2]
// 
// $EndLog$
//
// (c) Copyright 1996,
...
// All Rights Reserved
//
// Module:  test_pkss_3.cxx
//
// Purpose: Unit tests for PKSS:  sec_pvtkey_pkss_open
//				  sec_pvtkey_pkss_generate
//				  sec_pvtkey_pkss_update (mgmt)
//				  sec_pvtkey_pkss_update_generate (mgmt)
//				  sec_pvtkey_pkss_get
...
#define USER3_USERNAME	((unsigned char *)"user3")
#define USER3_PASSWORD	((unsigned char *)"user3pwd")
#define USER3_NEWPWD	((unsigned char *)"user3new")
...
int loc_main(int argc, char * argv[]) {
  ...
  sec_pk_mechanism_handle_t user3_psm_context = 0;
  char i_val[512];
  char h_val[2048];
  int size;
  ...
  (void) sprintf ((char *)uname, "%s%d", USER3_USERNAME, getpid());

  //
  // Open a session with the PKSS for USER3.
  //
  ...
  estatus = sec_pvtkey_pkss_generate (user3_psm_context,
			      (unsigned char *)USER3_PASSWORD,
  ...
  //
  // Mgmt change key pair; mgmt client supplies key pair.
  //
  ...
  estatus = sec_pvtkey_pkss_update (user3_psm_context,
				    (unsigned char *)0,
			            (unsigned char *)USER3_NEWPWD,
				    key_usages,
				    &ret_key_vno,
				    &kxu,
				    &kou);
  ...
  //
  // Obtain asymmetric key pair for mgmt chg, client generates key.
  //
  ...
  estatus = sec_pvtkey_pkss_get (user3_psm_context,
			    (unsigned char *)USER3_NEWPWD,
			    key_usages,
  ...
  //
  // Mgmt change key pair; PKSS supplies key pair.
  //
  ...
  estatus = sec_pvtkey_pkss_update_generate (user3_psm_context,
				    (unsigned char *)0,
			            (unsigned char *)USER3_NEWPWD,
			            (sec_pk_algorithm_id_t *)0,
				    pk_key_len,
...
PVTKEY_DELETE:
  ...
  pstatus = sec_pvtkey_pkss_delete (user3_psm_context,
				    (unsigned char *)USER3_PASSWORD,
				    key_usages,
				    key_vno);
...
PVTKEY_CLOSE:
  ...
  pstatus = sec_pvtkey_pkss_close (user3_psm_context);
...
EXIT:
  ...
  #ifdef ENVIRONMENT_TET
     { char text[256];
       sprintf(text, "Exiting test_pkss_3; estatus = 0x%x\n", estatus);
       tet_infoline(text);
       }
  #endif

  (void) printf ("Exiting test_pkss_3; estatus = 0x%x\n", estatus);
  (void) fflush (stdout);
...
static void tp(void) {
    char *argv[128];
	int argc;
	char * progname = "test_pkss_3";
	int result;

	argc = 1;
	argv[0] = progname;

	tet_infoline("Invoking test_pkss_3 program");
	result = loc_main(argc, argv);
	tet_infoline("Returned from test_pkss_3 program");
