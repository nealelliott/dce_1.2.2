 ...
 * HISTORY
 * $Log: ddbe.c,v $
 * Revision 1.1.6.2  1996/02/18  18:51:32  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  18:05:04  marty]
 *
 * Revision 1.1.6.1  1995/12/07  22:16:07  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/HPDCE02/jrr_1.2_mothra/1  1995/11/20  19:52 UTC  jrr
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/11/17  17:02 UTC  dat  /main/dat_xidl2/1]
 * 
 * 	oct 95 idl drop
 * 	[1995/10/23  00:43:21  bfc]
 * 	 *
 * 	may 95 idl drop
 * 	[1995/10/21  22:56:23  bfc]
 * 	 *
 * 	DCE for DEC OSF/1: populate from OSF DCE 1.1
 * 	[1995/10/21  17:24:01  bfc]
 * 	 *
 * 
 * 	HP revision /main/HPDCE02/1  1994/08/03  16:31 UTC  tatsu_s
 * 	Merged mothra up to dce 1.1.
 * 
 * 	HP revision /main/HPDCE01/1  1994/04/29  20:12 UTC  tatsu_s
 * 	Submitted tatsu_s_idl_spell_comment.
 * 
 * 	HP revision /main/tatsu_s_idl_spell_comment/1  1994/04/22  13:28 UTC  tatsu_s
 * 	Enabled the comment generation #if defined(HP_IDL_SPELL_COMMENT).
 * 	[1995/12/07  21:10:58  root]
 * 
 * Revision 1.1.2.2  1993/07/07  19:57:43  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:31:11  ganni]
 * 
 * $EndLog$
 */
/*
 ...
 */
#if defined(DUMPERS) || defined(HP_IDL_SPELL_COMMENT)
...
#endif
{
    NAMETABLE_id_t      nameid;     /* Interpreter tag name ID */
...
#endif
{
    DDBE_vec_rep_t      *ref_vec_p; /* Referenced vector entry */
 ...
 * routines below.
 *
 * After calling DDBE_push_indirection_scope, the separate definition vector
 * entries are made using &vip->defn_p (or vip->p_cur_p since the definition
 * vector is made the "current" vector).  Entries in the "parent", i.e. the
 * site that references the definition vector entries, i.e. the instance,
 * are made using vip->ind_sp which is the indirection scope pushed on the
 * stack.  In particular, type/definition vector entries in the parent are
 * made using &vip->ind_sp->ind_p.
 *
 * The most complicated case is when you call DDBE_push_indirection_scope,
 * and you are already processing definition vector entries (e.g. a struct
 * which contains a union).  In this case you simultaneously have two
 * distinct pointers into definition vector entries.  To make this case work
 * correctly, you must make an entry in the child scope (union, in the example)
 * before continuing to make entries in the parent scope (struct, in the
 * example).  In some cases this may force you to put a 'noop' entry in the
 * child's definition vector entries so you can add definition vector entries
 * in the parent.
 */
...
#endif
{
     ...
     * if necessary.
     */
    sprintf(inst_name, "%si_%d", DDBE_PREFIX_IDL, DDBE_inst_num++);
...
#endif
{
    ...
    if (IR_cur_scope(vip->ir_ctx_p) == IR_SCP_UNION && !vip->in_default_arm)
    {
        ...
        DDBE_pad_vec_entry(vip->p_cur_p, 
                          (byte)(DDBE_ARM_SIZE - vip->arm_byte_cnt));
    }
}
...
#endif
{
     ...
     */
    /*  byte: dt_modified (iff type tps_rpc_t_request_ctx) */
    if (strcmp(type_name, "tps_rpc_t_request_ctx") == 0)
        DDBE_tag_vec_entry(&vip->ind_sp->ind_p, DDBE_dt_modified);
...
#endif
{
    ...
    char                *bound_text;        /* Bound attr text for comment */
...
#endif
{
    ...
    char                *limit_text;        /* Limit attr text for comment */
    ...
    if (limit_kind == IR_lim_string_k)
    {
        ...
        if (tup_p->flags & IR_REP_AS)
        {
            ...
            DDBE_byte_vec_entry(&vip->defn_p, DDBE_vec_byte_k,
                (byte)tup_p->arg[IR_ARG_INT].int_val, "octetsize");
        DDBE_long_vec_entry(&vip->defn_p, 
                            DDBE_vec_long_k, (byte)-1, "dummy value");
        return;
    }
...
#endif
{
    ...
    if (pres_p->name == NAMETABLE_NIL_ID)
        NAMETABLE_id_to_string(
            pres_p->type_structure.pointer->pointee_type->name, &pres_name);
    else
