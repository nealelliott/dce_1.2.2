...
/*
 * HISTORY
 * $Log: mgmt_inq_com_timeout.c,v $
 * Revision 1.1.4.2  1996/02/17  23:20:46  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:39:38  marty]
 *
 * Revision 1.1.4.1  1995/12/11  20:05:49  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  19:43:33  root]
 * 
 * Revision 1.1.2.3  1993/01/25  18:05:09  bbelch
 * 	Fix duplicate log
 * 	[1993/01/25  17:37:16  bbelch]
 * 
 * Revision 1.1.2.2  1993/01/11  23:35:17  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/06  18:07:30  bbelch]
 * 
 * Revision 1.1  1992/01/19  03:35:26  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
...
switch(SubCase)
    {
    case INQ_COM_TIMEOUT:
        TSHMessage(ctx, MOINFO, "Running subcase INQ_COM_TIMEOUT\n");

        rpc_mgmt_set_com_timeout(binding, timeout, &status);
        if (TSHStatus(ctx, SETUP, "rpc_mgmt_set_com_timeout() error", 
           status, rpc_s_ok))
           {
           return(status);
           }
    ...
    case INVALID_BINDING:
        ...
        rpc_mgmt_inq_com_timeout(tsh_c_invalid_handle, &TestTimeout, &status);
        if (TSHStatus(ctx, TEST, "rpc_mgmt_inq_com_timeout() error", 
           status, EStatus))
           {
           return(status);
