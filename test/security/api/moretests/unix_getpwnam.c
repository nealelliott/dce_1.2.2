...
/*
 * HISTORY
 * $Log: unix_getpwnam.c,v $
 * Revision 1.1.9.2  1996/02/17  23:25:57  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:43:58  marty]
 *
 * Revision 1.1.9.1  1995/12/11  21:50:34  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  20:53:14  root]
 * 
 * Revision 1.1.7.1  1993/10/07  18:59:00  mccann
 * 	CR9057 remove unnecessary verison requirement
 * 	[1993/10/07  18:57:28  mccann]
 * 
 * Revision 1.1.5.3  1993/02/25  22:09:16  cuti
 * 	Change sec_rgy_site_open() to sec_rgy_site_open_update() for using site update operation.
 * 	[1993/02/25  16:35:00  cuti]
 * 
 * Revision 1.1.5.2  1992/12/31  17:34:37  htf
 * 	Embedding copyright notice
 * 	[1992/12/31  16:59:11  htf]
 * 
 * Revision 1.1.3.2  1992/06/02  21:26:08  sekhar
 * 	Drop Unix interface changes made by frisco
 * 	[1992/06/02  20:15:48  sekhar]
 * 
 * Revision 1.1.1.2  1992/05/22  20:14:34  frisco
 * 	Initial revision - getpwnam test
 * 
 * $EndLog$
 */
/* 
...
#ifndef LINT
static char sccs_id[] = "src/test/security/api/moretests/unix_getpwnam.c";
#endif
...
/* Local variables */
   ...
   sec_rgy_name_t       	name;	/* sec_rgy_unix_getpwnam */
   unsigned32              name_len;
...
/* Call the unix interface under test. */
   ...
   TSHMessage (ctx, MOTRACE,
               "sec_rgy_unix_getpwnam (test) status: %d\n", tstatus);

   if (TSHStatus (ctx, SETUP,
                  "sec_rgy_unix_getpwnam failed\n", rpc_s_ok, tstatus))
     goto EXIT_DEL_ORG_MEMBER;

/* If the lookup fails or if the data supplied as input to acct_add does
   not match the data returned as output from the getpwnam, then treat
   the getpwnam as if it had failed. */
    ...
    if (strcmp(passwd_cmp.name, 
       ...
       || (passwd_cmp.oid != passwd_entry.oid)) {
         ...
         TSHStatus (ctx, TEST, "sec_rgy_unix_getpwnam failed - entry mismatch\n",
                 tstatus, rpc_s_ok);
    }
