...
add_login_context(Tcl_Interp *interp, sec_login_handle_t login_context_h)
{
    ...
    if (new_context == NULL) {
        DCP_SET_RESULT_CODE(mcp_s_no_memory);
        return TCL_ERROR;
    }
    ...
    if (pthread_mutex_lock(&login_list) != 0) {
        DCP_SET_RESULT_CODE(mcp_s_mutex_lock_fail);
    ...
    if (pthread_mutex_unlock(&login_list) != 0) {
        DCP_SET_RESULT_CODE(mcp_s_mutex_unlock_fail);
        return TCL_ERROR;
    }
...
delete_login_context(Tcl_Interp *interp)
{
    ...
    if (global_context_list == NULL) {
        DCP_SET_RESULT_CODE(mcp_s_not_logged_in);
        return TCL_ERROR;
    }
    ...
    if (initial_context_flag) {
        if (global_context_list->next == NULL) {
            ...
            DCP_SET_RESULT_CODE(mcp_s_purge_initial_context);
            return TCL_ERROR;
        }
    }
...
check_initial_auth(Tcl_Interp *interp)
{
    ...
    /*
     * Initialize the mutex for the global_context_list regardless of
     * if we use it here. 
     */
    if (pthread_mutex_init(&login_list, pthread_mutexattr_default) != 0) {
        DCP_SET_RESULT_CODE(mcp_s_mutex_init_fail);
        return TCL_ERROR;
    }
