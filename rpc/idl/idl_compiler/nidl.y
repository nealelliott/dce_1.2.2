 ...
 * HISTORY
 * $Log: nidl.y,v $
 * Revision 1.1.13.2  1996/03/09  23:25:50  marty
 * 	Update OSF copyright year
 * 	[1996/03/09  22:41:30  marty]
 *
...
interface_derivation:
	/* Nothing */
	{
     ...
     | COLON interface_derivation_list
	{
	    the_interface->inherited_interface_name = $<y_id>2;
	}
    ;

interface_derivation_list:
	IDENTIFIER
     ;
...
type_spec:
        optional_const_qual simple_type_spec
	{
		$<y_type>$ = $<y_type>2;
	}
  |   optional_const_qual constructed_type_spec
	{
		$<y_type>$ = $<y_type>2;
	}
  ;
...
optional_tag: IDENTIFIER | /* Nothing */;
...
optional_value:
	/* Nothing */
	{ $<y_exp>$.type = AST_int_const_k; $<y_exp>$.val.integer = 0; }
   | EQUAL expression
	{ $<y_exp>$ = $<y_exp>2; }
   ;
...
operation_dcl:
        attributes optional_static type_spec declarators
        {
            if (ASTP_parsing_main_idl) {
                $<y_operation>$ = AST_operation_node (
                                    $<y_type>3,         /*The type node*/
                                    $<y_declarator>4,   /* Declarator list */
                                   &$<y_attributes>1);  /* attributes */
              if ($<y_ival>2)
	      {
		AST_SET_STATIC($<y_operation>$);
		$<y_operation>$->cxx_static_name = NULL;
	      }
	    }
            ASTP_free_simple_list((ASTP_node_t *)$<y_attributes>1.bounds);
        }
    | attributes optional_static error
        {
