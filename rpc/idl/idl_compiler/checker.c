 ...
 * HISTORY
 * $Log: checker.c,v $
 * Revision 1.1.17.2  1996/02/18  18:51:02  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  18:04:42  marty]
 *
 * Revision 1.1.17.1  1995/12/07  22:13:59  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/dat_xidl2/1  1995/11/17  17:01 UTC  dat
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/12/07  21:10:05  root]
 * 
 * Revision 1.1.2.2  1995/10/31  21:15:19  bfc
 * 	fix filtering
 * 	[1995/10/31  21:14:25  bfc]
 * 
 * Revision 1.1.2.1  1995/10/23  01:47:26  bfc
 * 	oct 95 idl drop
 * 	[1995/10/23  00:42:57  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/21  22:55:32  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE 1.1
 * 	[1995/10/21  17:23:34  bfc]
 * 
 * Revision 1.1.12.3  1994/06/23  18:25:10  tom
 * 	From DEC: add opaque context handle support.
 * 	[1994/06/23  18:23:54  tom]
 * 
 * Revision 1.1.12.2  1994/05/03  18:50:37  tom
 * 	From DEC: Add extended string support.
 * 	[1994/05/03  18:45:51  tom]
 * 
 * Revision 1.1.12.1  1994/02/14  15:36:46  rico
 * 	CR 9355: fix to allow [comm_status] on an [encode,decode] operation
 * 	CR 9356: fix to warn when [fault_status] used on an encode/decode operation
 * 	[1994/02/11  16:24:49  rico]
 * 
 * Revision 1.1.9.2  1993/10/14  12:31:44  hinxman
 * 	CR 8900 Warn that anonymous enum is not portable
 * 	[1993/10/14  12:31:17  hinxman]
 * 
 * Revision 1.1.9.1  1993/09/27  14:58:45  hinxman
 * 	CR 8680 - Allow [transmit_as] conformant type on base type of array
 * 	[1993/09/27  14:58:07  hinxman]
 * 
 * Revision 1.1.7.3  1993/07/14  20:15:33  hinxman
 * 	Fix for CR 7224 - semantic checking recognizes a named type that resolves
 * 	to void * as an invalid construct.
 * 	Also make ACF text appear when warnings about ACF are issued.
 * 	[1993/07/14  18:42:53  hinxman]
 * 
 * Revision 1.1.7.2  1993/07/07  19:56:12  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:30:05  ganni]
 * 
...

{
    ...
    long                is_ref_ptr;     /* 1 => ref_type_p is [ref] ptr */
...

{
    ...
    /* Arrays of interface are not allowed */
...

{
    ...
    if (!AST_LOCAL_SET(int_p)
        &&  type_p->xmit_as_type == NULL  /* Allow if void* isn't transmitted */
        &&  type_p->kind == AST_pointer_k
        &&  type_p->type_structure.pointer->pointee_type->kind == AST_void_k
        &&  !AST_CONTEXT_RD_SET(type_p)
...

{
    ...
    type_p = type_xmit_type(type_p);    /* Pick up transmissible type */
    type_p = ASTP_chase_ptr_to_kind(type_p, AST_structure_k);
...

{
    ...
    /*
     * If the parameter is a pointer, and it is a pointer to an interface,
     * then it should ignore the pointer attributes (REF, UNIQUE, or PTR).
     */

    if (top_type_p->kind == AST_pointer_k
        &&  type_p->kind == AST_interface_k
	&& AST_REF_SET(param_p))
    ...
    if (AST_OUT_SET(param_p)
        ...
        &&  ((top_type_p->kind != AST_pointer_k
          ...
          || (top_type_p->kind == AST_pointer_k     /* void * is special */
             && top_type_p->type_structure.pointer->pointee_type->kind
                == AST_void_k)
          || (top_type_p->kind == AST_pointer_k     /* interface * is special */
             && top_type_p->type_structure.pointer->pointee_type->kind
                == AST_interface_k)
	))
...

{
    ...
    /* Interface must be used as a * or &, but not just interface */
    ...
    param_struct(param_p, top_type_p, type_p, int_p);
...
**  o p _ s t a t i c
**
**  Checks an operation node's [cxx_static] attributes.
*/
...
{
    /* [cxx_static] is only used for -lang cxx. */

    if (AST_STATIC_SET(op_p))
    {
       if ((int)cmd_val[opt_lang] != opt_lang_cxx)
       {
	    /* reset cxx_static name to be the operation name */
	    op_p->cxx_static_name = op_p->name;
	    CHECKER_acf_warning(op_p, NIDL_STATICIGN);
       }
    }
}
...
/*
**  o p _ c o n s t r u c t o r
**
**  Checks an operation node's [cxx_new] attributes.
*/

static void op_constructor
#ifdef PROTO
(
    AST_operation_n_t   *op_p           /* [in] Ptr to AST operation node */
)
...

{
    ...
    if (IS_CONSTRUCTOR(op_p))
    {
	/* [cxx_new] is only used for -lang cxx. */

        if ((int)cmd_val[opt_lang] != opt_lang_cxx)
	    CHECKER_acf_error(op_p, NIDL_NEWNOTVAL);

        /* [cxx_new] operations must return an interface pointer */

	if (op_p->result->type->kind != AST_pointer_k ||
	    (op_p->result->type->kind == AST_pointer_k &&
	     op_p->result->type->type_structure.pointer->pointee_type->kind 
	     != AST_interface_k))
        {
            CHECKER_acf_error(op_p, NIDL_CONSTRUCTOR);
        }
    }
...

{
    ...
    deref_type_p = ASTP_chase_ptr_to_type(type_p);
    if (deref_type_p->kind == AST_pipe_k
        &&  node_p->fe_info->node_kind != fe_parameter_n_k)
...

{
    ...
    /* Base type of a pipe cannot be an interface */

    if (type_p->kind == AST_interface_k)
        CHECKER_error(pipe_p, NIDL_PIPECTYPE, "interface");

    /* Base type of a pipe cannot be an interface reference */
    if (type_p->kind == AST_pointer_k
        &&  type_p->type_structure.pointer->pointee_type->kind == AST_interface_k)
...

{
    ...
    /*
     * If the type is a pointer, and it is a pointer to an interface,
     * then it should ignore the pointer attributes (REF, UNIQUE, or PTR).
     */
    if (type_p->kind == AST_pointer_k
        &&  type_p->type_structure.pointer->pointee_type->kind == AST_interface_k
...

{
    ...
    max_len = MAX_ID - (int)strlen("_v#_#_c_ifspec"); /* Could do better here */
    if ((int)strlen(int_name) > max_len)
...

{
    ...
    if (AST_LOCAL_SET(int_p)
        && ((int) cmd_val[opt_lang] != opt_lang_cxx)
        &&  !uuid_null)
...
**  i n t _ l o o k u p
**
**  Checks an interface node's [cxx_lookup] attributes.
*/

static void int_lookup
#ifdef PROTO
(
    AST_interface_n_t   *int_p          /* [in] Ptr to interface node */)
#else
...
{
    /*
     * [cxx_lookup] is allowed only for -lang cxx.
     */
    if (int_p->lookup_callout_name != NULL)
    {
       if ((int)cmd_val[opt_lang] != opt_lang_cxx)
	    CHECKER_acf_error(int_p, NIDL_LOOKUPNOTVAL);
    }
...
/*
**  i n t _ d e l e g a t e
**
**  Checks an interface node's [cxx_delegate] attributes.
*/

static void int_delegate
#ifdef PROTO
(
    AST_interface_n_t   *int_p          /* [in] Ptr to interface node */)
#else
...
{
    /*
     * [cxx_delegate] is allowed only for -lang cxx.
     */
    if (int_p->cxx_delegate_name != NULL)
    {
       if ((int)cmd_val[opt_lang] != opt_lang_cxx)
	    CHECKER_acf_error(int_p, NIDL_DELEGATENOTVAL);
    }
...

{
    ...
    /*
     * inherited interface must be imported, thus have a binding defined for it
     */
    if (int_p->inherited_interface_name != NAMETABLE_NIL_ID)
    {
        if ((int)cmd_val[opt_lang] != opt_lang_cxx)
	    CHECKER_error(int_p, NIDL_INHERITNOTVAL);
        else if (!NAMETABLE_lookup_local(int_p->inherited_interface_name)) {
