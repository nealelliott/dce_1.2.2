 ...
 * HISTORY
 * $Log: ernodtbl.c,v $
 * Revision 1.1.12.2  1996/02/18  18:54:10  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  18:06:46  marty]
 *
 * Revision 1.1.12.1  1995/12/07  22:27:45  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/dat_xidl2/1  1995/11/17  17:06 UTC  dat
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/12/07  21:15:06  root]
 * 
 * Revision 1.1.2.1  1995/10/23  01:49:03  bfc
 * 	oct 95 idl drop
 * 	[1995/10/22  23:35:53  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/22  22:57:02  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE R1.1
 * 	[1995/10/21  18:25:23  bfc]
 * 
 * Revision 1.1.8.1  1994/08/09  15:32:45  rico
 * 	Added volatile to the argument rpc_ss_node_table_t * in definition of
 * 	function rpc_ss_init_node_table().
 * 	[1994/08/09  15:27:08  rico]
 * 
 * Revision 1.1.6.2  1993/07/07  20:07:24  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:37:14  ganni]
 * 
 * $EndLog$
 */
/*
...
#else
#   define  DTOPEN          ((void)0) /* DTOPEN */
#   define  DTRACE(ARGS)    ((void)0) /* DTRACE */
#endif
...
#endif
{
    ...
    *currently_mapped = *currently_mapped * rpc_ss_node_array_size;
    ...
    DTRACE((
        trace_fid, "Expanding: array=0x%X depth=%d, mapped=%d\n",
...
#endif
{
    ...
    */
    ...
    DTRACE((
        trace_fid, "Register(0x%x): num=%d, ptr=0x%X\n", str, num, ptr
    ));
...
(
    ...
    unsigned long       num,
    long                size,
    rpc_void_p_t        (IDL_ALLOC_ENTRY *p_allocate)(idl_size_t size),
    long                *has_been_unmarshalled,
    long                *new_node               /* NULL or addr of return flag */
)
#else
(IDL_msp, tab, num, size, p_allocate, has_been_unmarshalled, new_node)
    IDL_msp_t		IDL_msp;
    rpc_ss_node_table_t tab;
    unsigned long       num;
    long                size;
