 ...
 * HISTORY
 * $Log: sscmaset.c,v $
 * Revision 1.1.16.2  1996/02/17  23:58:38  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  22:52:49  marty]
 *
 * Revision 1.1.16.1  1995/12/07  22:32:35  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/dat_xidl2/1  1995/11/17  17:08 UTC  dat
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/12/07  21:16:46  root]
 * 
 * Revision 1.1.2.1  1995/10/23  01:49:44  bfc
 * 	oct 95 idl drop
 * 	[1995/10/22  23:36:27  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/22  22:58:15  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE R1.1
 * 	[1995/10/21  18:26:07  bfc]
 * 
 * Revision 1.1.12.3  1994/08/23  20:19:08  tom
 * 	Add rpc_x_ss_codeset_conv_error exception (OT 10410).
 * 	[1994/08/23  20:17:21  tom]
 * 
 * Revision 1.1.12.2  1994/07/15  20:24:22  tom
 * 	Bug 10872 - cast 512 to size_t for fread.
 * 	[1994/07/15  20:24:08  tom]
 * 
 * Revision 1.1.12.1  1994/02/18  13:10:20  rico
 * 	Fix for potential infinite loop when stub tries to unmarshall data that has
 * 	not been sent - CR 9391.
 * 	[1994/02/17  12:46:45  rico]
 * 
 * Revision 1.1.8.3  1993/08/26  18:01:08  tatsu_s
 * 	Bug 8073 - Nullify the buff_dealloc pointer after its use.
 * 	[1993/08/24  16:44:55  tatsu_s]
 * 
 * Revision 1.1.8.2  1993/07/07  20:11:17  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:39:43  ganni]
 * 
 * $EndLog$
 */
/*
...
**  26-Feb-93 A.I.Hinxman  User exceptions
**  11-Feb-93 markar/hinxman  OT5828 - change remote comm failure handling
**  21-Jan-93 harrow	   Changes for building in the KERNEL
**  04-Jan-93 A.I.Hinxman  Add exceptions for IDL encoding services
**  10-Nov-92 hinxman      OT5214 - don't free received buffer with zero data length
**  30-Dec-91 harrow       Add ability to map DCE status to/from local status format
*/
/*
**
...
FILE *fid;
    ...
    /*
     * The kernel libnidl only supports the statically defined
     * translation tables.
     */
    ndr_g_ascii_to_ebcdic = (rpc_trans_tab_p_t) &ndr_g_def_ascii_to_ebcdic[0];
    ndr_g_ebcdic_to_ascii = (rpc_trans_tab_p_t) &ndr_g_def_ebcdic_to_ascii[0];
...
globaldef EXCEPTION rpc_x_cant_create_socket;
...
globaldef EXCEPTION rpc_x_no_client_stub;
globaldef EXCEPTION rpc_x_no_server_stub;
...
)
{
    ...
    EXCEPTION_INIT(rpc_x_cant_create_socket);
    exc_set_status(&rpc_x_cant_create_socket,rpc_s_cant_create_socket);
    ...
    EXCEPTION_INIT(rpc_x_ss_codeset_conv_error);
    exc_set_status(&rpc_x_ss_codeset_conv_error,rpc_s_ss_codeset_conv_error);
    EXCEPTION_INIT(rpc_x_no_client_stub);
    exc_set_status(&rpc_x_no_client_stub,rpc_s_fault_no_client_stub);
    EXCEPTION_INIT(rpc_x_no_server_stub);
    exc_set_status(&rpc_x_no_server_stub,rpc_s_no_server_stub);
...
#endif
{
    unsigned long  facility_and_comp_code = 0;
...
#endif
{
	...
	case nca_s_fault_no_client_stub:
            p_exception = &rpc_x_no_client_stub;
            break;
        default:
...
#endif
{
    switch (fault_code) {
        ...
        case nca_s_fault_no_client_stub:
            return( rpc_s_fault_no_client_stub );
        default:
...
#endif
{
	...
	case rpc_s_cant_create_socket:
            p_exception = &rpc_x_cant_create_socket;
            break;
        ...
        case rpc_s_unknown_stub_rtl_if_vers:
            ...
            break;
        case rpc_s_ss_codeset_conv_error:
            p_exception = &rpc_x_ss_codeset_conv_error;
            break;
        case rpc_s_fault_no_client_stub:
            p_exception = &rpc_x_no_client_stub;
            break;
        case rpc_s_no_server_stub:
            p_exception = &rpc_x_no_server_stub;
            break;
        default:
            {
