...
/*
 * HISTORY 
 * $Log: acct_repl_all.c,v $
 * Revision 1.1.11.3  1996/02/17  23:25:07  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:43:19  marty]
 *
 * Revision 1.1.11.2  1995/12/11  20:20:20  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  19:51:56  root]
 * 
 * Revision 1.1.7.1  1994/10/25  17:17:57  sekhar
 * 	[ OT 12398 ]  Checking in Sean Mullan's changes for testing
 * 		      security replication.
 * 	[1994/10/24  17:00:07  sekhar]
 * 
 * Revision 1.1.4.1  1993/10/07  18:59:03  mccann
 * 	CR9057 remove unnecessary verison requirement
 * 	[1993/10/07  18:57:42  mccann]
 * 
 * Revision 1.1.2.4  1993/02/25  22:04:38  cuti
 * 	Change sec_rgy_site_open() to sec_rgy_site_open_update() for using site update operation.
 * 	[1993/02/25  16:29:25  cuti]
 * 
 * Revision 1.1.2.3  1993/01/29  21:10:53  cuti
 * 	Minro type
 * 	[1993/01/28  16:29:33  cuti]
 * 
 * 	Minor typo
 * 	[1993/01/28  16:17:23  cuti]
 * 
 * 	Change the code, so it exercise sec_rgy_acct_replace_all().
 * 	[1993/01/28  16:01:51  cuti]
 * 
 * Revision 1.1.2.2  1992/12/31  17:27:52  htf
 * 	Embedding copyright notice
 * 	[1992/12/31  16:53:15  htf]
 * 
 * Revision 1.1  1992/01/19  04:07:33  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
 ...
 *	from delta on	11/13/91 14:17:38
 *
 *  $LOG$
 *
 *
...
#ifndef LINT
static char sccs_id[] = "src/test/security/api/moretests/acct_repl_all.c, , dce_osf, dce.75d";
#endif
...
/* Local variables */
   ...
   sec_rgy_acct_admin_t	repl_admin_part;/* sec_rgy_acct_replace_all */
   sec_rgy_acct_user_t	repl_user_part; /* sec_rgy_acct_replace_all */
   sec_rgy_login_name_t *test_login_name;/* sec_rgy_acct_replace_all */
   sec_passwd_rec_t	chg_key;	/* sec_rgy_acct_replace_all */
   boolean32		set_passwd;	/* sec_rgy_acct_replace_all */
...
/* Add an additional test org item to the registry for the test of
   ...
   if (subcase == ACCT_REPL_ALL_UNIQUE_KEY_PARTS)
     {
...
/* Setup the replacement admin_part for the rpc under test. */

   bzero ((char *)&repl_admin_part, sizeof (repl_admin_part));
   ...
   repl_admin_part.creation_date = time(NULL) + 10;
   repl_admin_part.change_date = time(NULL) + 10;
   repl_admin_part.expiration_date = time(NULL) + 20;
   repl_admin_part.good_since_date = time(NULL) + 10;
   repl_admin_part.flags = sec_rgy_acct_admin_audit;
   repl_admin_part.authentication_flags = sec_rgy_acct_auth_post_dated;

/* Call the rpc under test. */

   if (subcase == ACCT_REPL_ALL_NOT_FOUND)
   ...
   sec_rgy_acct_replace_all (write_context,
			     test_login_name,
			     &key_parts,
			     &repl_user_part,
			     &repl_admin_part,
			     ...
			     &tstatus);
   TSHMessage (ctx, MOTRACE,
               "sec_rgy_acct_replace_all (test) status: %d\n", tstatus);
   ...
   if (estatus == tstatus)
       ...
       if (subcase != ACCT_REPL_ALL_NOT_FOUND)
	 {
        ...
        /* Even if tstatus came back with the expected return on the 
           acct_replace_all above, check to see if the account was 
           ...
           TSHMessage (ctx, MOTRACE,
                       "sec_rgy_acct_lookup (after replace_all) status: %d\n", tstatus);

        /* If the lookup failed or if the admin_part returned from the lookup
           did not match the admin_part supplied as input to replace_all,
           then signal a test failure. Check only the server insensitive fields */
           ...
           if ((tstatus != rpc_s_ok)
               ...
               || (repl_admin_part.expiration_date != lkp_admin_part.expiration_date)
               || (repl_admin_part.good_since_date != lkp_admin_part.good_since_date)
               || (repl_admin_part.flags != lkp_admin_part.flags)
               || (repl_admin_part.authentication_flags != lkp_admin_part.authentication_flags))
	     {
	       TSHMessage (ctx, MOFAILURE, "sec_rgy_acct_lookup failed\n");
	       if (tstatus == rpc_s_ok)
	         tstatus = TSH_TEST_FAILED;
    ...
    /* If this was the test of the non-unique key_parts replacement and
	 ...
	 {
	   TSHMessage (ctx, MOFAILURE, "sec_rgy_acct_replace_all failed\n");
	   tstatus = TSH_TEST_FAILED;
         }
     }
    ...
    TSHMessage (ctx, MOFAILURE,
                  "sec_rgy_acct_replace_all failed\n");
