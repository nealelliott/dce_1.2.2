...
/*
 * HISTORY
 * $Log: unix_getgrgid.c,v $
 * Revision 1.1.7.2  1996/02/17  23:25:54  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:43:56  marty]
 *
 * Revision 1.1.7.1  1995/12/11  21:50:30  root
 ...
 * 	merged forward
 * 	[1995/12/11  20:53:12  root]
 * 
 * Revision 1.1.5.2  1993/07/09  10:44:22  root
 * 	Initial King Kong branch
 * 	[1993/07/09  10:43:41  root]
 * 
 * Revision 1.1.3.2  1993/06/11  17:51:23  mob
 * 	Initialize group_cmp.members
 * 	[1993/06/10  17:57:34  mob]
 * 
 * Revision 1.1.1.8  1993/05/04  22:17:07  mgm
 * 	    Loading drop DCE1_0_2FNL
 * 
 * Revision 1.1.5.3  1993/02/25  22:08:47  cuti
 * 	Change sec_rgy_site_open() to sec_rgy_site_open_update() for using site update operation.
 * 	[1993/02/25  16:34:11  cuti]
 * 
 * Revision 1.1.5.2  1992/12/31  17:34:20  htf
 * 	Embedding copyright notice
 * 	[1992/12/31  16:59:03  htf]
 * 
 * Revision 1.1.3.2  1992/06/02  21:22:29  sekhar
 * 	Drop Unix interface changes made by frisco.
 * 	[1992/06/02  20:15:09  sekhar]
 * 
 * Revision 1.1.1.2  1992/05/22  20:07:05  frisco
 * 	Initial revision - getgrid test
 * 
 * $EndLog$
 */
/* 
...
#ifndef LINT
static char sccs_id[] = "src/test/security/api/moretests/unix_getgrid.c";
#endif
...
/* Call the unix interface under test. */
   ...
   if (TSHStatus (ctx, SETUP,
                  "sec_rgy_unix_getgrgid failed\n", rpc_s_ok, tstatus))
     goto EXIT_DEL_ORG_MEMBER;

/* If the lookup fails or if the data supplied as input to pgo_add does
   not match the data returned as output from the getgrid, then treat
   the getgrid as if it had failed. */
     ...
     if (strcmp(group_cmp.name, 
       ...
       || (group_cmp.gid != group_entry.gid)) {
         ...
         TSHStatus (ctx, TEST, "sec_rgy_unix_getgrid failed - entry mismatch\n",
                  tstatus, rpc_s_ok);
    }
...
/* check membership list */
   ...
   for (i = 0; i < NUM_PERSONS; i++) {
       ...
       if (strcmp(members[i], person_name)) {
           ...
           TSHStatus (ctx, TEST, 
               "sec_rgy_unix_getgrid failed - member list mismatch\n",
                tstatus, rpc_s_ok);
           break;
       }
   }
