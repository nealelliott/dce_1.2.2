...
# HISTORY
# $Log: uu_setup.ksh,v $
# Revision 1.1.2.2  1996/07/09  15:02:58  arvind
# 	OSF DCE 1.2.2 Drop 3
# 	[1996/07/09  14:49:34  arvind]
#
# Revision 1.1.2.1  1996/06/04  22:10:54  arvind
# 	DCE1.2.2 Drop 2 to OSF
# 	[1996/06/04  21:30:12  arvind]
# 
# $EndLog$
...
then
      echo "KRB5_UU_SERVER_HOST not set in environment"
      exit -1
fi
...
then
      echo "KRB5_UU_SERVICE not set in environment"
      exit -1
fi

if [ -z "${KRB5_UU_PW}" ]
then
      echo "KRB5_UU_PW not set in environment"
      exit -1
fi

test_principal=${KRB5_UU_SERVICE}/${fully_qual_host}
test_passwd=${KRB5_UU_PW}
...
add ${test_principal}
domain account
add ${test_principal} -g none -o none -enable dup -pw ${test_passwd} -mp ${CELL_ADMIN_PW}
...
rm -f /tmp/uu.env
echo "KRB5CCNAME=${KRB5CCNAME}" > /tmp/uu.env
