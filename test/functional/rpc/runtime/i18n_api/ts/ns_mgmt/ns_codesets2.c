...
/*
 * HISTORY
 * $Log: ns_codesets2.c,v $
 * Revision 1.1.4.2  1996/02/18  00:34:05  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  23:19:19  marty]
 *
 * Revision 1.1.4.1  1995/12/11  19:51:56  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  19:26:36  root]
 * 
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  18:18:00  root]
 * 
 * Revision 1.1.2.3  1994/07/28  18:30:37  mori_m
 * 	CR 11294  Changed attr_type to a uuid_t* for codesets attribute ops.
 * 	[1994/07/28  18:26:06  mori_m]
 * 
 * Revision 1.1.2.2  1994/06/10  20:44:26  devsrc
 * 	cr10872 - fixed copyright
 * 	[1994/06/10  17:16:21  devsrc]
 * 
 * Revision 1.1.2.1  1994/05/04  21:54:52  mori_m
 * 	CR 9701: RPC runtime I18N extension functional test drop.
 * 	[1994/05/04  21:18:32  mori_m]
 * 
 * $EndLog$
 */
...
tp1()
{
	...
	tet_infoline("test case (ns_mgmt_2)");
	...
	{
		mtet_infoline("First time) out_arr->version - found %d - expecting %d\n",
	...
	{
		mtet_infoline("First time) out_arr->count - found %d - expecting %d\n",
	...
	for (i = 0; i < out_arr->count; i++)
	{
		...
		{
			mtet_infoline("First time) out_arr->codesets[%d].c_set - found %d - expecting %d\n",
		...
		{
			mtet_infoline("First time) out_arr->codesets[%d].c_max_bytes - found %d - expecting %d\n",
	...
	if (out_arr->version != arr->version)
	{
		mtet_infoline("Second time) out_arr->version - found %d - expecting %d\n",
			i, out_arr->version, arr->version);
		failed = TRUE;
	}
	...
	if (out_arr->count != arr->count)
	{
		mtet_infoline("Second time) out_arr->count - found %d - expecting %d\n",
			out_arr->count, arr->count);
		failed = TRUE;
	}
	...
	for (i = 0; i < out_arr->count; i++)
	{
		if (out_arr->codesets[i].c_set != arr->codesets[i].c_set)
		{
			mtet_infoline("Second time) out_arr->codesets[%d].c_set - found %d - expecting %d\n",
				i, out_arr->codesets[i].c_set, arr->codesets[i].c_set);
			failed = TRUE;
		}
		...
		if (out_arr->codesets[i].c_max_bytes != arr->codesets[i].c_max_bytes)
		{
			mtet_infoline("Second time) out_arr->codesets[%d].c_max_bytes - found %d - expecting %d\n",
				i, out_arr->codesets[i].c_max_bytes, arr->codesets[i].c_max_bytes);
			failed = TRUE;
		}
	...
	if (out_arr->version != arr->version)
	{
		mtet_infoline("Third time) out_arr->version - found %d - expecting %d\n",
			i, out_arr->version, arr->version);
		failed = TRUE;
	}
	...
	if (out_arr->count != arr->count)
	{
		mtet_infoline("Third time) out_arr->count - found %d - expecting %d\n",
			out_arr->count, arr->count);
		failed = TRUE;
	}
	...
	for (i = 0; i < out_arr->count; i++)
	{
		if (out_arr->codesets[i].c_set != arr->codesets[i].c_set)
		{
			mtet_infoline("Third time) out_arr->codesets[%d].c_set - found %d - expecting %d\n",
				i, out_arr->codesets[i].c_set, arr->codesets[i].c_set);
			failed = TRUE;
		}
	...
	rpc_ns_mgmt_free_codesets (&arr, &st);
	CHECK_STATUS(TRUE, "rpc_ns_mgmt_free_codeset", st, rpc_s_ok);
