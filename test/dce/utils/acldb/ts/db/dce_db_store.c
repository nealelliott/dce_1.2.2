...
/*
 * HISTORY
 * $Log: dce_db_store.c,v $
 * Revision 1.1.3.2  1996/02/18  00:23:48  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  23:11:14  marty]
 *
 * Revision 1.1.3.1  1995/12/08  22:07:53  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/08  21:43:08  root]
 * 
 * Revision 1.1.1.4  1994/06/10  20:18:47  devsrc
 * 	cr10872 - fixed copyright
 * 	[1994/06/10  16:58:08  devsrc]
 * 
 * Revision 1.1.1.3  1994/04/07  19:22:15  baum
 * 	Checkin again in another set
 * 	[1994/04/07  19:21:57  baum]
 * 
 * Revision 1.1.1.2  1994/04/05  01:22:39  baum
 * 	Initial checkin
 * 
 * $EndLog$
 */
...
/*
 * Function: 	db_store_key_name
 *
 * Abstract:	Add an item to a backing store, indexed by name
 */
void
db_store_key_name ()
{
...
/*
 * Function: 	db_store_key_name_bad_type
 *
 * Abstract:	Attempt to store an item in a backing store of the
 *		wrong index type
 */
void
db_store_key_name_bad_type ()
{

  int		dummy_tet_return;
  dbtest_t	test_data, test_data_out;

  p_test = "db_store_key_name_bad_type";

  /*
   * Setup: Create a DB file, indexed by uuid.  Init the test data.
   */
  ...
  /*
   * Assertion: Add a record with wrong index type.  Failure.
   */
  dce_db_store_by_name ( db_h, DB_KEY1, &test_data, &dce_status );
  CHECK_EXPECTED_STATUS( "dce_db_store_by_name", dce_status, 
			 db_s_bad_index_type );
...
/*
 * Function: 	db_store_key_uuid_bad_type
 *
 * Abstract:	Attempt to store an item in a backing store of the
 *		wrong index type
 */
void
db_store_key_uuid_bad_type ()
{
  ...
  p_test = "db_store_key_uuid_bad_type";
   ...
   * Assertion: Add a record with wrong index type.  Failure.
   */
  dce_db_store_by_uuid ( db_h, &uuid, &test_data, &dce_status );
  CHECK_EXPECTED_STATUS( "dce_db_store_by_uuid", dce_status, 
			 db_s_bad_index_type );
...
{
    { db_store_key_name,  1 },
    { db_store_key_name_bad_type,  1 },
    { db_store_key_uuid_bad_type,  1 },
    { (void (*)())NULL, 0 }
};
#endif
