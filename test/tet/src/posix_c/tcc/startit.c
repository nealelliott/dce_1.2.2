...
MODIFICATIONS:
	       ...
	       Alterted analyze_jnl_entry() to look for Abort result codes.
	       tpend() no longer looks for abort result codes.
	       David G. Sawyer, UniSoft Ltd, 08 Apr 1992.

	       Added message from tcc for when a TC with output capture set
	       gives an exit code that maps to a result code with an action
	       of Abort.
	       David G. Sawyer, UniSoft Ltd, 08 Apr 1992.
...
static char zeetime[18];          /* Big enough to hold "HH:MM:YY YYYYMMDD" */
    ...
    else
    {
                ...
                while ( fgets(linein,sizeof(linein),fp) != NULL)
		{
                        ...
                        else
			{
			    ...
			    /* Check for an Abort */
                            if (*rescode_num_to_action(context_status) == 'A')
	                    {
                                abort_requested = TRUE;
#ifdef DBG2
		                (void) fprintf(stderr,"Found ABORT result code !\n");
#endif
	    		    }
			}
		...
		/*
		 *  Put a message into journal file from the TCC saying a 
		 *  result code with an action of Abort has been found.
		 */
...
#ifdef DBG
    (void) fprintf(stderr,"copy_results_file(res_fp,mode : %d)\n", what_mode);
#endif
