 ...
 * HISTORY
 * $Log: irepgen.c,v $
 * Revision 1.1.8.2  1996/02/18  18:52:05  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  18:05:29  marty]
 *
 * Revision 1.1.8.1  1995/12/07  22:19:15  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/dat_xidl2/1  1995/11/17  17:04 UTC  dat
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/12/07  21:12:05  root]
 * 
 * Revision 1.1.2.1  1995/10/23  01:48:17  bfc
 * 	oct 95 idl drop
 * 	[1995/10/23  00:43:59  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/21  22:57:46  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE 1.1
 * 	[1995/10/21  17:24:42  bfc]
 * 
 * Revision 1.1.4.1  1994/06/23  18:25:14  tom
 * 	From DEC: add opaque context handle support.
 * 	[1994/06/23  18:24:00  tom]
 * 
 * Revision 1.1.2.2  1993/07/07  20:00:08  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:32:55  ganni]
 * 
 * $EndLog$
 */
/*
...
#endif
{
    ...
    IR_tup_n_t          *cs_tup_p;  /* Ptr to codeset shadow tuple */
    IR_tup_n_t          *pred_tup_p;/* Saved tuple pointer */
    IR_tup_n_t          *info_tup_p;/* Ptr to conformance info tuple */
    IR_tup_n_t          *last_tup_p;/* Ptr to tup before last field tuples */
    AST_structure_n_t   *struct_p;  /* Ptr to AST structure node */
    AST_field_n_t       *field_p;   /* Ptr to AST field node */
    AST_field_n_t   *last_field_p;  /* Ptr to last field in struct */
...
#endif
{
         ...
         atype_p = atype_p->type_structure.array->element_type)
    {
        ...
        IR_gen_bound_tups(ctx_p, atype_p, ((atype_p == type_p) ? inst_p : NULL),
            (IR_flags_t)((AST_STRING_SET(atype_p) || AST_STRING0_SET(atype_p)) ?
            IR_STRING : 0));
    }
    ...
    if (inst_p != NULL && AST_VARYING_SET(inst_p))
    {
             ...
             atype_p = atype_p->type_structure.array->element_type)
        {
            ...
            IR_gen_limit_tups(ctx_p, atype_p,
                (atype_p == type_p) ? inst_p : NULL, (IR_flags_t)
                ((AST_STRING_SET(atype_p) || AST_STRING0_SET(atype_p)) ? 
                IR_STRING : 0) );
        }
    }
...
#endif
{
    ...
    IR_tup_n_t *full_tup_p, *flat_tup_p; /* Saved tuple ptrs */
...
#endif
{
    ...
    new_index_p = 
         AST_array_index_node((unsigned short)(array_p->index_count-1));
...
#endif
{
    AST_type_n_t    *ptee_type_p;   /* Pointee type of pointer node */
    ...
    case AST_pointer_k:
         ...
         * Also test for object references, which aren't normal pointers.
         */
        ptee_type_p = type_p->type_structure.pointer->pointee_type;
        if (ptee_type_p->kind == AST_void_k)
        {
            if (inst_p == NULL || !AST_CONTEXT_SET(inst_p))
            {
                ...
                IR_gen_context_rep(ctx_p, type_p, (AST_parameter_n_t *)inst_p);
                 ...
                 && ptee_type_p->kind == AST_structure_k)
            IR_gen_context_rep(ctx_p, type_p, (AST_parameter_n_t *)inst_p);
        else if (ptee_type_p->kind == AST_interface_k)
            IR_gen_interface_rep(ctx_p, ptee_type_p, inst_p);
        else
    ...
    default:
        INTERNAL_ERROR("Unexpected type kind in IR_gen_type_rep");
    }
}
