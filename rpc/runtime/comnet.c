 ...
 * HISTORY
 * $Log: comnet.c,v $
 * Revision 1.1.88.2  1996/02/18  00:02:43  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  22:55:22  marty]
 *
 * Revision 1.1.88.1  1995/12/08  00:18:27  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/HPDCE02/jrr_1.2_mothra/1  1995/11/16  21:31 UTC  jrr
 * 	Remove Mothra specific code
 * 
 * 	HP revision /main/HPDCE02/6  1995/07/26  19:02 UTC  tatsu_s
 * 	Submitted the fix for CHFts15710.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.local_rpc.b5/1  1995/07/25  21:00 UTC  tatsu_s
 * 	Fixed rpc_server_listen()'s exception handling.
 * 
 * 	HP revision /main/HPDCE02/5  1995/05/25  21:40 UTC  lmm
 ...
 * 
 * 	HP revision /main/HPDCE02/4  1995/04/02  01:14 UTC  lmm
 ...
 * 
 * 	HP revision /main/HPDCE02/3  1995/01/16  19:17 UTC  markar
 * 	Merging Local RPC mods
 * 
 * 	HP revision /main/HPDCE02/markar_local_rpc/1  1995/01/05  20:25 UTC  markar
 * 	Implementing Local RPC bypass mechanism
 * 
 * 	HP revision /main/HPDCE02/2  1994/12/09  19:18 UTC  tatsu_s
 * 	Submitted rfc31.0: Single-threaded DG client and RPC_PREFERRED_PROTSEQ.
 * 
 * 	HP revision /main/HPDCE02/tatsu_s.st_dg.b0/1  1994/12/05  18:53 UTC  tatsu_s
 * 	RPC_PREFERRED_PROTSEQ: Initial version.
 * 
 * 	HP revision /main/HPDCE02/1  1994/08/03  16:35 UTC  tatsu_s
 * 	Merged mothra up to dce 1.1.
 * 
 * 	HP revision /main/HPDCE01/1  1994/03/08  23:47 UTC  rmm
 * 	merge loopback changes in.
 * 
 * 	HP revision /main/mk_LOOPBACK_IC1/1  1994/03/08  00:53 UTC  mk
 * 	add loopback support for sd
 * 	[1995/12/07  23:58:16  root]
 * 
 * Revision 1.1.86.2  1994/07/15  16:56:13  tatsu_s
 * 	Fix OT10589: Reversed the execution order of fork_handlers.
 * 	[1994/07/09  02:42:41  tatsu_s]
 * 
 * Revision 1.1.86.1  1994/01/21  22:35:34  cbrooks
 * 	RPC Code Cleanyp - Initial Submission
 * 	[1994/01/21  21:55:36  cbrooks]
 * 
 * Revision 1.1.84.1  1993/09/15  15:39:48  damon
 * 	Synched with ODE 2.1 based build
 * 	[1993/09/15  15:32:02  damon]
 * 
 * Revision 1.1.9.2  1993/09/14  16:49:04  tatsu_s
 * 	Bug 8103 - rpc__nlsn_fork_handler() is now called from within
 * 	rpc__network_fork_handler().
 * 	[1993/09/13  17:08:41  tatsu_s]
 * 
 * Revision 1.1.7.2  1993/08/26  19:15:47  tatsu_s
 * 	Bug 1709 - If addr_vector is empty, return rpc_s_no_addrs status
 * 	in rpc__network_inq_local_addr().
 * 	[1993/08/24  15:55:01  tatsu_s]
 * 
 * Revision 1.1.4.4  1993/01/03  23:22:25  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/03  20:02:20  bbelch]
 * 
 * Revision 1.1.4.3  1992/12/23  20:44:24  zeliff
 * 	Embedding copyright notice
 * 	[1992/12/23  15:34:04  zeliff]
 * 
 * Revision 1.1.4.2  1992/10/02  14:29:53  markar
 * 	 OT 4382 fix: fixed error status in rpc__server_stop_listening()
 * 	[1992/09/22  17:18:41  markar]
 * 
 * Revision 1.1.2.3  1992/06/04  15:58:46  mishkin
 * 	- Unlock mutex on error return path in rpc_server_listen() (CR 3857).
 * 	- Unlock mutex on error return path in rpc__server_stop_listening() (CR 3774).
 * 	[1992/06/03  17:13:26  mishkin]
 * 
 * Revision 1.1.2.2  1992/05/21  13:15:22  mishkin
 * 	Raise debug level on a debug printf.
 * 	[1992/05/20  21:09:48  mishkin]
 * 
 * Revision 1.1  1992/01/19  03:12:45  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
...
#endif
{
    int                     nd_index;       /* network info table index    */
    int                     bv_index;       /* binding vector index        */
    int                     av_index;       /* RPC Address vector index    */
...
#endif
{
    ...
    for (i = 0; i < listener_state.high_water; i++)
    {
		...
		if (*status != rpc_s_ok)
		{
		    ...
		    for (; i >= 0; i--)
		    {
			lsock = &listener_state.socks[i];
			if (lsock->busy && lsock->is_server
			    && lsock->is_active)
			{
			    rpc__nlsn_deactivate_desc (&listener_state,
						       i, &lst);
			}
		    }
