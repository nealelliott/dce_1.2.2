 ...
 * HISTORY
 * $Log: acct_passwd.c,v $
 * Revision 1.1.11.3  1996/02/17  23:25:04  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:43:16  marty]
 *
 * Revision 1.1.11.2  1995/12/11  20:20:17  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  19:51:53  root]
 * 
 * Revision 1.1.7.1  1994/10/25  17:17:54  sekhar
 * 	[ OT 12398 ] call sec_rgy_cursor_reset() before account lookup.
 * 	[1994/10/24  18:12:07  sekhar]
 * 
 * 	[ OT 12398 ]  Checking in Sean Mullan's changes for testing
 * 		      security replication.
 * 	[1994/10/24  16:59:38  sekhar]
 * 
 * Revision 1.1.4.1  1993/10/07  18:58:53  mccann
 * 	CR9057 remove unnecessary verison requirement
 * 	[1993/10/07  18:56:52  mccann]
 * 
 * Revision 1.1.2.3  1993/02/25  22:04:05  cuti
 * 	Change sec_rgy_site_open() to sec_rgy_site_open_update() for using site update operation.
 * 	[1993/02/25  16:28:34  cuti]
 * 
 * Revision 1.1.2.2  1992/12/31  17:27:36  htf
 * 	Embedding copyright notice
 * 	[1992/12/31  16:53:02  htf]
 * 
 * Revision 1.1  1992/01/19  04:07:30  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
 ...
 *	from delta on	11/13/91 14:17:25
 *
 *  $LOG$
 *
 *
...
#ifndef LINT
static char sccs_id[] = "src/test/security/api/moretests/acct_passwd.c, , dce_osf, dce.75d";
#endif
...
/* Local variables */
   ...
   sec_passwd_rec_t	chg_key;	/* sec_rgy_acct_passwd */
...
/* Add the test account for the normal case.  For the case
   of changing a password for an acccount that does not
   exist, skip adding the account. */

   if (subcase != ACCT_PASSWD_NOT_FOUND)
     {
    ...
    /* Verify that the account was actually added. */
    ...
    /* If the lookup fails or if the data supplied as input to acct_add does
       ...
       the acct_lookup as if it had failed. Check only the server insensitive
       fields */
...
/* Call the rpc under test. */
   ...
   sec_rgy_acct_passwd (write_context,
                        &ACCT_LOGIN_NAME,
		        &caller_key,
		        &chg_key,
                        sec_passwd_des,
...
/* Verify that the password for the specified account has been changed unless
   this is the account (object) not found test case. */

   if ((tstatus == rpc_s_ok) &&
       (subcase != ACCT_PASSWD_NOT_FOUND))
     {

       /* wait for propagation to any replicas */
       SEC_RGY_SITE_SYNCH(write_context);

    /* Lookup the account to obtain the data necessary to verify the change. */
       ...
       TSHMessage (ctx, MOTRACE,
                   "sec_rgy_acct_lookup (after passwd) status: %d\n", tstatus);

    /* If the lookup succeeded, check to see whether the passwd was actually
       changed. */

       if (tstatus == rpc_s_ok)
         if (memcmp ((char *)new_user_part.passwd,
                     (char *)lkp_user_part.passwd,
                     sizeof (sec_rgy_unix_passwd_buf_t) == 0))
	   {
             TSHMessage (ctx, MOFAILURE, "passwd not changed\n");
	     tstatus = TSH_TEST_FAILED;
           }
   ...
   if (subcase != ACCT_PASSWD_NOT_FOUND)
     {
       ...
       if (TSHStatus (ctx, CLNUP,
        ...
        /* On the acct_lookup after the acct_delete, we expect that the lookup
           ...
           TSHMessage (ctx, MOTRACE,
                       "sec_rgy_acct_lookup (verify cleanup delete) status: %d\n", cstatus);
         }
     }
