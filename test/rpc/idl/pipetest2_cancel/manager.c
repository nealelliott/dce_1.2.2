 ...
 * HISTORY
 * $Log: manager.c,v $
 * Revision 1.1.6.2  1996/02/17  23:16:23  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:35:47  marty]
 *
 * Revision 1.1.6.1  1995/12/11  19:27:42  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  19:14:52  root]
 * 
 * Revision 1.1.4.1  1993/10/14  17:52:43  ganni
 * 	OT 7426 - malloc the buffers to avoid the alignment problems.
 * 	[1993/10/14  17:51:58  ganni]
 * 
 * Revision 1.1.2.3  1993/01/13  21:50:05  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/06  13:39:49  bbelch]
 * 
 * Revision 1.1  1992/01/19  03:33:02  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
...
TRY
    *correct_to_server = true;
    ...
    {
#ifdef DEBUG
        printf("Wait 20.0 seconds...\n");
#endif
        cma_delay(20.0);  /* Slow things down to make sure we get the cancel */
        for (i=0; i<data_count; i++)
        {
    ...
    printf("Out of PUSH loop without cancel, wait some more...\n");
    cma_delay(100.0);  /* Slow things down to make sure we get the cancel */
...
CATCH (cma_e_alerted)
#ifdef DEBUG
    printf("***Success***: Server operation alerted during pipe as expected!\n");
    fflush(stdout);
#endif
    RERAISE; /* Pass the canel upward */
CATCH_ALL
    printf("***ERROR***: Wrong signal received on the manager\n");
    print_exception(THIS_CATCH);
    fflush(stdout);
    RERAISE;
ENDTRY

printf("***ERROR***: Cancel signal not received on the manager\n");
fflush(stdout);
}

globaldef pipetest2_cancel_v0_0_epv_t pipetest2_cancel_v0_0_m_epv = {his_oper2};
