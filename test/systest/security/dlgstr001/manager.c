 ...
 * HISTORY
 * $Log: manager.c,v $
 * Revision 1.1.4.2  1996/02/17  23:28:29  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:45:28  marty]
 *
 * Revision 1.1.4.1  1995/12/11  22:45:35  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  22:21:39  root]
 * 
 * Revision 1.1.2.2  1994/10/06  20:43:13  agd
 * 	expand copyright
 * 	[1994/10/06  14:31:52  agd]
 * 
 * Revision 1.1.2.1  1994/09/19  18:17:37  bhaim
 * 	multidelegate test
 * 	[1994/09/19  17:50:54  bhaim]
 * 
 * $EndLog$
 */
/*
...
#define IFSPEC_IN         string_conv_v1_0_s_ifspec
#define IFSPEC_OUT        string_conv2_v1_0_s_ifspec
#define INTERFACE_NUMBER  conv_1_interface_number
...
/******/
    ...
    if (BAD_STATUS(st_p))  {
        dce_error_inq_text(*st_p, dce_err_string, (int *)&ignore);
        PRINT_FUNC(PRINT_HANDLE, "rpc_binding_inq_auth_caller failed: %s\n", dce_err_string);
        return;
    }
    ...
    if (BAD_STATUS(st_p))  {
        dce_error_inq_text(*st_p, dce_err_string, (int *)&ignore);
        PRINT_FUNC(PRINT_HANDLE, "sec_cred_is_authenticated failed: %s\n", dce_err_string);
        return;
    }
    ...
    if (BAD_STATUS(st_p))  {
        dce_error_inq_text(*st_p, dce_err_string, (int *)&ignore);
        PRINT_FUNC(PRINT_HANDLE, "sec_cred_get_initiator failed: %s\n", dce_err_string);
        return;
    }
	...
	pa = (sec_id_pa_t *) sec_cred_get_pa_data(delegate, st_p);
		...
		if (!STATUS_OK(&xst)) {
            ...
            dce_error_inq_text(*st_p, dce_err_string, (int *)&ignore);
            PRINT_FUNC(PRINT_HANDLE, "cannot get principal data: %s\n", dce_err_string);
        }
        ...
        if (init_flag) {	
 	    sec_cred_initialize_attr_cursor(&attr_cursor, st_p);
	        ...
	        printf("\tEnd of Attribute List.\n");
                ...
                printf("Unable to initialize attr cursor - status %d\n", 
                                             *st_p);
        }
...
/******/
   ...
   if (num_requested > INTERFACE_NUMBER)
   {
      ...
      sprintf(cds_entry_name, "/.:/subsys/HP/sample-apps/string-convert2");
      ...
      rpc_binding_set_auth_info (
          newbh,
          (unsigned char *)string_conv2_SERVER_PRINCIPAL_NAME,  /* from real-common.h */
      ...
      PRINT_FUNC(PRINT_HANDLE, "Calling string_convert_uppercase2(%s), servers(%s)\n",
		 conv_string, num_serv);
      
      string_convert_uppercase2(newbh, conv_string, num_serv, auth_status);

      PRINT_FUNC(PRINT_HANDLE, "string_convert_uppercase2 returned \"%s\"\n",
		 conv_string);
       ...
       */
      PRINT_FUNC(PRINT_HANDLE, ">>> string_convert_uppercase(%s), servers(%s)\n",
                 conv_string, num_serv);
