 ...
 * HISTORY
 * $Log: comtwrref.c,v $
 * Revision 1.1.514.2  1996/02/18  00:03:01  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  22:55:36  marty]
 *
 * Revision 1.1.514.1  1995/12/08  00:18:53  root
 ...
 * 	[1995/12/07  23:58:29  root]
 * 
 * Revision 1.1.512.3  1994/06/30  18:36:31  tom
 * 	Bug 10346 - Ignore protocol towers we don't understand by
 * 	not passing return status of rpc__tower_ref_inq_protseq_id back from
 * 	rpc__tower_ref_is_compatible.
 * 	[1994/06/30  18:13:30  tom]
 * 
 * Revision 1.1.512.2  1994/03/17  23:06:45  tom
 * 	Big PAC/protocol version support:
 * 	  Don't reject a CN tower binding based on its minor version.
 * 	  Pass protocol version arugment to rpc__tower_flr_from_rpc_prot_id().
 * 	[1994/03/17  23:04:38  tom]
 * 
 * Revision 1.1.512.1  1994/01/21  22:36:06  cbrooks
 * 	RPC Code Cleanyp - Initial Submission
 * 	[1994/01/21  21:56:13  cbrooks]
 * 
 * Revision 1.1.4.4  1993/01/30  20:20:30  weisman
 * 	OT 5761 (part II): In error path after failure from
 * 	rpc__naf_tower_flrs_from_addr, call RPC_MEM_FREE on the
 * 	the newly allocated tower_vector before returning to caller.
 * 	[1993/01/30  20:02:42  weisman]
 * 
 * Revision 1.1.4.3  1993/01/03  23:23:07  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/03  20:03:28  bbelch]
 * 
 * Revision 1.1.4.2  1992/12/23  20:45:54  zeliff
 * 	Embedding copyright notice
 * 	[1992/12/23  15:35:13  zeliff]
 * 
 * Revision 1.1.2.2  1992/05/01  17:02:40  rsalz
 * 	  6-mar-92 wh        DCE 1.0.1 merge.
 * 	 06-feb-92 sommerfeld add enough braces to tower_prot_ids to
 * 	                     silence picky compiler.
 * 	[1992/05/01  16:50:17  rsalz]
 * 
 * Revision 1.1  1992/01/19  03:12:47  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
	...
	*status = rpc_s_no_memory;
    ...
    for (i=0; i < source_tower->count; i++)
    {
        RPC_MEM_ALLOC (
            ...
            RPC_C_MEM_TOWER_FLOOR,
            RPC_C_MEM_WAITOK);
	if ((*dest_tower)->floor[i] == NULL){
	    unsigned32	temp_status;

	    (*dest_tower)->count = i;
	    rpc__tower_ref_free(dest_tower, &temp_status);
	    *status = rpc_s_no_memory;
	    return;
	}
...
**      protseq_id      The protocol sequence for the client.
**                      If RPC_C_INVALID_PROTSEQ_ID,
**                      the protocol sequence compatibility check is bypassed.
**      protseq_match   If true, the tower's protocol sequence must match
**                      with the client's protocol sequence.
