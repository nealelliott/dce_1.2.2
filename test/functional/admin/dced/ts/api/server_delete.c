...
/*
 * HISTORY
 * $Log: server_delete.c,v $
 * Revision 1.1.4.2  1996/02/18  00:31:12  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  23:17:06  marty]
 *
 * Revision 1.1.4.1  1995/12/11  15:19:44  root
 * 	Submit
 * 	[1995/12/11  14:33:29  root]
 * 
 * Revision 1.1.2.1  1994/09/28  21:48:18  melman
 * 	moved admin tests to test/functional
 * 	[1994/09/28  21:15:56  melman]
 * 
 * Revision 1.1.2.2  1994/08/10  19:22:48  annie
 * 	expand copyright with OSF copyright text
 * 	[1994/08/10  17:17:08  annie]
 * 
 * Revision 1.1.2.1  1994/07/20  19:59:09  ganni
 * 	update the tests for more validation
 * 	[1994/07/20  19:45:50  ganni]
 * 
 * $EndLog$
 */

/*
** Module: 	server_delete.c
**
...
void setup(void);
void cleanup(void);
void dced_server_delete_1(void);          /* Test purposes 1 */
void dced_server_delete_2(void);          /* Test purposes 2 */
void dced_server_delete_3(void);          /* Test purposes 2 */
...
{
    { dced_server_delete_1,  1 },
    { dced_server_delete_2,  2 },
    { dced_server_delete_3,  2 },
    { NULL, 0 }
};
...
dced_server_delete_1()
{
	...
	tet_infoline("dced_server_delete_1, Assertion 01(A):");
	tet_infoline(
	"When a srvrconf dced binding handle is supplied to ");
	tet_infoline(
	"dced_server_delete along with the server object id,");
	tet_infoline(
	"its entry will be deleted and the status is set to error_status_ok.");
	...
	else
	{
		...
		if (st != dced_s_not_found)
		{
		    ...
		    tet_infoline("Expected dced_s_not_found, but received:");
		    dced_util_printerr(st);
#endif
		    tet_result(TET_FAIL);
		    fail_stat = true;
...
dced_server_delete_2()
{
	...
	tet_infoline("dced_server_delete_2, Assertion 02(A):");
	tet_infoline(
	"When a srvrexec dced binding handle is supplied to ");
	tet_infoline(
	"dced_server_delete, dced_s_bad_binding_status is returned.");
	...
	dced_server_delete( exec_handle, &server_entry.id, &st);
...
dced_server_delete_3()
{
	...
	tet_infoline("dced_server_delete_3, Assertion 02(A):");
	tet_infoline(
	"When a srvrconf dced binding handle is supplied to ");
	tet_infoline(
	"dced_server_delete along with a non-existant server object id,");
	tet_infoline(
	...
	dced_server_delete( conf_handle, &srvr_id, &st);
	if (st != dced_s_not_found)
	{
#ifdef DEBUG
		tet_infoline("dced_server_delete failed:");
