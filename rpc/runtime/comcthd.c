 ...
 * HISTORY
 * $Log: comcthd.c,v $
 * Revision 1.1.82.2  1996/02/18  00:02:25  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  22:55:07  marty]
 *
 * Revision 1.1.82.1  1995/12/08  00:18:06  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/gaz_dce_instr/jrr_1.2_mothra/1  1995/11/16  21:30 UTC  jrr
 * 	Remove Mothra specific code
 * 
 * 	HP revision /main/gaz_dce_instr/9  1995/10/18  01:33 UTC  gaz
 * 	store the right handle for later release
 * 
 * 	HP revision /main/gaz_dce_instr/8  1995/10/16  17:54 UTC  gaz
 * 	initialize dms_tpd at declaration
 * 
 * 	HP revision /main/gaz_dce_instr/7  1995/09/07  20:13 UTC  gaz
 * 	unregister DMS subsystem when freeing a threadpool
 * 	fix threadpool metrics
 * 
 * 	HP revision /main/gaz_dce_instr/6  1995/08/17  04:18 UTC  gaz
 * 	Ensure that DMS is NOT built in the Kernel RPC
 * 
 * 	HP revision /main/gaz_dce_instr/5  1995/08/04  20:41 UTC  gaz
 * 	DMS code cleanup
 * 
 * 	HP revision /main/gaz_dce_instr/4  1995/07/17  20:38 UTC  gaz
 * 	Merge out fix for OT7990
 * 	count queue-full rejects
 * 
 * 	HP revision /main/HPDCE02/1  1995/06/30  21:07 UTC  tatsu_s
 * 	Submitted the workaround for OT7990.
 * 
 * 	HP revision /main/tatsu_s.fix_ot12885.b1/1  1995/06/20  20:40 UTC  tatsu_s
 * 	Workaround for OT7990 (broken server stubs).
 * 
 * 	HP revision /main/gaz_dce_instr/3  1995/07/15  20:16 UTC  gaz
 * 	remove DMS tree creation code to libdms/dmsrpc.c
 * 	move calltag identification code to ensure that queued calls are
 * 	initialized too
 * 
 * 	HP revision /main/gaz_dce_instr/2  1995/07/09  00:45 UTC  gaz
 * 	ensure that calltag carries required identification
 * 
 * 	HP revision /main/gaz_dce_instr/1  1995/07/05  01:46 UTC  gaz
 * 	Add threadpool registration for the DMS
 * 	[1995/12/07  23:58:02  root]
 * 
 * Revision 1.1.80.1  1994/01/21  22:35:10  cbrooks
 * 	RPC Code Cleanyp - Initial Submission
 * 	[1994/01/21  21:55:00  cbrooks]
 * 
 * Revision 1.1.6.6  1993/01/18  20:37:54  markar
 * 	     OT CR 6303 fix: As required by DFS, allow dynamic adjustments to a
 * 	                     thread pool's call queue size.
 * 	[1993/01/18  18:10:20  markar]
 * 
 * Revision 1.1.6.5  1993/01/14  21:35:08  markar
 * 	    OT CR 6841 fix: ifdef'd out unused, unbuildable code.
 * 	[1993/01/14  20:05:33  markar]
 * 
 * Revision 1.1.6.4  1993/01/12  16:21:15  markar
 * 	      Disallow dynamic adjustments of call queue size.
 * 	[1993/01/06  16:19:32  markar]
 * 
 * 	     Fixed problem with setting an already-started pool's call queue size.
 * 	[1992/12/22  15:08:24  markar]
 * 
 * 	     OT CR 6303 fix: allow application to set size of call queues.
 * 	[1992/12/09  19:16:21  markar]
 * 
 * Revision 1.1.6.3  1993/01/03  22:59:57  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/03  20:01:27  bbelch]
 * 
 * Revision 1.1.6.2  1992/12/23  20:19:39  zeliff
 * 	Embedding copyright notice
 * 	[1992/12/23  15:33:08  zeliff]
 * 
 * Revision 1.1.2.3  1992/05/20  21:21:26  sommerfeld
 * 	Execute calls queued before executor starts up [sommerfeld]
 * 	This is a somewhat kludgey fix, but they need it now.
 * 	[1992/05/20  19:02:25  sommerfeld]
 * 
 * Revision 1.1.2.2  1992/05/01  17:01:36  rsalz
 * 	  5-mar-92 wh            DCE 1.0.1 merge.
 * 	 21-jan-92 markar        startup newly created pools immediately if server_listen
 * 	                         has already been called.
 * 	[1992/05/01  16:49:20  rsalz]
 * 
 * Revision 1.1  1992/01/19  03:08:14  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
...
# define DMS_CTHREAD_POOL_ASSIGN_THREAD(P)
# define DMS_CTHREAD_POOL_IDLE_THREAD(P)
    ...
    { \
        ...
        { \
            *(ct) = (p)->idle_cthread; \
            ...
            (p)->n_idle--; \
            DMS_CTHREAD_POOL_ASSIGN_THREAD(p); \
        } \
        else \
        { \
...
#define CTHREAD_POOL_IDLE_THREAD(p, ct) \
    { \
        DMS_CTHREAD_POOL_IDLE_THREAD(p); \
        (p)->n_idle++; \
        (p)->idle_cthread = ct; \
    }
...
#endif
{
	...
	if (!skip_startup) 
	{
		...
		(*(pvt->executor)) (pvt->optargs, run_queued_call);
                /*
                 * The bug in the old IDL compiler causes the sstub returning
                 * with the general cancellability enabled. Disable it.
                 */
                pthread_setcancel(CANCEL_OFF);
		/*
...
#endif
{
    ...
    if (call->u.server.cancel.count)
    {
        ...
        oc = pthread_setcancel(CANCEL_ON);
        TRY
            pthread_testcancel();
        CATCH_ALL
