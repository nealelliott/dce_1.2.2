...
SCCS:          @(#)tcc_mac.h    1.6 1/24/93 DTET release 1.0b
NAME:          tcc_mac.h
...
#define TET_REALLOC2(_ptr, _val) tet_realloc((char *) _ptr, (size_t)(_val), __FILE__, __LINE__, 1)
#define TET_REALLOC(_ptr, _val) tet_realloc((char *) _ptr, (size_t)(_val), __FILE__, __LINE__, 0)
#define TET_FREE(_ptr) tet_free((char *) (_ptr), __FILE__, __LINE__)
...
#define BAIL_OUT(_msg) 		{ 					\
					int errnotmp; 			\
					errnotmp = errno;		\
					(void) fprintf(stderr,"tcc (%d) (%s %d): ",(int) getpid(), __FILE__,__LINE__); 		\
					errno = errnotmp;		\
					perror(_msg);			\
					tet_shutdown(); 		\
				}
...
#define BAIL_OUT2(_msg) 	{					\
					(void) fprintf(stderr,"tcc (%d) (%s %d): %s\n", (int) getpid(), __FILE__,__LINE__,_msg); 	\
					tet_shutdown(); 		\
				}
...
#define ERROR(_msg)		{					\
					int errnotmp;			\
					errnotmp = errno;		\
					tcc_error(_msg, NONFATAL, __FILE__, __LINE__);									\
					errno = errnotmp;		\
				}


/* determines whether to call tcc_error() which gives diagnostic reports to
...
#define FATAL_ERROR(_msg)	{				\
					tcc_error(_msg, FATAL, __FILE__, __LINE__); 								\
					tet_shutdown(); 	\
				}

#define NOT_LOGGED_ON    0
#define LOGGED_ON        1
#define LOGOFF		 0
#define LOGON		 1
...
/* Bit fields used in comparison with debug_flag */

#define DBGFDESC        1	/* function call details */
#define DBGFINFO        2	/* info generated by a function */
#define DBGFILES        4	/* opening and closing files, pipes etc */
#define DBGMEM          8	/* malloc()ing, realloc()ing and free()ing */
#define DBGMEM2         16	/* table of unfreed memory */
#define DBGMEM3         32	/* including  memory we didn't expect to free */
/* 64 is spare */
#define DBGSYNTAX  	128	/* messgaes in the syntax checker */
#define DBGCHILDREN  	256	/* generate messages from inside children */
#define DBGUTIL		512	/* debug from frequently used utility funcs */
#define DBGSIGNALS	1024	/* messages related to signal processing */
#define DBGRUNRES	2048	/* rerun/resume messages */
#define DBGWARN		4096	/* warning messages */

extern    int       debug_flag;
extern    int       mypid;

#define DBUG(n)	if((mypid != 0 && (debug_flag & DBGCHILDREN)) && (debug_flag & n))   \
			(void) fprintf(stderr,"%d:",(int) getpid());			     \
		if(((mypid == 0) || (mypid != 0 && (debug_flag & DBGCHILDREN))) && (debug_flag & n)) (void) fprintf
