 ...
 * HISTORY
 * $Log: misc.c,v $
 * Revision 1.1.5.2  1996/11/21  14:13:49  arvind
 * 	OSF DCE 1.2.2 Drop5
 * 	[1996/11/21  14:12:43  arvind]
 *
 * Revision /main/DCE_1.2.2/2  1996/11/20  16:25 UTC  arvind
 * 	Fix from OSF
 * 	[1996/11/20  14:23 UTC  arvind  /main/DCE_1.2.2/arvind_work/1]
 * 
 * Revision /main/DCE_1.2.2/1  1996/10/15  19:04 UTC  bartf
 * 	fix dced acl code returns wrong error on unauthorized
 * 
 * Revision /main/bartf_pk_fix/1  1996/10/09  19:39 UTC  bartf
...
)
{
	...
	ep->description = strdup_for_client("A server", st);
	ep->storage_tag = strdup_for_client("#Internal", st);
	ep++;
	items->count++;
    }
...
{
    dced_entry_data_t           E;
    dced0_entry_data_t          oldE;
    attr_schema_data_t          S;
    attr_schema0_data_t         oldS;
    server_data_t               S2;
    server0_data_t              oldS2;
    uuid_t                      *u;
    error_status_t              st2;
    boolean32                   convertit;
    dce_db_handle_t             old_db;
    dce_db_handle_t             new_db;
    char                        other_name[128];
    char                        *p;
    ...
    if (*st != db_s_no_more) {
       ...
       if(((strcmp(name, HOSTDATA_DB_NAME) ==0)) ||
         ((strcmp(name, RKEYTAB_DB_NAME) ==0))) {
               dce_db_fetch_by_uuid(db, u, &E, st);    /* entries */
        }
       else if(((strcmp(name, SRVRCONF_DB_NAME) ==0)) ||
       ((strcmp(name, SRVREXEC_DB_NAME) ==0))) {
               dce_db_fetch_by_uuid(db, u, &S2, st);   /* servers */
       }
       else if(strcmp(name, ATTR_SCHEMA_DB_NAME) ==0) {
               dce_db_fetch_by_uuid(db, u, &S, st);    /* attr_schemas */
       }
       RETURN_IF_UNRECOVERABLE(*st);
    }
    dce_db_iter_done(db, st);
    ...
    if (!convertit)
        return;
    /* Just remove srvrexec because it will be regenerated */
    if(strcmp(name, SRVREXEC_DB_NAME) ==0) {
       dce_db_close(&db, &st2);
       unlink(name);
    ...
    /* Yes.  Close and re-open with the old conversion function. */
    dce_db_close(&db, &st2);
    dce_db_open(name, NULL, oflags, func2, &old_db, st);
    CHECK_STAT_RET(*st);

    /* Open a new database. */
    strcpy(other_name, name);
    if ((p = strchr(other_name, '.')) == NULL)
       p = other_name + strlen(other_name);
    strcpy(p, ".NEW");
    dce_db_open(other_name, NULL, oflags | db_c_create,
       func, &new_db, st);

    /* Iterate over the old database, write new entries. */
    dce_db_iter_start(old_db, st);
    CHECK_STAT_RET(*st);
    for ( ; ; ) {
        dce_db_iter_next_by_uuid(old_db, &u, st);
        if (*st == db_s_no_more) break;
       CHECK_STAT_RET(*st);

    if(((strcmp(name, HOSTDATA_DB_NAME) ==0)) ||
      ((strcmp(name, RKEYTAB_DB_NAME) ==0))) {
       dce_db_fetch_by_uuid(old_db, u, &oldE, st);
       CHECK_STAT_RET(*st);
       E.h = oldE.h;
       E.data.id = oldE.data.id;
       E.data.name = oldE.data.name;
       E.data.description = oldE.data.description;
       E.data.storage_tag = oldE.data.storage_tag;
       /* XXX This is safe... */
       E.flags = entry_c_data_imported;
       dce_db_store_by_uuid(new_db, &E.data.id, &E, st);
       entry_free_ptrs(&oldE.data, dced_sm_client_free);
       CHECK_STAT_RET(*st);
    }
    else if(strcmp(name, SRVRCONF_DB_NAME) ==0) {
       dce_db_fetch_by_uuid(old_db, u, &oldS2, st);    /* servers */
       CHECK_STAT_RET(*st);
        S2.h = oldS2.h;
        S2.used = oldS2.used;
        S2.flags = oldS2.flags;
        S2.data = oldS2.data;
        S2.srvr_file = oldS2.srvr_file;
       dce_db_store_by_uuid(new_db, &S2.data.id, &S2, st);
       CHECK_STAT_RET(*st);
    }
    else if(strcmp(name, ATTR_SCHEMA_DB_NAME) ==0) {
       dce_db_fetch_by_uuid(old_db, u, &oldS, st);     /* attr_schemas */
       CHECK_STAT_RET(*st);
       S.h = oldS.h;
       S.data = oldS.data;
       dce_db_store_by_uuid(new_db, &S.data.attr_id, &S, st);
       CHECK_STAT_RET(*st);
    }
   }

    /* Close database, rename files. */
    dce_db_close(&new_db, st);
    dce_db_close(&old_db, st);

    strcpy(p, ".BAK");
    rename(name, other_name);
    strcpy(p, ".NEW");
    rename(other_name, name);
    dce_db_open(name, NULL, oflags, func, &db, st);
}
...
)
{
	...
	*cp++= '/';
    ...
    if (hd_db != NULL) {
       convert_and_open(fname, oflags,
                 (dce_db_convert_func_t)dced_entry_convert,
                 (dce_db_convert_func_t)dced0_entry_convert, hd_db, st);
       if (*st != error_status_ok) {
           dce_svc_printf(DCED_S_CANT_OPEN_DATABASE_MSG, fname, errno);
       }
    }
    ...
    if (km_db != NULL) {
       convert_and_open(fname, oflags,
                 (dce_db_convert_func_t)dced_entry_convert,
                 (dce_db_convert_func_t)dced0_entry_convert, km_db, st);
       if (*st != error_status_ok) {
           dce_svc_printf(DCED_S_CANT_OPEN_DATABASE_MSG, fname, errno);
       }
    }
    ...
    if (as_db != NULL) {
       convert_and_open(fname, oflags,
                 (dce_db_convert_func_t)dced_attr_schema_convert,
                 (dce_db_convert_func_t)dced0_attr_schema_convert, as_db, st);
       if (*st != error_status_ok) {
           dce_svc_printf(DCED_S_CANT_OPEN_DATABASE_MSG, fname, errno);
       }
    }
    ...
    if (sc_db != NULL) {
       convert_and_open(fname, oflags,
                 (dce_db_convert_func_t)dced_server_convert,
                 (dce_db_convert_func_t)dced0_server_convert, sc_db, st);
       if ((*st != error_status_ok) && (*st != db_s_no_more)) {
           dce_svc_printf(DCED_S_CANT_OPEN_DATABASE_MSG, fname, errno);
       }
    }
    ...
    if (sx_db != NULL) {
       convert_and_open(fname, oflags,
                 (dce_db_convert_func_t)dced_server_convert,
                 (dce_db_convert_func_t)dced0_server_convert, sx_db, st);
       if ((*st != error_status_ok) && (*st != db_s_no_more)) {
           dce_svc_printf(DCED_S_CANT_OPEN_DATABASE_MSG, fname, errno);
       }
    }
