 ...
 * HISTORY
 * $Log: command.c,v $
 * Revision 1.1.12.2  1996/02/18  18:51:18  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  18:04:51  marty]
 *
 * Revision 1.1.12.1  1995/12/07  22:14:51  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/HPDCE02/jrr_1.2_mothra/1  1995/11/20  19:07 UTC  jrr
 * 	Merge second XIDL drop for DCE 1.2.1
 * 	[1995/11/17  17:02 UTC  dat  /main/dat_xidl2/1]
 * 
 * 	fix standard versioning
 * 	[1995/10/31  17:26:21  bfc]
 * 	 *
 * 
 * 	oct 95 idl drop
 * 	[1995/10/23  00:43:04  bfc]
 * 
 * 	may 95 idl drop
 * 	[1995/10/21  22:55:49  bfc]
 * 
 * 	DCE for DEC OSF/1: populate from OSF DCE 1.1
 * 	[1995/10/21  17:23:42  bfc]
 * 
 ...
 * 	Merge idl++ mods into Mothra
 * 
 * 	HP revision /main/HPDCE02/2  1994/09/21  14:49 UTC  tatsu_s
 * 	Merged mothra up to dce1_1_b18.
 * 	[1995/12/07  21:10:26  root]
 * 
 * 	Initial King Kong branch
 * 	[1993/07/08  08:45:21  root]
 * 	Revision 1.1.6.5  1993/07/12  18:27:03  hinxman
 * 	     Re-check-in
 * 	[1993/07/12  18:23:22  hinxman]
 * 
 * Revision 1.1.8.1  1994/08/17  21:19:55  tom
 * 	Bug 10359 - Process header filename,
 * 	even if we aren't going to create it.
 * 	[1994/08/17  21:14:20  tom]
 * 
 ...
 * 	[1993/07/12  18:23:22  hinxman]
 * 
 * Revision 1.1.8.3  1993/06/04  21:20:25  lmm
 * 	Added the get_idl_opts and get_quoted_args routines along with calls
 * 	to them in _HP_INSTRUMENTED_IDL_ sections of CMD_parse_args.  This
 * 	code provides the IDLOPTS environment variable functionality: at
 * 	invocation, the contents of IDLOPTS are placed in front of the first
 * 	command line option before being passed to the getflags parsing routine.
 * 	[1993/06/04  21:20:04  lmm]
 * 	SRetransimission of same change
 * 	[1993/07/12  18:11:34  hinxman]
 * 
 * Revision 1.1.6.3  1993/07/09  12:31:58  hinxman
 * 	OT 8242 - add -no_header flag to command line
 * 
 * Revision 1.1.6.2  1993/07/07  19:56:44  ganni
 * 	reduced stub idl sources
 * 	[1993/07/07  19:30:30  ganni]
 * 
 * $EndLog$
 */
/*
**  05-Jun-92 harrow    Obsolete -[no_]bug 4.
**  03-Apr-92 harrow    Default reverts to -bug 4 for DCE V1.*
*/
/*
**  @OSF_COPYRIGHT@
**
...
static char     *dce_include_dir = NULL;
static char     *dce_include_root = NULL;
...
#include <lnmdef.h>
int SYS$TRNLNM();
static $DESCRIPTOR(lnm_system_table, "LNM$SYSTEM_TABLE");
static $DESCRIPTOR(decc_cc_default, "DECC$CC_DEFAULT");
static char decc_cc_def_str[256];
static unsigned short decc_cc_def_str_len;
static unsigned long trnlnm_itmlst[] = {
    (LNM$_STRING << 16) | 256,
    (unsigned long)decc_cc_def_str,
    (unsigned long)&decc_cc_def_str_len,
    0
};
#endif
#endif
...
static char     *cxxmgr_file;	/* manager class file used with C++ option */

/*  List of import directories - allow two extra slots for the implicit
 *  -I CD_DIR -I DEFAULT_IDIR -I DCE_INCLUDE_EV and one for the sentinel.
 */
static char     *(import_directories[MAX_IMPORT_DIRECTORIES + 3 + 1]);
...
static char     *standard_vals[] = {
                "portable", "dce_v10", "dce_v103",
                "dce_v11", "extended", NULL };
static int      standard_ivals[] = {
                opt_standard_dce_1_0, opt_standard_dce_1_0,opt_standard_dce_1_0,
                opt_standard_dce_1_1, opt_standard_dce_1_2 };
/* Note: order of values in this array must match opt_lang_* in command.h */
static char     *lang_vals[] = { "c",
				"cxx",
                                NULL };
static char     *lang;
...
OPTIONS option_table[]={
    ...
    "no_cxxmgr",    DENYARG,	                    FD(cmd_opt[opt_cxxmgr]),
...
#ifdef DUMPERS
typedef enum {bit, string, string_list, int_list, number, integer} opt_kind_t;
typedef struct
{
    char *opt_name;
    opt_kind_t opt_kind;
...

{
    ...
    char    l_cxxmgr_file[PATH_MAX];    /* Work buf for manager class filespec*/
    ...
    cc_opt                      = "-std1";
#elif defined(_ALPHA_) && defined (_MSDOS)
    cc_opt                      = "-D_ALPHA_=1 -DWIN32";
#elif defined (_MSDOS)
    cc_opt                      = "-D_X86_=1 -DWIN32";
#else
    cc_opt                      = "";
#endif
    ...
    CMD_def_cpp_cmd             = CPP;
#if defined(VMS) && !defined(__alpha)  
    if (SYS$TRNLNM(NULL, &lnm_system_table, &decc_cc_default, NULL,
        trnlnm_itmlst) == SS$_NORMAL)
    {
        if (strncmp("/DECC", decc_cc_def_str, decc_cc_def_str_len) == 0)
            CMD_def_cpp_cmd     = CPP_DECC;
    }
#endif
...
#ifdef CPP_BY_DEFAULT
    cpp_cmd                     = CMD_def_cpp_cmd;
    ...
    cmd_opt[opt_lang]           = TRUE;
    lang                        = lang_vals[opt_lang_c];
    ...
    *p_cmd_val = cmd_val;
...
#endif
    /*
     * Check the -lang option.
     */
    cmd_val[opt_lang] = (void *)check_str_list("lang", lang, lang_vals);

    if ((int)cmd_val[opt_lang] == opt_lang_cxx) {
        if (!strcmp(cc_cmd, CC_DEF_CMD))
        	cc_cmd = CXX_DEF_CMD;
	if (!strcmp(cstub_suffix, CSTUB_SUFFIX))
		cstub_suffix = CSTUB_SUFFIX_CXX;
	if (!strcmp(sstub_suffix, SSTUB_SUFFIX))
		sstub_suffix = SSTUB_SUFFIX_CXX;
    } else
        cmd_opt[opt_cxxmgr] = FALSE;
    ...
    if (cmd_opt[opt_def_idir])
    {
        for (i = 0; import_directories[i] != NULL
            ...
            ;
#if defined(_MSDOS)
	dce_include_root = getenv(DCE_INCLUDE_EV);
        if (dce_include_root == NULL)
           import_directories[i+2] = NULL;
	else
	{
	   dce_include_dir = (char *)MALLOC(strlen(dce_include_root) + 13);
	   strcat(dce_include_dir,dce_include_root);
	   strcat(dce_include_dir,"\\dce\\include");
           import_directories[i+2] = dce_include_dir;
           import_directories[i+3] = NULL;
	}
#else
        import_directories[i+2] = NULL;
#endif
...
#else
	if (dce_include_dir == NULL)
           flags_incr_count(option_table, "I", 2);
	else
           flags_incr_count(option_table, "I", 3);
#endif
    }
...
#define extlen(s) (strchr(s, '.') == NULL) ? strlen(s) : (strchr(s, '.') - s)
    ...
    sprintf(filespec, "%s_mgr.h", src_filename);  	       /* =tbl */
    if (!FILE_form_filespec(cxxmgr_file, out_dir, (char *)NULL,
                            filespec, l_cxxmgr_file))
    {
        message_print(NIDL_INVFILESPEC, header_file);
        return FALSE;
    }
    cxxmgr_file = l_cxxmgr_file;    /* Point at local buffer */
...
#ifdef CPP_BY_DEFAULT
    if (cpp_cmd[0] == '\0')
        cpp_cmd = (void *)CMD_def_cpp_cmd;  /* Set to default */
    ...
    cmd_val[opt_cxxmgr]     = (void *)alloc_and_copy(cxxmgr_file);
