 ...
 * HISTORY
 * $Log: cnp.h,v $
 * Revision 1.1.103.1  1996/06/04  21:53:55  arvind
 * 	u2u: Pass handle  from rpc__cn_call_start() to RPC_CN_AUTH_CRED_REFRESH()
 * 	[1996/05/06  20:14 UTC  burati  /main/DCE_1.2/2]
 *
 * 	merge u2u work
 * 	[1996/04/29  20:07 UTC  burati  /main/DCE_1.2/mb_u2u/1]
 *
 * 	u2u work off Mothra, before 1.2 branch is available
 * 	Pass handle from rpc__cn_call_start() to RPC_CN_AUTH_CRED_REFRESH()
 * 	[1996/04/29  20:04 UTC  burati  /main/HPDCE02/mb_mothra8/1]
 *
 ...
 *
 * Revision 1.1.101.2  1996/02/18  22:55:33  marty
 * 	Update OSF copyright years
 * 	[1996/02/18  22:14:44  marty]
 * 
 * Revision 1.1.101.1  1995/12/08  00:17:26  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/HPDCE02/jrr_1.2_mothra/1  1995/11/16  21:30 UTC  jrr
 * 	Remove Mothra specific code
 * 
 * 	HP revision /main/HPDCE02/2  1995/10/03  16:50 UTC  sommerfeld
 * 	increase general debug levels for CN to reduce noise.
 * 	[1995/09/21  00:22 UTC  sommerfeld  /main/HPDCE02/sommerfeld_dfsperf/1]
 * 
 ...
 * 	Local RPC Security Bypass.
 * 	[1995/12/07  23:57:41  root]
 * 
 * Revision 1.1.98.6  1994/09/01  16:12:53  ganni
 * 	Introduce a new assoc flag RPC_C_CN_ASSOC_AUTH_EXPECTED,
 * 	to monitor the authentiaction in bind or alter_context PDU
 * 	exchange.
 * 	[1994/09/01  16:10:43  ganni]
 * 
 * Revision 1.1.98.5  1994/07/14  21:31:09  tom
 * 	 07-jun-93   us	Added tbl_id to rpc_cn_sm_ctl_blk_t struct.
 * 		Use the tbl_id to bypass event evaluation
 * 		routines in the rpc protocol itself.
 * 	  4-apr-93   us  Changed the state defines by adding
 * 	                 value rpc_c_cn_statebase to each
 * 	                 state as part of the runtime
 * 	                 performance changes. The state value
 * 	                 was incremented so that we could distinguish
 * 	                 between states and action routine indexes
 * 	                 in the state tables, making the protocol
 * 	                 engine (rpc_cn_sm_eval_event) work more
 * 	                 efficiently.  State values are >= 100 and
 * 	                 action routine indexes are < 100.
 * 	                 Changed rpc_cn_sm_state_entry_t/
 * 	                 _p to reflect the collapsing of the predicate
 * 	                 function into the action routines.  Removed
 * 	                 reference to predicate table in rpc_cn_sm_
 * 	                 ctl_blk_t.
 * 	[1994/07/11  17:23:23  tom]
 * 
 * Revision 1.1.98.4  1994/07/06  20:26:46  sommerfeld
 * 	[OT11176] add "old_client" argument to VFY_CLIENT_REQ
 * 	[1994/07/06  19:43:19  sommerfeld]
 * 
 * Revision 1.1.98.3  1994/03/17  23:06:37  tom
 * 	More Big PAC -
 * 	  Moved rpc_cn_assoc_sm_work_t here from cnassm.c.
 * 	  Added assoc_sm_work to assoc structure.
 * 	  Added retry argument to RPC_CN_AUTH_FMT_CLIENT_REQ.
 * 	  Add bind_packets_sent to assoc structure.
 * 	[1994/03/17  23:04:12  tom]
 * 
 * Revision 1.1.98.2  1994/01/21  22:34:34  cbrooks
 * 	RPC Code Cleanup - Initial Submission
 * 	[1994/01/21  21:54:16  cbrooks]
 * 
 * Revision 1.1.98.1  1994/01/14  20:31:12  tom
 * 	Big PAC - Added two arguments to RPC_CN_AUTH_FMT_CLIENT_REQ.
 * 	          Added assoc_vers_minor to assoc structure.
 * 	          Added error debug levels to trigger fragmentation.
 * 	[1994/01/13  22:51:02  tom]
 * 
 * Revision 1.1.7.2  1993/08/18  19:52:25  tom
 * 	Bug 8110 - correctly print the routine name in RPC_CN_DBG_RTN_PRINTF.
 * 	[1993/08/18  19:51:31  tom]
 * 
 * Revision 1.1.5.4  1993/01/03  22:59:00  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/03  20:00:00  bbelch]
 * 
 * Revision 1.1.5.3  1992/12/23  20:16:28  zeliff
 * 	Embedding copyright notice
 * 	[1992/12/23  15:31:43  zeliff]
 * 
 * Revision 1.1.5.2  1992/10/13  20:54:03  weisman
 * 	      15-sep-92   ko      Remove CRED_VALID auth entry point macro.
 * 	                          Add CRED_CHANGED auth entry point macro.
 * 	                          Add CRED_REFRESH auth entry point macro.
 * 	[1992/10/13  20:46:39  weisman]
 * 
 * Revision 1.1.3.3  1992/05/22  18:57:34  wei_hu
 * 	 19-may-92   ko      Add bit fields to cn_ctlblk in assoc for
 * 	                     synchronization between rcvr and sender threads.
 * 	 12-may-92	r$	Set CN_AUTH depending on AUTH_KRB_CN,AUTH_KRB_DG flags.
 * 	 01-may-92   sp      Fix RPC_CN_AUTH_CVT_ID* macros for non- CN_AUTH case.
 * 	 29-apr-92   gh      Add assoc_shutdown_req_count field in rpc_cn_assoc_t and
 * 	                     macro rpc_c_cn_assoc_server_max_shutdown_req_count.
 * 	 12-apr-92   sp      Fix calls to rpc__auth_cvt_id_*.
 * 	 30-mar-92   af      Add the macro RPC_CN_ALIGN_PTR(ptr, boundary)
 * 	                     for aligning pointers on specified boundaries.
 * 	 16-mar-92   sp      Add field for raw packet pointer in cn_assoc_t.
 * 	                     Change signature of RPC_CN_AUTH_RECV_CHECK.
 * 	 27-feb-92   sp      Change signature of RPC_CN_AUTH_CRED_VALID.
 * 	 16-dec-91   ko      Bug fix for binding serialization. Add
 * 	                     resolving_thread_id to rpc_cn_binding_rep_t.
 * 	[1992/05/22  18:42:59  wei_hu]
 * 
 * Revision 1.1  1992/01/19  03:12:10  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
...
#define RPC_C_CN_DBG_THREADS            3
#define RPC_C_CN_DBG_ERRORS             1
#define RPC_C_CN_DBG_BUFFS              2
#define RPC_C_CN_DBG_GENERAL            2
...
#define RPC_CN_AUTH_CRED_REFRESH(auth_info, handle, st)\
{\
    rpc_cn_auth_epv_t   *_cn_epv;\
    _cn_epv = RPC_CN_AUTH_PROT_EPV((auth_info)->authn_protocol);\
    (*_cn_epv->cred_refresh)(auth_info, handle, st);\
}
