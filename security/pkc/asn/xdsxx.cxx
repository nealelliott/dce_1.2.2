 ...
 * HISTORY
 * $Log: xdsxx.cxx,v $
 * Revision 1.1.2.1  1996/10/03  20:26:43  arvind
 * 	OSF DCE 1.2.2 Drop 4
 * 	[1996/10/03  19:29:28  arvind]
 *
...
xds_status_t::xds_status_t(OM_object ds_status) : om_translation_t(ds_status) {
...
#ifdef XDS_DEBUG
        fprintf(stderr,
                "Error %d converting changes to OM_object\n",
                error_status);
#endif
	return NULL;
    };
...
xds_status_t * xds_t::modify_rdn(const x500name & name,
    ...
    if (error_status = new_rdn.xom_object(om_name)) {
#ifdef XDS_DEBUG
        fprintf(stderr,
...
om_translation_t::om_translation_t(OM_object & obj) {
    OM_return_code om_return_code;
...
#ifdef XDS_DEBUG
            fprintf(stderr,"xds_result_t::DS_ALIAS_DEREFERENCED\n");
#endif
...
#ifdef XDS_DEBUG
fprintf(stderr, "Constructing an xds_list_info_item_t object...\n");
fflush(stderr);
#endif
    ...
    while (obj[i].type != OM_NO_MORE_TYPES) {
        ...
        case DS_ALIAS_ENTRY:
#ifdef XDS_DEBUG
fprintf(stderr, "Decoding DS_ALIAS_ENTRY...\n");
fflush(stderr);
#endif
            if ((obj[i].syntax & OM_S_SYNTAX) != OM_S_BOOLEAN) {
#ifdef XDS_DEBUG
        ...
        case DS_FROM_ENTRY:
#ifdef XDS_DEBUG
fprintf(stderr, "Decoding DS_FROM_ENTRY...\n");
fflush(stderr);
#endif
            if ((obj[i].syntax & OM_S_SYNTAX) != OM_S_BOOLEAN) {
#ifdef XDS_DEBUG
        ...
        case DS_RDN:
#ifdef XDS_DEBUG
fprintf(stderr, "Decoding DS_RDN...\n");
fflush(stderr);
#endif
...
#ifdef XDS_DEBUG
fprintf(stderr, "Parsed an xds_list_info_item_t object.\n");
fflush(stderr);
#endif
}
...
fflush(stderr);
    ...
    while (obj[i].type != OM_NO_MORE_TYPES) {
        ...
        case DS_LIST_INFO:
#ifdef XDS_DEBUG
fprintf(stderr, "Decoding DS_LIST_INFO...\n");
fflush(stderr);
#endif
...
#ifdef XDS_DEBUG
fprintf(stderr, "Re-starting object scan at 0 in sub-object...\n");
#endif
        ...
        case DS_OBJECT_NAME:
#ifdef XDS_DEBUG
fprintf(stderr, "Decoding DS_OBJECT_NAME...\n");
fflush(stderr);
#endif
            if ((obj[i].syntax & OM_S_SYNTAX) != OM_S_OBJECT) {
#ifdef XDS_DEBUG
...
#ifdef XDS_DEBUG
fprintf(stderr, "Decoding DS_PARTIAL_OUTCOME...\n");
fflush(stderr);
#endif
            wasPartialOutcome = 1;
            break;
...
#ifdef XDS_DEBUG
fprintf(stderr, "Decoding DS_SUBORDINATES...\n");
fflush(stderr);
#endif
            if ((obj[i].syntax & OM_S_SYNTAX) != OM_S_OBJECT) {
#ifdef XDS_DEBUG
...
#ifdef XDS_DEBUG
fprintf(stderr,
        "Descent for subobject...\n");
fflush(stderr);
#endif
            *item_ptr = new xds_list_info_item_t(obj[i].value.object.object);
#ifdef XDS_DEBUG
fprintf(stderr, "Got new xds_list_info_item_t (*item_ptr = %p)\n", *item_ptr);
fflush(stderr);
#endif
            if (error_status = (*item_ptr)->error_status) {
#ifdef XDS_DEBUG
fprintf(stderr, "info_item decode returned %d\n", error_status);
fflush(stderr);
#endif
                return;
            };
...
#ifdef XDS_DEBUG
fprintf(stderr, "Finished decoding xds_list_result_t\n");
fflush(stderr);
#endif
}
...
#ifdef XDS_DEBUG
            fprintf(stderr,
                    "Unrecognized xds_read_reault_t component, type = %d\n",
                    obj[i].type);
#endif
        };
        i++;
...
//    case OM_S_BIT_STRING : return VC_BITSTRING;
    case OM_S_OCTET_STRING : return VC_OCTETSTRING;
    case OM_S_GENERAL_STRING :
    case OM_S_GENERALIZED_TIME_STRING :
...
unsigned32 xds_value_t::get_value_syntax(OM_syntax &s) {
    s = theValueSyntax;
    return 0;
}

unsigned32 xds_value_t::get_value_class(value_class_t & s) {
    s = theValueType;
    return 0;
}

unsigned32 xds_value_t::get_value(unsigned char * &s, size_t & l) {
...
unsigned32 xds_value_t::get_value(char * &s) {
...
unsigned32 xds_value_t::get_value(long & s) {
    switch (theValueType) {
    case VC_INTEGER:
        s = theInteger;
        break;
