 ...
 * HISTORY
 * $Log: cma_signal.c,v $
 * Revision 1.1.6.2  1996/02/17  23:33:28  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:49:41  marty]
 *
 ...
 *
 * Revision 1.1.6.1  1995/12/07  21:56:37  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/07  21:03:31  root]
 * 
 * 	Move latest 1.0.1-based revision into 1.0.2.
 * 	[1992/08/28  20:42:13  bolinger]
 * 
 * Revision 1.1.2.2  1994/06/10  20:52:46  devsrc
 * 	cr10872 - fixed copyright
 * 	[1994/06/10  16:49:44  devsrc]
 * 
 ...
 * 
 * Revision 1.1.2.1  1994/04/14  21:31:46  ohara
 * 	moved again, with changes
 * 	[1994/04/14  20:39:13  ohara]
 * 
 * Revision 1.1.2.1  1994/04/01  20:14:44  jd
 * 	Moved from src/threads to here
 * 	[1994/04/01  18:30:33  jd]
 * 
...
static void cma___sig_async_term (int,cma__t_siginfo,cma__t_sigcontext *);

static void cma___sig_io(int,cma__t_siginfo,cma__t_sigcontext *);

static void cma___sig_sync_term (int,cma__t_siginfo,cma__t_sigcontext *);

void cma___sig_sync_kill (int, cma__t_siginfo, cma__t_sigcontext *);
...
cma_sigwait(
    ...
    if (__nr_sigprocmask (SIG_BLOCK, &cma___g_sig_block_mask, &sig_mask) == -1)
	...
	if (sigismember (set, i)) {
		...
		if (__nr_sigaction (i, &act, &sig_oact[i]) == -1)  
     ...
     */
    if (__nr_sigprocmask (SIG_SETMASK, &sig_mask, (sigset_t *)cma_c_null_ptr) == -1)
	cma__bugcheck ("sigwait:5");

    if (__nr_sigprocmask (SIG_UNBLOCK, set, (sigset_t *)cma_c_null_ptr) == -1)
	...
	*	
	*	Use a temp variable to work around this problem.
	*/
	{
	cma__t_queue *tmp_queue_ptr = (cma__t_queue *)&wblk.queue;
	cma__queue_remove (tmp_queue_ptr);
	}
#else
	cma__queue_remove (&wblk.queue);
#endif /* CMA_PLATFORM_ == _CMA__SINIX_MX300I */
	 ...
	 */
	if (__nr_sigprocmask (SIG_SETMASK, &sig_mask, (sigset_t *)cma_c_null_ptr) 
		== -1)
...
cma__abort_process(
	...
	if ( __nr_sigaction( abort_signal, &act, (struct sigaction *)NULL ) == -1 )
		{
		cma__abort();
		}
    /*
...
# if defined(SIGLOST) && SIGIOT != SIGLOST
...
# endif
    if (sigaddset (&cma___g_sigact_block_mask, SIGPOLL) == -1)
...
# endif
	...
	if (i == cma__c_timer_signal) {
	    ...
	    if (__nr_sigaction (i, &act, cma_c_null_ptr) == -1)
		cma__bugcheck ("init_signal:46");
	    act.sa_flags &= ~SA_RESTART;
...
# endif
	     ...
	     */
            ...
            case SIGSEGV:
		{
                static char  crash_stack_area[SIGSTKSZ];
		static stack_t crash_sigstack = {crash_stack_area, SIGSTKSZ, 0};

		if ( sigaltstack(&crash_sigstack, (stack_t *)NULL) != 0 )
			{
			perror("sigalstack");
			exit(1);
			}
		...
		if (__nr_sigaction (i,&act,(struct sigaction *)cma_c_null_ptr) == -1)
...
cma__sig_deliver(
    ...
    cma_t_integer   code = cma__sigcode(siginfo);

    if (tcb->sigaction_data[sig].sa_handler == SIG_IGN) {
	  tcb->syscall_state   |= cma__c_syscall_extintrpt;
		return cma_c_true;  /* Ignore this signal */
	}
    else if ((tcb->sigaction_data[sig].sa_handler != 0)
	     && (tcb->sigaction_data[sig].sa_handler != SIG_DFL)) 
	{
	cma_t_natural save_syscall_state = tcb->syscall_state;
    tcb->syscall_state    = 0;
	(tcb->sigaction_data[sig].sa_handler)(sig, siginfo, scp);
	 tcb->syscall_state    = save_syscall_state | cma__c_syscall_extintrpt;
	return cma_c_true;
	}
 ...
 */
	...
	cma__puthex    (output, scp->
#if _CMA_PLATFORM_ == _CMA__SINIX_MX300I
		uc_mcontext.gregs[EIP]);
#elif (_CMA_PLATFORM_ == _CMA__DCOSX_MIPS)
		uc_mcontext.gpregs[CXT_EPC]);
#else
		sc_pc);
#endif
	cma__putstring (output, ".");
    ...
    if (__nr_sigprocmask (SIG_SETMASK, 
			&scp->uc_sigmask, 
...
cma___sig_sync_kill
        ...
        cma__t_siginfo      siginfo,
        cma__t_sigcontext   *scp)
/*** Big not here:  changed sigcontext to cma__t_sigcontex ***, it builds
now but I don't know if it is correct, Abby ***/
    
{
    ...
    if (cma__sig_deliver (sig, siginfo, scp)) return;
    /*
     * Dump core
     */
#if _CMA_PROTECT_MEMORY_
    cma__remap_stack_holes ();          /* Unprotect stack guard pages */
#endif
    act.sa_mask = cma___g_sig_block_mask;
    ...
    if (__nr_sigaction (sig, &act, cma_c_null_ptr) == -1)
...
cma___sig_io(
    ...
    cma_t_integer   code = cma__sigcode(siginfo);
    /*
     * Inform any threads that are sigwait'ing for this signal.
     */
    (void)cma__sig_deliver (sig, siginfo, scp);
	...
	cma__defer (cma__c_defer_io_avail, cma_c_null_ptr, cma_c_null_ptr);
...
cma___sig_sync_term(
	...
	case SIGILL:
		...
		case ILL_ILLOPC:
		{
		       exception_status = exc_s_illopc;
		       break;
 		}
		case ILL_ILLOPN:
		{
			exception_status = exc_s_illopn;
			break;
		}
		case ILL_ILLADR:
		{
		   exception_status = exc_s_illadr;
		   break;
		}
		case ILL_ILLTRP:
		{
			exception_status = exc_s_illtrp;
			break;
		}
		case ILL_PRVOPC:
		{
			exception_status = exc_s_prvopc;
			break;
		}
		case ILL_PRVREG:
		{
			exception_status = exc_s_prvreg;
			break;
		}
		case ILL_COPROC:
		{
			exception_status = exc_s_coproc;
			break;
		}
		case ILL_BADSTK:
		{
			exception_status = exc_s_badstk;
			break;
		}
#endif
		default:
		...
		case FPE_FLTRES:
			{
			exception_status = exc_s_fltres;
			break;
			}
		case FPE_FLTINV:
			{
			exception_status = exc_s_fltinv;
			break;
			}
#endif
		default:
