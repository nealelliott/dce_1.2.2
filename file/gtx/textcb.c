 ...
 * HISTORY
 * $Log: textcb.c,v $
 * Revision 1.1.75.1  1996/10/02  17:51:36  damon
 * 	Newest DFS from Transarc
 * 	[1996/10/01  18:40:24  damon]
 *
 * Revision 1.1.70.3  1994/08/03  16:15:54  ruby
 * 	Implement partial S12Y (Internationalization) for DFS commands and daemons
 * 	[1994/08/01  17:01:32  ruby]
 * 
 * Revision 1.1.70.2  1994/06/09  14:10:43  annie
 * 	fixed copyright in src/file
 * 	[1994/06/09  13:24:53  annie]
 * 
 * Revision 1.1.70.1  1994/02/04  20:20:40  devsrc
 * 	Merged from 1.0.3a to 1.1
 * 	[1994/02/04  15:14:01  devsrc]
 * 
 * Revision 1.1.68.1  1993/12/07  17:26:34  jaffe
 * 	1.0.3a update from Transarc
 * 	[1993/12/03  15:34:22  jaffe]
 * 
 * Revision 1.1.5.3  1993/01/19  16:04:15  cjd
 * 	embedded copyright notice
 * 	[1993/01/19  14:14:07  cjd]
 * 
 * Revision 1.1.5.2  1992/11/24  17:52:49  bolinger
 * 	Change include file install directory from .../afs to .../dcedfs.
 * 	[1992/11/22  18:06:17  bolinger]
 * 
 * Revision 1.1.3.2  1992/05/20  19:55:30  mason
...
#include <dcedfs/gtxtextcb.h>		/*Module interface*/
...
{ /*gator_textcb_Init*/
    ...
    if (initd) {
      dce_svc_printf(GTX_S_TC_INIT_CALLED_MULTIPLE_TIMES_MSG, rn);
      return(0);
    }
    else
      initd = 1;
...
{ /*gator_textcb_Create*/
    ...
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_ALLOCATE_TEXT_BUFFER_MSG, rn, bytesToAllocate);
    newBuff = (char *) malloc(bytesToAllocate);
    if (newBuff == (char *)0) {
      dce_svc_printf(GTX_S_TC_CANT_ALLOCATE_TEXT_BUFFER_MSG, rn, bytesToAllocate, errno);
      return((struct gator_textcb_hdr *)0);
    }
    else
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_ALLCATED_TEXT_BUFFER_MSG, rn, newBuff);
    blankLine = (char *) malloc(a_maxCharsPerEntry + 1);
    if (blankLine == (char *)0) {
      dce_svc_printf(GTX_S_TC_CANT_ALLOCATE_LINE_BUFFER_MSG, rn, a_maxCharsPerEntry+1, errno);
      free(newBuff);
      return((struct gator_textcb_hdr *)0);
    }
    ...
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_ALLOCATE_ARRAY_ITEM_MSG, rn, bytesToAllocate, a_maxEntriesStored);
    newEntries = (struct gator_textcb_entry *) malloc(bytesToAllocate);
    if (newEntries == (struct gator_textcb_entry *)0) {
      dce_svc_printf(GTX_S_TC_CANT_ALLOCATE_ARRAY_ITEM_MSG, rn, bytesToAllocate, a_maxEntriesStored, errno);
      free(newBuff);
      free(blankLine);
      return((struct gator_textcb_hdr *)0);
    }
      ...
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_ALLOCATED_ARRAY_ITEM_MSG, rn, newEntries);
    ...
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_ALLOCATE_CIRCULAR_BUFFER_MSG, rn, bytesToAllocate);
    newHdr = (struct gator_textcb_hdr *) malloc(bytesToAllocate);
    if (newHdr == (struct gator_textcb_hdr *)0) {
      dce_svc_printf(GTX_S_TC_CANT_ALLOCATE_CIRCULAR_BUFFER_MSG, rn, bytesToAllocate, errno);
      free(newBuff);
      free(blankLine);
      free(newEntries);
      return((struct gator_textcb_hdr *)0);
      ...
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_ALLOCATED_CIRCULAR_BUFFER_MSG, rn, newHdr);

    /*
      * Now, just initialize all the pieces and plug them in.
      */
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_ZERO_TEXT_BUFFER_MSG, rn, numBuffBytes, newBuff);
    bzero(newBuff, numBuffBytes);

    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_INIT_LINE_BUFFER_MSG, rn, blankLine);
    ...
    *(blankLine+a_maxCharsPerEntry) = '\0';
    ...
    for (curr_ent_num = 0, curr_buff = newBuff, curr_ent = newEntries;
	 ...
	 curr_ent++, curr_ent_num++, curr_buff += (a_maxCharsPerEntry+1)) {
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_INIT_BUFFER_ENTRY_MSG, rn, curr_ent_num, curr_buff);
      curr_ent->ID	      = 0;
    ...
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_FILL_CIRCULAR_BUFFER_MSG, rn, newHdr);
...
{ /*bumpEntry*/
     ...
     */
    if (gator_textcb_debug)
	  dce_svc_printf(GTX_S_TC_BUMP_CIRCULAR_BUFFER_MSG, rn, a_cbhdr, a_cbhdr->currEnt, a_cbhdr->currEntIdx, a_cbhdr->oldestEnt, a_cbhdr->oldestEntIdx, a_cbhdr->maxEntriesStored);
    ...
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_ZERO_ENTRY_MSG, rn, a_cbhdr->currEnt, a_cbhdr->currEntIdx, curr_ent);
    ...
    if (a_cbhdr->currEnt >= a_cbhdr->maxEntriesStored) {
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_ADVANCE_OLDEST_ENTRY_MSG, rn, a_cbhdr->oldestEnt, a_cbhdr->oldestEntIdx);
      a_cbhdr->oldestEnt++;
      if (++(a_cbhdr->oldestEntIdx) >= a_cbhdr->maxEntriesStored)
	a_cbhdr->oldestEntIdx = 0;
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_ENTRY_MSG, a_cbhdr->oldestEnt, a_cbhdr->oldestEntIdx);
...
{ /*gator_textcb_Write*/
    ...
    if (a_cbhdr == (struct gator_textcb_hdr *)0) {
      dce_svc_printf(GTX_S_TC_NULL_POINTER_MSG, rn);
      return(-1);
    }
    lock_Obtain(&(a_cbhdr->cbLock), WRITE_LOCK);
    ...
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_CURRENT_ENTRY_MSG, rn, a_cbhdr->currEnt, curr_ent_idx, a_cbhdr->maxEntriesStored, effective_highlight, curr_ent);
	...
	*/
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_TOP_WRITE_LOOP_MSG, rn, a_numChars);
	...
	/*
	  ...
	  */
	if (gator_textcb_debug)
	  dce_svc_printf(GTX_S_TC_ENTRY_INDEX_MSG, rn, a_cbhdr->currEnt, curr_ent_idx); 
	curr_ent = bumpEntry(a_cbhdr);
	curr_ent_idx = a_cbhdr->currEntIdx;
	if (gator_textcb_debug)
	  dce_svc_printf(GTX_S_TC_NEW_CB_ENTRY_MSG, rn, a_cbhdr->currEnt, a_cbhdr->currEntIdx, a_cbhdr->oldestEnt, a_cbhdr->oldestEntIdx, curr_ent);
      } /*Bump current entry*/
	...
	chars_to_copy = a_numChars;
      ...
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_COPY_TO_ENTRY_MSG, rn, chars_to_copy, dest, curr_ent->textp);
	...
	*/
	...
	/*
	  ...
	  */
	if (gator_textcb_debug)
	  dce_svc_printf(GTX_S_TC_HIGHLIGHT_MISMATCH_MSG, rn, curr_ent->charsUsed);
	if (curr_ent->numInversions < GATOR_TEXTCB_MAXINVERSIONS) {
	  ...
	  if (gator_textcb_debug)
	    dce_svc_printf(GTX_S_TC_OUT_OF_INVERSIONS_MSG, rn);
      } /*Handle inversion*/
	...
	*/
    ...
    if (a_skip) {
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_SKIP_TO_NEXT_ENTRY_MSG, rn);
      if (curr_ent->charsUsed > 0)
	curr_ent = bumpEntry(a_cbhdr);
      else
	if (gator_textcb_debug)
	  dce_svc_printf(GTX_S_TC_NOT_SKIPPING_MSG, rn);
    } /*Skip to the next entry*/

    /*
      * We can now unlock the CB and return successfully.
      */
    lock_Release(&(a_cbhdr->cbLock), WRITE_LOCK);
    return(0);
...
{ /*gator_textcb_BlankLine*/
    ...
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_OUTPUT_BLANK_LINES_MSG, rn, a_numBlanks, a_cbhdr);

    if (a_cbhdr == (struct gator_textcb_hdr *)0) {
      if (gator_textcb_debug)
	dce_svc_printf(GTX_S_TC_NULL_CB_POINTER_MSG, rn);
      return(-1);
    }
...
{ /*gator_textcb_Delete*/
    ...
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_DELETE_CIRCULAR_BUFFER_MSG, rn, a_cbhdr);
    lock_Obtain(&(a_cbhdr->cbLock), WRITE_LOCK);
    /*
      ...
      */
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_FREE_TEXT_BUFFER_MSG, rn, a_cbhdr->entry[0].textp,
	      (a_cbhdr->maxEntriesStored * a_cbhdr->maxCharsPerEntry));
    free(a_cbhdr->entry[0].textp);
    a_cbhdr->entry[0].textp = (char *)0;

    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_FREE_ENTRY_MSG, rn, a_cbhdr->entry,
    ...
    lock_Release(&(a_cbhdr->cbLock), WRITE_LOCK);
    if (gator_textcb_debug)
      dce_svc_printf(GTX_S_TC_FREE_CIRC_BUFFER_HEADER_MSG, rn, a_cbhdr);
    free(a_cbhdr);
    return(0);
