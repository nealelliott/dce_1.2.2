 ...
 * HISTORY
 * $Log: util_signal.c,v $
 * Revision 1.2  1996/12/03  10:42:44  salamone
 * Fix headers
 *
 ...
 * implied warranty.
 *-----------------------------------------------------------------------------
 * $Id: util_signal.c,v 1.2 1996/12/03 10:42:44 salamone Exp $
 *-----------------------------------------------------------------------------
 */
    ...
    int signalNum;
{
    ...
    if (signalNum != SIGCHLD) {
        if (SetSignalAction (signalNum, TclSignalTrap) == TCL_ERROR)
            panic ((char *)dce_sprintf(mcp_s_signal_bug));
    }
#endif
}

  ...
  badSpec:
    {
        ...
        badSpec [0] = scanPtr [1];
        badSpec [1] = '\0';
        DCP_APPEND_RESULT_CODE_ONE_ARG(mcp_s_bad_signal_format, badSpec);
        return TCL_ERROR;
    }
}

    ...
    int         signalNum;
{
	...
	i18n_signal_name = Tcl_SignalId(signalNum, &i18n_result);
        tmp = (char *)dce_sprintf(mcp_t_signal_trap_info, i18n_signal_name);
	if (i18n_result)
    ...
    int         signalNum;
{
	...
	if (i18n_result)
		free(signalName);
        DCP_APPEND_RESULT_CODE(mcp_t_signal_received); 
    ...
    int         signalList [];
{
    ...
    if (signalListSize > MAXSIG) {
        DCP_APPEND_RESULT_CODE(mcp_s_too_many_signals);
        goto errorExit;
    }

    if (signalListSize == 0) {
        DCP_APPEND_RESULT_CODE(mcp_s_signal_list_not_empty);
        goto errorExit;
    }
    ...
    for (idx = 0; idx < signalListSize; idx++) {
        ...
        if (signalName == NULL) {
            ...
            DCP_APPEND_RESULT_CODE_ONE_ARG(mcp_s_signal_invalid_num, numBuf);
            goto errorExit;
        }

        if ((signalNum < 1) || (signalNum > MAXSIG)) {
            DCP_APPEND_RESULT_CODE_ONE_ARG(mcp_s_signal_invalid_name, 
                                           signalName);
	    if (i18n_result)
		free(signalName);
            goto errorExit;
    ...
    int          signalList [MAXSIG];
{
    ...
    return TCL_OK;
#else
    interp->result = (char *)dce_sprintf(mcp_s_signal_posix_unavailable);
    return TCL_ERROR;
#endif
}

    ...
    char      **argv;
{
    ...
    if ((argc < 3) || (argc > 4)) {
        DCP_APPEND_RESULT_CODE_ONE_ARG(mcp_t_signal_usage, argv[0]);
        return TCL_ERROR;
    }
    ...
    if (STREQU (argv [1], "trap")) {
        ...
        if (argc != 4) {
            DCP_APPEND_RESULT_CODE(mcp_s_signal_trap_req_cmd);
            return TCL_ERROR;
        }
        command = argv [3];
    } else {
        if (STREQU (argv [1], "default")) {
            ...
            DCP_APPEND_RESULT_CODE_ONE_ARG(mcp_s_signal_invalid_action, 
                                           argv[1]);
            return TCL_ERROR;
        }
        if (argc != 3) {
            DCP_APPEND_RESULT_CODE_ONE_ARG(mcp_s_signal_takes_no_cmd, argv[1]);
            return TCL_ERROR;
        }
    }
    ...
    Tcl_Interp *interp;
{
    ...
    if (idx == numInterps)
        panic ((char *)dce_sprintf(mcp_s_signal_lost_interp));
