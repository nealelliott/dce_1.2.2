...
do
	echo; log "CLONE iteration $index"; echo;
	...
	do
		...
		run "ls $MNTDIR_FSET_1_1" 0 "Robustness check after clone iter $index failed" "$FATAL"
	done 
	index=`expr $index + 1`
done
...
while [ $index -le "$NUMITER" ]
do
	echo; log "RECLONE iteration $index"; echo;
	for test in 1 2 3 4 4a 5 5a 5b 5c 5d 5e 6 7 7a 7b 8 9 
	do
		runcthon $MNTDIR_FSET_1_1 $test	
		run "$EFTS reclone $AGGR1 $FSETID_1_1" 0 "Reclone iter $index failed" "$FATAL"
		run "ls $MNTDIR_FSET_1_1" 0 "Robustness check after reclone iter $index failed" "$FATAL"
	done
	index=`expr $index + 1`
done
