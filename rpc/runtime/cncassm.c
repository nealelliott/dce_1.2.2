 ...
 * HISTORY
 * $Log: cncassm.c,v $
 * Revision 1.1.90.2  1996/02/18  00:01:39  marty
 * 	Update OSF copyright years
 * 	[1996/02/17  22:54:23  marty]
 *
 * Revision 1.1.90.1  1995/12/08  00:17:02  root
 * 	Submit OSF/DCE 1.2.1
 * 
 * 	HP revision /main/HPDCE02/jrr_1.2_mothra/1  1995/11/16  21:29 UTC  jrr
 * 	Remove Mothra specific code
 * 
 * 	HP revision /main/HPDCE02/3  1995/05/25  21:39 UTC  lmm
 ...
 * 	[1995/12/07  23:57:26  root]
 * 
 * Revision 1.1.87.8  1994/09/01  16:12:50  ganni
 * 	Set the RPC_C_CN_ASSOC_AUTH_EXPECTED flag when sending an
 * 	authenticated bind-pdu/alter_context-pdu. If an unauthenticated
 * 	response pdu is received for an authenticated bind or alter_context,
 * 	set the status to rpc_s_level_authn_mismatch.
 * 	[1994/09/01  16:09:19  ganni]
 * 
 * Revision 1.1.87.7  1994/08/12  21:30:34  tatsu_s
 * 	Fix OT8157: Abort the association upon the INIT_WAIT/REQ event.
 * 
 * 	Made rpc_g_cn_assoc_client_events and
 * 	rpc_g_cn_assoc_client_states available for non-debugging build.
 * 	[1994/08/11  16:37:13  tatsu_s]
 * 
 * Revision 1.1.87.6  1994/07/14  21:31:05  tom
 * 	    07-jun-93   us   Made functional part of incr_active_action_rtn
 * 		        into an externally accessible macro
 * 			(RPC_CN_INCR_ACTIVE_CL_ACTION). Macro is
 * 		 	defined in cnsm.h.  Macro is used here and in
 * 		 	cnassoc.c as an optimization.
 ...
 * 	[1994/07/11  17:04:10  tom]
 * 
 * Revision 1.1.87.5  1994/06/21  21:53:34  hopkins
 * 	Change DIE to RPC_DCE_SVC_PRINTF.
 * 	[1994/06/08  21:28:34  hopkins]
 * 
 * 	Big PAC:
 * 	  Add version_mismatch predicate and retry_assoc action to
 * 	  support new (5.1) clients trying to talk to old (5.0) servers.
 * 	  Modify send_pdu to take old_server argument.
 * 	  Call fmt_client_req with old_server argument.
 * 	[1994/01/24  16:03:21  tom]
 * 
 * Revision 1.1.87.4  1994/05/19  21:14:23  hopkins
 * 	Merge with DCE1_1.
 * 	[1994/05/04  19:36:29  hopkins]
 * 
 * 	Serviceability work.
 * 	[1994/05/03  20:17:36  hopkins]
 * 
 * Revision 1.1.87.3  1994/03/17  23:06:35  tom
 * 	More Big PAC:
 * 	  For alter context, make sure we indicate to send_pdu that we
 * 	  are talking to an old server.
 * 	  Use protocol information in binding to determine if we are talking
 * 	  to an old (5.0) server.
 * 	[1994/03/17  23:04:07  tom]
 * 
 * Revision 1.1.87.2  1994/01/21  22:34:02  cbrooks
 * 	RPC Code Cleanup - Initial Submission
 * 	[1994/01/21  21:53:42  cbrooks]
 * 
 * Revision 1.1.87.1  1994/01/14  20:31:09  tom
 * 	Set first and last frag bits when no security.
 * 	Make sure all debug printfs have routine name.
 * 	Big PAC work - severely rewrite send_pdu()
 * 	[1994/01/13  22:50:58  tom]
 * 
 * Revision 1.1.6.1  1993/09/14  18:31:49  ganni
 * 	OT7968-correct the SM table for an OPEN association
 * 	with alter context response event - no action is necessary
 * 	and stay in the same state.
 * 	[1993/09/14  18:29:12  ganni]
 * 
 * Revision 1.1.4.6  1993/01/27  18:50:01  weisman
 * 	OT 6637, correct typo in debugging output.
 * 	[1993/01/27  18:41:14  weisman]
 * 
 * Revision 1.1.4.5  1993/01/03  22:58:22  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/03  19:58:56  bbelch]
 * 
 * Revision 1.1.4.4  1992/12/23  20:14:08  zeliff
 * 	Embedding copyright notice
 * 	[1992/12/23  15:30:44  zeliff]
 * 
 * Revision 1.1.4.3  1992/12/18  19:53:52  wei_hu
 * 	           3-dec-92    ko      No longer copy transfer syntax
 * 	                               UUID into presentation context
 * 	                               element. The stub does not require it.
 * 	[1992/12/18  19:43:42  wei_hu]
 * 
 * Revision 1.1.4.2  1992/10/13  20:53:45  weisman
 * 	     25-sep-92    ko      Change rpc__list_element_free of
 * 	                          presentation syntax element to
 * 	                               rpc__cn_assoc_syntax_free.
 * 	[1992/10/13  20:46:18  weisman]
 * 
 * Revision 1.1.2.5  1992/07/07  21:28:00  rsalz
 * 	     25-jun-92    ko      Use RPC_CN_ASSOC_WAKEUP macro instead
 * 	                          of queuing dummy fragbuf directly.
 * 	     19-jun-92    ko      Remove alignment of header size after
 * 	                          formatting of pres cont list in
 * 	                          send_pdu(). It should already be aligned.
 * 	     19-jun-92    ko      Fix the size in the pdu allocated for
 * 	                          an empty pres cont list in send_pdu().
 * 	                          We were using a pres cont elem with no
 * 	                          transfer syntaxes instead of a pres
 * 	                          cont list with no elements.
 * 	[1992/07/07  21:24:02  rsalz]
 * 
 * Revision 1.1.2.4  1992/06/23  21:37:41  sekhar
 * 	Drop the changes made by wei_hu
 * 	[1992/06/23  20:51:55  sekhar]
 * 
 * Revision 1.1.2.3  1992/05/22  18:57:24  wei_hu
 * 	21-may-92    wh      Update and set the key_id in send_pdu for
 * 	                     BIND and ALT_CTX.
 * 	12-may-92    sp      Don't try to lookup presentation context by
 * 	                     call id if presentation context list is empty.
 ...
 * 	                     packet after use.
 * 	 4-mar-92    wh      DCE 1.0.1 merge.
 * 	 2-mar-92    ko      Bug fix: memory leak when freeing presentation
 * 	                     context element when negotiation fails.
 * 	10-feb-92    ko      Bug fix: assign assoc_local_status before
 * 	                     generating assoc_local_error event when a
 * 	                     connect req fails.
 * 	[1992/05/22  18:42:48  wei_hu]
 * 
 * Revision 1.1  1992/01/19  03:09:20  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
    ...
    if (RPC_CN_PKT_AUTH_TLR_PRESENT (header))
    {
        ...
        if (assoc->assoc_status == rpc_s_ok)
        {
            ...
            if (assoc->security.auth_buffer_info.auth_buffer != NULL)
            {
                 ...
                 */
                if (process_frag_action_rtn(spc_struct, event_param, sm) != rpc_s_ok) {
		    sm_p->cur_state = RPC_C_CLIENT_ASSOC_ACTIVE;
		    return (assoc->assoc_status);
		}
...
#endif
{
    ...
    if (auth_buffer == NULL)
    {
        ...
        RPC_MEM_ALLOC(auth_buffer, unsigned8 *, 
                      ...
                      RPC_C_MEM_WAITOK);
	if (auth_buffer == NULL){
	    /* same code as RPC_CN_ASSOC_CHECK_ST */
	    rpc_cn_sm_event_entry_t     _event;
	    (assoc)->assoc_local_status = rpc_s_no_memory;
  ...
  ("(process_frag_action_rtn) Alloc'd auth_buffer: %x, auth_buffer_max = %d\n",
	...
	unsigned8 *tp = auth_buffer;
        ...
        RPC_MEM_REALLOC(auth_buffer, unsigned8 *,
                        ...
                        RPC_C_MEM_WAITOK);
	if (auth_buffer == NULL){
	    /* same code as RPC_CN_ASSOC_CHECK_ST except for auth_buffer line*/
	    rpc_cn_sm_event_entry_t     _event;
	    auth_buffer = tp;
	    (assoc)->assoc_local_status = rpc_s_no_memory;
	    _event.event_id = RPC_C_ASSOC_LOCAL_ERROR;
