...
$ ! HISTORY
$ ! $Log: nidlmsg.m,v $
$ ! Revision 1.1.15.2  1996/03/09  20:44:57  marty
$ ! 	Add OSF copyright
$ ! 	[1996/03/09  20:24:39  marty]
$ !
$ ! Revision 1.1.15.1  1995/12/07  22:21:35  root
$ ! 	Submit OSF/DCE 1.2.1
$ ! 	[1995/12/07  21:30:41  root]
$ ! 
$ ! Revision 1.1.11.3  1994/06/23  18:25:18  tom
$ ! 	From DEC: add opaque context handle support.
$ ! 	[1994/06/23  18:24:03  tom]
$ ! 
$ ! Revision 1.1.11.2  1994/05/03  18:50:42  tom
$ ! 	From DEC: add extended string support.
$ ! 	[1994/05/03  18:46:00  tom]
$ ! 
$ ! Revision 1.1.11.1  1994/02/14  15:54:06  rico
$ ! 	OT 9356 - new warning message when [fault_status] used on an IDL
$ ! 	encoding services operation.
$ ! 	[1994/02/11  16:22:58  rico]
$ ! 
$ ! 	CR 9356 - new error code for warning when [fault_status] used on an
$ ! 	Encoding Services operation.
$ ! 
$ ! Revision 1.1.9.1  1993/10/14  12:50:18  hinxman
...
$ ! 	[1993/10/14  12:49:48  hinxman]
$ ! 
$ ! Revision 1.1.7.3  1993/07/26  20:12:52  hinxman
$ ! 	OT 8309 - Check [cs_*tag] parameters have correct directionality
$ ! 	[1993/07/26  20:08:11  hinxman]
$ ! 
$ ! Revision 1.1.7.2  1993/07/07  20:46:16  ganni
$ ! 	reduced stub idl sources
$ ! 	[1993/07/07  20:44:06  ganni]
$ ! 
$ ! $EndLog$
$ !
$ !  Copyright (c) 1989 by
...
44 %1$s requires the %2$s command line option
$ Explanation:
$ There are particular options and constructs
$ that are extensions to the language, these
$ require a particular commandline option.
$ User Action:
$ Add the required option to the command line

45 ACF attributes in the IDL file, require the -app_config command line option
$ Explanation:
$ Placing ACF attributes in the IDL file are
$ extensions to the language, and require
$ the -app_config command line option.
$ User Action:
$ Add -app_config to the command line

46 Pipes must be defined with a typedef
$ Explanation:
...
47 Declaration of %1$s cannot contain an anonymous type
$ Explanation:
...
48 An arm of a union cannot be or contain a [ref] pointer
$ Explanation:
...
49 Array elements cannot be conformant arrays or conformant structures
$ Explanation:
...
50 Array elements cannot be context handles
$ Explanation:
...
51 Array elements cannot be pipes
$ Explanation:
...
52 Array size information required for %1$s
$ Explanation:
...
53 Arrays can be conformant in the first dimension only
$ Explanation:
...
54 An array with a pointer attribute is valid only as a parameter
$ Explanation:
...
55 Use array syntax to declare multidimensional arrays
$ Explanation:
...
56 Arrays can be varying in the first dimension only
$ Explanation:
...
57 Arrays with [transmit_as] cannot be conformant or varying
$ Explanation:
...
58 The attribute is no longer allowed in this context; use translator
$ Explanation:
...
59 An attribute variable cannot be an indirect field reference
$ Explanation:
...
61 The attribute [broadcast] is not valid on an operation with pipes
$ Explanation:
...
62 Case label type does not agree with discriminator type
$ Explanation:
...
63 Case label must be a constant from the enumeration of the discriminant
$ Explanation:
$ The value of a <kw>(case) expression in a discriminated <kw>(union)
...
64 Structures containing conformant arrays must be passed by reference
$ Explanation:
...
65 The base type of a pipe cannot be a conformant type
$ Explanation:
...
66 A conformant field must be the last field in the structure
$ Explanation:
...
67 Conformant array or structure is invalid within a union
$ Explanation:
...
68 Interface attributes [auto_handle] and [implicit_handle] cannot occur together
$ Explanation:
...
69 The attributes [in_line] and [out_of_line] cannot occur together

70 The attributes [%1$s] and [%2$s] cannot occur together
$ Explanation:
...
71 The [represent_as] type %1$s conflicts with previously defined %2$s
$ Explanation:
...
72 Constant name: %1$s not found
$ Explanation:
$ The named <kw>(constant) is not defined.
$ User Action:
$ Modify the interface and specify a known constant.

73 Constant type does not agree with constant expression
$ Explanation:
...
74 A hyper constant type is not allowed
$ Explanation:
...
75 Missing \"}\" on interface declaration
$ Explanation:
...
78 Missing result type on operation declaration
$ Explanation:
...
79 Missing type on parameter declaration
$ Explanation:
...
80 The base type of a pipe cannot be a [context_handle] type
$ Explanation:
...
81 Attribute [context_handle] only applies to void * types
$ Explanation:
...
82 Context handles are not valid as structure fields
$ Explanation:
...
83 Context handle is not valid as a member of a union
$ Explanation:
...
84 Definition of tag %1$s is not yet complete
$ Explanation:
...
87 Unexpected end-of-file near '%2$.*1$s'
$ Explanation:
$ The end of the source file was encountered before
$ the end of the interface definition.
$ The source file may be incomplete.
...
88 Syntax error in attribute list
$ Explanation:
...
89 Data type %1$s must be defined in %2$s
$ Explanation:
...
90 File is not a directory: %1$s
$ Explanation:
...
92 The [first_is] parameter must have the [in] attribute
$ Explanation:
...
93 A [first_is] variable must be a small, short, or long integer
$ Explanation:
...
94 Floating point constants not supported
$ Explanation:
$ Floating point constants cannot be specified in IDL.
$ User Action:
$ Remove the <kw>(float) constant specification.

95 Function pointers are not valid as elements of conformant arrays

96 Function pointers are allowed only in local interfaces

97 Function pointer parameters cannot be of type handle_t

98 Function pointers in an operation must be [in] parameters

99 The base type of a pipe cannot be a function pointer

100 Function pointers are not valid as structure fields

101 Function pointers are not valid as members of unions

102 Function types are allowed only in local interfaces

103 Array elements cannot be of type handle_t
$ Explanation:
...
104 A [handle] binding parameter must be [in] or [in,out]
$ Explanation:
...
105 Pointers to type handle_t are valid only in parameter declarations
$ Explanation:
$ Data type <kw>(handle_t) is only meaningful when used as the data
$ type of the first parameter in an operation.  It is used to
$ establish a binding to a server of the interface.
...
106 A handle_t as other than the first parameter requires a [transmit_as]
$ Explanation:
$ Data type <kw>(handle_t) is only meaningful when used as the data
...
107 The base type of a pipe cannot be handle_t
$ Explanation:
$ Data type <kw>(handle_t) is only meaningful when used as the data
$ type of the first parameter in an operation.  It is used to
$ establish a binding to a server of the interface.
...
108 A handle_t binding parameter must be an [in] parameter
$ Explanation:
...
109 Structure fields cannot be of type handle_t
$ Explanation:
$ Data type <kw>(handle_t) is only meaningful when used as the data
$ type of the first parameter in an operation.  It is used to
$ establish a binding to a server of the interface.
...
110 Members of unions cannot be of type handle_t
$ Explanation:
$ Data type <kw>(handle_t) is only meaningful when used as the data
$ type of the first parameter in an operation.  It is used to
$ establish a binding to a server of the interface.
...
111 A handle_t first parameter must not have [transmit_as] type
$ Explanation:
...
112 The attribute [idempotent] is not valid on an operation with pipes
$ Explanation:
...
122 A nonlocal interface cannot import a local interface
$ Explanation:
...
123 V1 attributes are incompatible with this type
$ Explanation:
...
124 Interface attributes [code] and [nocode] cannot occur together
$ Explanation:
...
126 Interface attributes [in_line] and [out_of_line] cannot occur together
$ Explanation:
...
127 ACF interface name %1$s differs from IDL interface name %2$s
$ Explanation:
...
128 Integer value overflows %1$s
$ Explanation:
...
134 Invalid octal digit in %1$s
$ Explanation:
...
135 Could not invoke the C preprocessor
$ Explanation:
...
136 The attribute [out_of_line] is not allowed on parameters
$ Explanation:
...
137 Command option -%1$s %2$s is not valid
$ Explanation:
...
138 Invalid parameters on command line:
$ Explanation:
...
139 Pointers to context handles are valid only in parameter declarations
$ Explanation:
...
140 Pointers to pipes are valid only in parameter declarations
$ Explanation:
...
141 The [last_is] parameter must have the [in] attribute
$ Explanation:
...
142 The attributes [last_is] and [length_is] cannot occur together
$ Explanation:
...
143 A [last_is] variable must be a small, short, or long integer
$ Explanation:
...
144 The lower bound must not be greater than the upper bound
$ Explanation:
...
145 The [length_is] parameter must have the [in] attribute
$ Explanation:
...
146 A [length_is] variable must be a small, short, or long integer
$ Explanation:
...
147 The major version number is too large; the maximum is %1$lu
$ Explanation:
...
148 The attribute [max_is] cannot be applied to dimension %1$lu; upper bound is not dynamic
$ Explanation:
...
149 Maximum identifier length for interface name is %1$lu characters
$ Explanation:
...
150 Maximum identifier length for type with [transmit_as] is %1$lu characters
$ Explanation:
$ The IDL compiler constructs new identifier names
$ that are referenced by generated stub code
...
151 Maximum identifier length for [handle] type is %1$lu characters
$ Explanation:
$ The IDL compiler constructs new identifier names
$ that are referenced by generated stub code
...
152 Maximum identifier length for [context_handle] type is %1$lu characters
$ Explanation:
$ The IDL compiler constructs new identifier names
$ that are referenced by generated stub code
...
153 Maximum identifier length for pointed-to type is %1$lu characters
$ Explanation:
$ The IDL compiler constructs new identifier names
$ that are referenced by generated stub code
...
154 Maximum identifier length for pipe type is %1$lu characters
$ Explanation:
$ The IDL compiler constructs new identifier names
$ that are referenced by generated stub code
...
155 Maximum identifier length for [represent_as] type is %1$lu characters
$ Explanation:
$ The IDL compiler constructs new identifier names
$ that are referenced by generated stub code
...
156 Maximum identifier length for [out_of_line] type is %1$lu characters
$ Explanation:
$ The IDL compiler constructs new identifier names
$ that are referenced by generated stub code
...
157 A [max_is] parameter must have the [in] attribute
$ Explanation:
...
158 The attribute [max_is] or [size_is] is required
$ Explanation:
...
159 The attributes [max_is] and [size_is] cannot occur together
$ Explanation:
...
160 A [max_is] variable must be a small, short, or long integer
$ Explanation:
...
161 A bit field specification can be applied only to simple, integral types
$ Explanation:
$ A bit field specification indicates the bit width of a simple
$ integral type.  This cannot be applied to other types such as
$ pointers and structs for obvious reasons.
$ User Action:
$ Remove bit field specifications that appear on non-integral 
$ simple types.

162 A [maybe] operation cannot have [out] parameters or a function result
$ Explanation:
...
163 The attribute [min_is] is required
$ Explanation:
...
164 The attribute [min_is] cannot be applied to dimension %1$lu; lower bound is not dynamic
$ Explanation:
...
165 A [min_is] parameter must have the [in] attribute
$ Explanation:
...
166 The minor version number is too large; the maximum is %1$lu
$ Explanation:
...
167 A [min_is] variable must be a small, short, or long integer
$ Explanation:
...
169 Name is not a constant: %1$s
$ Explanation:
...
172 Name is not a parameter: %1$s
$ Explanation:
$ The name referenced in an array attribute is not
...
174 Name %1$s previously declared in file %2$s, line %3$lu
$ Explanation:
...
175 %1$sNLS message catalog version mismatch in \"%2$s\", Expected: \"%3$lu\", Actual: \"%4$s\"

176 %1$sError messages may be incorrect
$ Explanation:
...
177 Non-integer values are not allowed in expressions
$ Explanation:
...
178 The [align] attribute is not yet supported

179 The attribute [in(shape)] is not yet supported

180 Arrays with a nonzero lower bound are not yet supported

181 The attribute [out(shape)] is not yet supported

182 Attribute [unique] is not yet supported

183 Operation attributes [code] and [nocode] cannot occur together
$ Explanation:
...
184 Unable to open %1$s for read access
$ Explanation:
...
185 Unable to open %1$s for write access
$ Explanation:
...
186 Operation name %1$s referenced in ACF is not defined
$ Explanation:
...
187 An [out] conformant array must be a top-level parameter or under a non-[ref] pointer
$ Explanation:
...
188 Output parameters must be passed by reference
$ Explanation:
...
189 An [out,ptr] parameter is not valid
$ Explanation:
...
190 An [out] parameter or operation result cannot contain [unique] pointers
$ Explanation:
...
191 An operation result cannot be a pipe
$ Explanation:
...
192 Output parameters require an explicit top-level *
$ Explanation:
$ Parameters with the <kw>([out]) or <kw>([in,out]) attributes must be
$ passed by reference, so the changed value of the
$ parameter can be reflected back to the caller.
...
193 An [out,unique] parameter is not valid
$ Explanation:
...
194 The base type of a pipe cannot be a pipe type
$ Explanation:
...
195 Pipes are not valid as structure fields
$ Explanation:
...
196 Pipes are not valid as members of unions
$ Explanation:
...
197 A pipe cannot have a [transmit_as] type
$ Explanation:
...
198 A parameter with [%1$s] cannot be passed by value
$ Explanation:
...
199 A parameter must have either or both the [in] and [out] attributes
$ Explanation:
...
200 Parameter attributes [in_line] and [out_of_line] cannot occur together

201 Parameter name %1$s referenced in ACF operation %2$s is not defined
$ Explanation:
...
202 The attribute [ptr] is invalid on a binding handle parameter
$ Explanation:
...
203 The attribute [ptr] is valid only for pointer types or array parameter types
$ Explanation:
...
204 The base type of a pipe cannot be or contain a pointer
$ Explanation:
...
205 Incorrect syntax for pointer to conformant array
$ Explanation:
$ An IDL declaration attempts to use mixed pointer <kw>(*) and
...
206 Full pointers to context handles are not allowed
$ Explanation:
...
207 The attribute [ptr] is not valid on pipe parameters
$ Explanation:
...
208 Pointers to [v1_enum] types are not allowed
$ Explanation:
...
209 Pointers to varying arrays are not allowed
$ Explanation:
$ An IDL declaration attempts to use mixed pointer <kw>(*) and
$ array <kw>([]) syntax to declare a pointer to an array that has
...
210 void * must be used in conjunction with the [context_handle] attribute
$ Explanation:
...
211 The attribute [ref] is valid only for pointer types or array parameter types
$ Explanation:
...
212 A [ref] function result is not valid
$ Explanation:
...
213 Rename of %1$s to %2$s failed
$ Explanation:
...
214 Types with [represent_as] cannot be nested
$ Explanation:
$ IDL does not allow a data type that has a <kw>([represent_as]) type
$ to itself be used as a <kw>([represent_as]) type in another type
$ definition.
...
216 Size attributes can only be applied to array types
$ Explanation:
...
217 The attribute [size_is] cannot be applied to dimension %1$lu; upper bound is not dynamic
$ Explanation:
...
218 A [size_is] parameter must have the [in] attribute
$ Explanation:
...
219 The size attributes do not match the array dimension
$ Explanation:
...
220 The array size attribute variable %1$s cannot have the [ptr] or [unique] attributes
$ Explanation:
...
221 A [size_is] variable must be a small, short, or long integer
$ Explanation:
...
222 A size attribute variable must not have a represent_as type
$ Explanation:
...
223 A size attribute variable must not have a transmit_as type
$ Explanation:
$ The parameter or field referenced
$ in a <kw>([max_is]),
$ <kw>([size_is]), <kw>([first_is]), <kw>([last_is]),
...
224 The attribute [v1_array] must be in array, not pointer, syntax
$ Explanation:
...
225 A conformant [v1_array] must also be varying
$ Explanation:
...
226 The attribute [v1_array] is invalid for an array with more than 65535 elements
$ Explanation:
...
227 The attribute [v1_array] cannot occur with the [min_is] or [first_is] attributes
$ Explanation:
...
228 A [v1_array] can be conformant or varying in first dimension only
$ Explanation:
...
229 A [v1_array] cannot have a conformant lower bound
$ Explanation:
...
230 Arrays of strings are not allowed

231 The attribute [string] cannot be applied to a [v1_array]
$ Explanation:
...
232 The attribute [string] is valid only for one-dimensional array of valid base type
$ Explanation:
...
233 Structures with [transmit_as] or [represent_as] cannot be conformant
$ Explanation:
...
234 Unterminated string literal
$ Explanation:
...
235 The [v1_string] attribute can only be applied to a [v1_array]
$ Explanation:
...
236 A [v1_string] must be an array of char with fixed bounds
$ Explanation:
$ A <kw>([v1_string]) is compatible with
$ the NCS Version 1 NIDL <kw>(string0) data
...
237 A [string] array cannot have varying array attributes
$ Explanation:
...
238 The attribute [%1$s] can occur at most once per operation
$ Explanation:
...
239 A parameter with [%1$s] must be an [out] parameter
$ Explanation:
...
240 An operation with [%1$s] must return a value of type error_status_t
$ Explanation:
...
241 A parameter with [%1$s] must be of type error_status_t
$ Explanation:
$ A <kw>([comm_status]) or <kw>([fault_status]) parameter
$ or function result
...
244 Syntax error in UUID format
$ Explanation:
...
247 Too many endpoint specifications; Maximum is %1$lu
$ Explanation:
...
249 Type attributes [in_line] and [out_of_line] cannot occur together
$ Explanation:
...
250 Type name %1$s referenced in ACF is not defined
$ Explanation:
...
254 A union discriminator type must be small, short, long, char, boolean, or enum
$ Explanation:
...
255 The attribute [unique] is invalid on a binding handle parameter
$ Explanation:
$ The first parameter in an operation is a <kw>(handle) parameter,
...
256 The attribute [unique] is valid only for pointer types or array parameter types
$ Explanation:
...
257 [unique] pointers to context handles are not allowed
$ Explanation:
...
258 A [unique] function result is not valid
$ Explanation:
...
261 The attribute [uuid] is invalid when the [local] attribute is specified
$ Explanation:
...
262 Variable declarations are not supported; declaration of %1$s ignored
$ Explanation:
...
263 void is valid only in an operation or a context handle pointer declaration
$ Explanation:
...
264 A type used in a [transmit_as] clause cannot have a [represent_as] type
$ Explanation:
...
265 A [transmit_as] or [represent_as] type cannot be a conformant array
$ Explanation:
...
266 The base type of a pipe cannot be a [transmit_as] type
$ Explanation:
...
267 A translated transmittable type cannot contain [%1$s] pointers
$ Explanation:
...
268 A type with [transmit_as] cannot have other type attributes
$ Explanation:
...
269 Command option -bug %1$lu conflicts with -no_bug %2$lu
$ Explanation:
$ The command line options conflict with each other.
$ User Action:
$ Remove one of the conflicting options.

270 Command option %1$s conflicts with %2$s
$ Explanation:
$ The command line options conflict with each other.
$ User Action:
$ Remove one of the conflicting options.

271 Unterminated comment, end-of-file encountered
$ Explanation:
...
274 Command option -bug %1$lu is not known
$ Explanation:
...
275 Command option -no_bug %1$lu is not known
$ Explanation:
$ The documentation defines all the valid integers that can
$ accompany the <kw>(-bug) and <kw>(-no_bug) options.  You have chosen
$ an invalid value.
...
276 Reached maximum of %1$lu warnings; exiting
$ Explanation:
...
277 Command option %1$s does not take a value
$ Explanation:
...
279 Length of source filename and stub suffix exceed 8 chars

280 A source IDL filename is required
$ Explanation:
...
281 Unknown command line option: %1$s
$ Explanation:
...
282 Warning: Use of a nonencapsulated union requires %1$s
$ Explanation:
...
283 Warning: Use of [unique] pointers requires %1$s
$ Explanation:
...
284 An [in] or [in,out] union must have an [in] discriminator
$ Explanation:
...
285 An [in,out] or [out] union must have an [out] discriminator
$ Explanation:
...
286 A [switch_is] variable must not have a [represent_as] type
$ Explanation:
...
287 A [switch_is] variable must not have a [transmit_as] type
$ Explanation:
$ The parameter or field referenced in a <kw>([switch_is]) clause
$ must not be of a data type that has been declared to have a
$ <kw>([transmit_as]) type.
...
288 An encapsulated union member cannot have the [%1$s] attribute
$ Explanation:
...
289 Arrays of nonencapsulated unions are not allowed
$ Explanation:
...
290 A [represent_as] type cannot be a nonencapsulated union
$ Explanation:
...
291 A nonencapsulated union cannot have a [represent_as] type
$ Explanation:
...
292 A nonencapsulated union declaration must have a [switch_is] attribute
$ Explanation:
...
293 The union switch variable %1$s cannot be a [ptr] or [unique] pointer
$ Explanation:
...
294 A nonencapsulated union type must have a [switch_type] attribute
$ Explanation:
...
295 A [transmit_as] type cannot be a nonencapsulated union
$ Explanation:
...
296 A nonencapsulated union cannot have a [transmit_as] type
$ Explanation:
$ A nonencapsulated union type cannot be defined to have a
...
297 Cannot have more than one level of indirection to a nonencapsulated union
$ Explanation:
...
298 A [switch_is] attribute is only valid on a nonencapsulated union
$ Explanation:
...
299 Data type of [switch_is] variable %1$s does not agree with [switch_type] %2$s
$ Explanation:
...
300 A [switch_type] attribute is only valid on a nonencapsulated union type
$ Explanation:
...
301 A structure field cannot be of a presented type for which the transmitted type is conformant
$ Explanation:
...
302 A type with the [represent_as] attribute cannot be conformant
$ Explanation:
...
303 An arm of a union cannot be or contain a [unique] pointer
$ Explanation:
$ IDL-generated server stub code must allocate storage for
...
304 Warning: Conformant structures not supported in target language
$ Explanation:
...
305 Warning: Conformant strings not supported in target language
$ Explanation:
...
306 Warning: Character doesn't map to target language
$ Explanation:
...
307 Warning: Pipes not supported for target language
$ Explanation:
...
308 Warning: Non-scalar function result converted to trailing [out] parameter
$ Explanation:
...
309 Target language supports maximum of %1$lu array dimensions
$ Explanation:
...
310 Warning: Name %1$s differs from a previous name only in case
$ Explanation:
...
311 Warning: Attribute [%1$s] not supported for target language
$ Explanation:
...
312 Array function results are not allowed
$ Explanation:
...
313 Attribute [%1$s] cannot be applied to a void * type
$ Explanation:
...
314 The [handle] attribute is valid only on transmittable types
$ Explanation:
...
315 Warning: Forward tag reference %1$s in this declaration is not ANSI C compliant
$ Explanation:
...
316 Attribute cannot be used more than once
$ Explanation:
$ An attribute is repeated multiple times in an attribute list.
$ User Action:
$ Remove all but one occurrence of the offending attribute.

317 Type with [%1$s] cannot be used in definition of type with [%2$s]
$ Explanation:
...
318 Warning: Use of nonzero array lower bound requires %1$s
$ Explanation:
...
321 Operation with [encode] must have at least one [in] or [in,out] parameter
$ Explanation:
...
322 Operation with [decode] must have at least one [out] or [in,out] parameter
$ Explanation:
...
323 Warning: Operation with [reflect_deletions] has no [in] or [in,out] full pointers
$ Explanation:
...
324 Warning: Operation with [encode] has [out]-only parameter %1$s
$ Explanation:
...
325 Warning: Operation with [decode] has [in]-only parameter %1$s
$ Explanation:
...
326 Operation with [encode,decode] must have all [in,out] parameters
$ Explanation:
...
327 Operation with [encode] or [decode] may not contain pipe parameters
$ Explanation:
...
328 Operation with [encode] or [decode] must use explicit binding
$ Explanation:
...
329 Exception name %1$s referenced in ACF is not defined
$ Explanation:
...
330 Invalid file specification: %1$s
$ Explanation:
...
331 File specification required for %1$s
$ Explanation:
$ The indicated IDL command option requires a file specification.
$ User Action:
$ Supply a file specification following the option.

332 Warning: Operation with [cs_tag_rtn] contains no codeset tag parameters
$ Explanation:
...
333 Types with the [%1$s] attribute cannot be nested
$ Explanation:
...
334 The [%1$s] attribute cannot be duplicated in the same parameter list
$ Explanation:
...
335 Operation with [in] [cs_char] data requires [cs_stag] parameter
$ Explanation:
...
336 Operation with [out] [cs_char] data requires [cs_rtag] parameter
$ Explanation:
...
337 Array attribute variable cannot be used for both [cs_char] and non-[cs_char] arrays
$ Explanation:
...
338 An [in,size_is] parameter for an [out,cs_char] array cannot be used for other array attributes

339 A [size_is] or [max_is] attribute cannot be applied to a pointer to a [cs_char] type
$ Explanation:
...
340 An array with [%1$s] base type cannot have the [ptr] or [unique] attributes
$ Explanation:
...
341 A [transmit_as] transmitted type cannot contain a [%1$s] type
$ Explanation:
...
342 A [cs_stag] parameter must precede any [in] [cs_char] data in a parameter list
$ Explanation:
...
343 A [cs_rtag] parameter must precede any [out] [cs_char] data in a parameter list
$ Explanation:
...
344 A [handle] binding parameter cannot contain a [%1$s] type
$ Explanation:
...
345 The base type of a pipe cannot be or contain a [%1$s] type
$ Explanation:
...
346 Arrays of [%1$s] type cannot be multidimensional
$ Explanation:
...
347 Arrays of [%1$s] type can only use the [size_is] and [length_is] array attributes
$ Explanation:
$ The indicated attribute cannot be applied to a type if there
...
348 Type with [%1$s] cannot be or contain type with [%2$s]
$ Explanation:
...
349 Type with [%1$s] must resolve to byte or structure containing only byte fields
$ Explanation:
...
350 Tag parameters must have type unsigned long int passed by value or reference
$ Explanation:
...
351 Maximum identifier length for [%1$s] type is %2$lu characters
$ Explanation:
...
352 Warning: Use of user-defined exceptions requires %1$s
$ Explanation:
...
353 Warning: Use of [%1$s] attribute requires %2$s
$ Explanation:
...
354 A [%1$s] parameter must have the [%2$s] attribute
$ Explanation:
...
355 Warning: Use of C preprocessor directive requires %1$s
$ Explanation:
...
356 Warning: Use of anonymous %1$s may not be portable across C compilers
$ Explanation:
...
357 Warning: A [fault_status] parameter has no utility in this operation
$ Explanation:
...
358 A [cxx_new] operation must return an interface pointer
$ Explanation:
$ A <kw>([cxx_new]) operation attribute is used to identify a 
$ remote object creator operation. A remote object creator
$ operation must return a pointer to an interface.
$ User Action:
$ Insure that the return value for the operation is 
$ an <interface-name> * type.

359 Interface reference %1$s allowed only as target of a * or & pointer
$ Explanation:
$ When an IDL inteface is compiled with <kw>(-lang cxx), the
...
$ User Action:
$ Change the declaration to use a * or &, or change the type
$ used in the declaration.

360 Warning: Pointer attributes are ignored on an interface reference
$ Explanation:
$ Interface references are not managed in the same way as
...
$ User Action:
$ No action is required, since the pointer attribute is ignored
$ and the IDL compiler continues processing.  To eliminate the
$ warning message, remove the pointer attribute from the
$ declaration.

361 Warning: Static property or ACF attribute [cxx_static] is ignored except for -lang cxx
$ Explanation:
$ The IDL <kw>(static) keyword on an operation, as well as the ACF
$ <kw>([cxx_static]) attribute on an operation, only pertain to
$ interfaces that are compiled with the <kw>(-lang cxx) IDL
$ compiler option.
$ User Action:
$ If your application code is written in C++, then you must use
...
362 ACF attribute [cxx_lookup] is only valid for -lang cxx
$ Explanation:
$ The ACF <kw>([cxx_lookup]) interface attribute only pertains to
$ interfaces that are compiled with the <kw>(-lang cxx) IDL
$ compiler option.
$ User Action:
$ If your application code is written in C++, then you must use
...
363 ACF attribute [cxx_new] is only valid for -lang cxx
$ Explanation:
$ The ACF <kw>([cxx_new]) operation attribute only pertains to
$ interfaces that are compiled with the <kw>(-lang cxx) IDL
$ compiler option.
$ User Action:
$ If your application code is written in C++, then you must use
...
364 IDL inheritance operator ':' is only valid for -lang cxx
$ Explanation:
$ The Inheritance operator ':' is only valid for
$ interfaces that are compiled with the <kw>(-lang cxx) IDL
$ compiler option.
$ User Action:
$ If your application code is written in C++ and using interface inheritance,
...
365 Inherited interfaces must be imported: %1$s 
$ Explanation:
$ No definition for the inherited interface identified by the 
$ inheritance operator was found.
$ User Action:
$ If your application code is using interface inheritance,
$ then you must import a definition for the inherited
$ interface.
$ Otherwise, edit the <v>(interface-name)).idl file,
$ and remove any uses of the inheritance ':' operator.

366 ACF attribute [cxx_delegate] is only valid for -lang cxx
$ Explanation:
$ The ACF <kw>([cxx_delegate]) interface attribute only pertains to
$ interfaces that are compiled with the <kw>(-lang cxx) IDL
$ compiler option.
$ User Action:
$ If your application code is written in C++, then you must use
