...
/*
 * HISTORY
 * $Log: acct_admin_repl.c,v $
 * Revision 1.1.11.3  1996/02/17  23:25:00  marty
 * 	Update OSF copyright year
 * 	[1996/02/17  22:43:13  marty]
 *
 * Revision 1.1.11.2  1995/12/11  20:20:11  root
 * 	Submit OSF/DCE 1.2.1
 * 	[1995/12/11  19:51:50  root]
 * 
 * Revision 1.1.7.1  1994/10/25  17:17:53  sekhar
 * 	[ OT 12398 ] call sec_rgy_cursor_reset() before account lookup.
 * 	[1994/10/24  18:10:58  sekhar]
 * 
 * 	[ OT 12398 ]  Checking in Sean Mullan's changes for testing
 * 		      security replication.
 * 	[1994/10/24  16:59:24  sekhar]
 * 
 * Revision 1.1.4.1  1993/10/07  18:58:56  mccann
 * 	CR9057 remove unnecessary verison requirement
 * 	[1993/10/07  18:57:08  mccann]
 * 
 * Revision 1.1.2.3  1993/02/25  22:03:35  cuti
 * 	Change sec_rgy_site_open() to sec_rgy_site_open_update() for using site update operation.
 * 	[1993/02/25  16:27:44  cuti]
 * 
 * Revision 1.1.2.2  1992/12/31  17:27:21  htf
 * 	Embedding copyright notice
 * 	[1992/12/31  16:52:51  htf]
 * 
 * Revision 1.1  1992/01/19  04:07:27  devrcs
 * 	Initial revision
 * 
 * $EndLog$
 */
 ...
 *	from delta on	11/13/91 14:17:15
 *
 *  $LOG$
 *
 *
...
#ifndef LINT
static char sccs_id[] = "src/test/security/api/moretests/acct_admin_repl.c, , dce_osf, dce.75d";
#endif
...
/* Local variables */
   ...
   sec_rgy_acct_admin_t	repl_admin_part;/* sec_rgy_acct_admin_replace */
   sec_rgy_login_name_t *test_login_name;/* sec_rgy_acct_admin_replace */
...
/* Add an additional test org item to the registry for the test of
   ...
   if (subcase == ACCT_ADMIN_REPL_UNIQUE_KEY_PARTS)
     {
...
/* Call the rpc under test. */
   ...
   if (subcase == ACCT_ADMIN_REPL_NOT_FOUND)
     test_login_name = &ACCT_NON_LOGIN_NAME;
   else
     test_login_name = &ACCT_LOGIN_NAME;

   sec_rgy_acct_admin_replace (write_context,
			       test_login_name,
			       &key_parts,
			       &repl_admin_part,
			       &tstatus);
   TSHMessage (ctx, MOTRACE,
               "sec_rgy_acct_admin_replace status: %d\n", tstatus);
   ...
   if (estatus == tstatus)
       ...
       if (subcase != ACCT_ADMIN_REPL_NOT_FOUND)
	 {
        ...
        /* Even if tstatus came back with the expected return on the 
           acct_admin_replace above, check to see if the account was 
           ...
           TSHMessage (ctx, MOTRACE,
                       "sec_rgy_acct_lookup (after repl) status: %d\n", tstatus);

        /* If the lookup failed or if the admin_part returned from the lookup
           did not match the admin_part supplied as input to admin_replace,
    ...
    /* If this was the test of the non-unique key_parts replacement and
	 ...
	 {
	   TSHMessage (ctx, MOFAILURE, "sec_rgy_acct_admin_replace failed\n");
	   tstatus = TSH_TEST_FAILED;
         }
     }
    ...
    TSHMessage (ctx, MOFAILURE,
                  "sec_rgy_acct_admin_replace failed\n");
