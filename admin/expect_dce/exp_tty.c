 ...
 * HISTORY
 * $Log: exp_tty.c,v $
 * Revision 1.1.2.1  1996/08/09  19:18:50  arvind
 * 	Change for tcl_dce changes
...
/* (CleanupChildren in tclUnixAZ.c) that we can't use directly. */
	...
	if (!WIFEXITED(waitStatus) || (WEXITSTATUS(waitStatus) != 0)) {
		...
		Tcl_SetErrorCode(interp, "CHILDKILLED", msg1,
			Tcl_SignalId((int) (WTERMSIG(waitStatus)), &dummy_res), p,
			(char *) NULL);
		...
		Tcl_SetErrorCode(interp, "CHILDSUSP", msg1,
			Tcl_SignalId((int) (WSTOPSIG(waitStatus)), &dummy_res), p, (char *) NULL);
